...
class LZMADecoder extends CoderBase {
  LZMADecoder() { super(LZMA2Options.class, Number.class); }
  ...
  InputStream decode(final String archiveName, final InputStream in,
    ...
    if (dictSize > LZMAInputStream.DICT_SIZE_MAX) {
      throw new IOException(
          "Dictionary larger than 4GiB maximum size used in " + archiveName);
    }
  ...
  OutputStream encode(final OutputStream out, final Object opts)
    ...
    return new FlushShieldFilterOutputStream(
        new LZMAOutputStream(out, getOptions(opts), false));
  }
  ...
  byte[] getOptionsAsProperties(final Object opts) throws IOException {
    ...
    final byte props =
        (byte)((options.getPb() * 5 + options.getLp()) * 9 + options.getLc());
  ...
  private int getDictionarySize(final Coder coder)
