...
public class SevenZOutputFile implements Closeable {
   ...
   * @since 1.13
   */
  public SevenZOutputFile(final SeekableByteChannel channel)
      throws IOException {
  ...
  public void setContentCompression(final SevenZMethod method) {
    setContentMethods(
        Collections.singletonList(new SevenZMethodConfiguration(method)));
  }
  ...
  public void setContentMethods(
      final Iterable<? extends SevenZMethodConfiguration> methods) {
  ...
  public void finish() throws IOException {
    ...
    ByteBuffer bb =
        ByteBuffer
            .allocate(SevenZFile.sevenZSignature.length + 2 /* version */
                      + 4 /* start header CRC */
  ...
  private CountingOutputStream setupFileOutputStream() throws IOException {
    ...
    boolean first = true;
    for (final SevenZMethodConfiguration m :
         getContentMethods(files.get(files.size() - 1))) {
      if (!first) {
    ...
    if (!moreStreams.isEmpty()) {
      additionalCountingStreams =
          moreStreams.toArray(new CountingOutputStream[moreStreams.size()]);
    }
  ...
  getContentMethods(final SevenZArchiveEntry entry) {
    final Iterable<? extends SevenZMethodConfiguration> ms =
        entry.getContentMethods();
  ...
  private void writePackInfo(final DataOutput header) throws IOException {
    ...
    for (final SevenZArchiveEntry entry : files) {
      if (entry.hasStream()) {
        header.writeInt(
            Integer.reverseBytes((int)entry.getCompressedCrcValue()));
      }
    }
  ...
  private void writeFolder(final DataOutput header,
                           final SevenZArchiveEntry entry) throws IOException {
  ...
  private void writeSingleCodec(final SevenZMethodConfiguration m,
                                final OutputStream bos) throws IOException {
  ...
  private void writeFileEmptyStreams(final DataOutput header)
      throws IOException {
  ...
  private void writeFileMTimes(final DataOutput header) throws IOException {
    ...
    if (numLastModifiedDates > 0) {
      ...
      for (final SevenZArchiveEntry entry : files) {
        if (entry.getHasLastModifiedDate()) {
          out.writeLong(Long.reverseBytes(SevenZArchiveEntry.javaTimeToNtfsTime(
  ...
  private void writeFileWindowsAttributes(final DataOutput header)
      throws IOException {
  ...
  private void writeUint64(final DataOutput header, final long value)
      throws IOException {
    int firstByte = 0;
    int mask = 0x80;
    int i;
  ...
  private void writeBits(final DataOutput header, final BitSet bits,
