 ...
 * @since 1.17
 */
public SevenZFile(final File filename, final char[] password)
throws IOException {
	this(new FileInputStream(filename).getChannel(), filename.getAbsolutePath(),
	     utf16Decode(password), true);
}
 ...
 */
public SevenZFile(final File filename, final byte[] password)
throws IOException {
	this(new FileInputStream(filename).getChannel(), filename.getAbsolutePath(),
	     password, true);
}
 ...
 * @since 1.17
 */
public SevenZFile(final FileChannel channel, final char[] password)
throws IOException {
 ...
 * @since 1.17
 */
public SevenZFile(final FileChannel channel, final String filename,
 ...
 * @deprecated use the char[]-arg version for the password instead
 */
public SevenZFile(final FileChannel channel, final byte[] password)
throws IOException {
...
private SevenZFile(final FileChannel channel, final String filename,
...
private StartHeader readStartHeader(final long startHeaderCrc)
throws IOException {
	final StartHeader startHeader = new StartHeader();
	// using Stream rather than ByteBuffer for the benefit of the
	// built-in CRC check
	try (DataInputStream dataInputStream =
		     new DataInputStream(new CRC32VerifyingInputStream(
						 new BoundedFileChannelInputStream(channel, 20), 20,
						 startHeaderCrc))) {
		...
		startHeader.nextHeaderCrc =
			0xffffFFFFL & Integer.reverseBytes(dataInputStream.readInt());
		return startHeader;
	}
}
...
private void readArchiveProperties(final ByteBuffer input)
throws IOException {
...
private ByteBuffer readEncodedHeader(final ByteBuffer header,
...
throws IOException {
	...
	InputStream inputStreamStack = new BoundedFileChannelInputStream(
	...
	for (final Coder coder : folder.getOrderedCoders()) {
		...
		inputStreamStack = Coders.addDecoder(
			fileName, inputStreamStack, // NOSONAR
			folder.getUnpackSizeForCoder(coder), coder, password);
	}
	if (folder.hasCrc) {
		inputStreamStack = new CRC32VerifyingInputStream(
			inputStreamStack, folder.getUnpackSize(), folder.crc);
	}
	final byte[] nextHeader = new byte[(int)folder.getUnpackSize()];
	try (DataInputStream nextHeaderInputStream =
		     new DataInputStream(inputStreamStack)) {
		nextHeaderInputStream.readFully(nextHeader);
	}
	return ByteBuffer.wrap(nextHeader).order(ByteOrder.LITTLE_ENDIAN);
}

private void readStreamsInfo(final ByteBuffer header, final Archive archive)
throws IOException {
	int nid = getUnsignedByte(header);
...
private void readUnpackInfo(final ByteBuffer header, final Archive archive)
throws IOException {
	int nid = getUnsignedByte(header);
	if (nid != NID.kFolder) {
...
private void readSubStreamsInfo(final ByteBuffer header,
                                final Archive archive) throws IOException {
	...
	for (final Folder folder : archive.folders) {
		...
		subStreamsInfo.unpackSizes[nextUnpackStream++] =
			...
			if (folder.numUnpackSubStreams == 1 && folder.hasCrc) {
				...
				for (int i = 0; i < folder.numUnpackSubStreams; i++) {
					subStreamsInfo.hasCrc.set(nextCrc,
					                          hasMissingCrc.get(nextMissingCrc));
...
throws IOException {
	...
	for (int i = 0; i < files.length; i++) {
		...
		if (files[i].hasStream()) {
			...
			files[i].setHasCrc(
				archive.subStreamsInfo.hasCrc.get(nonEmptyFileCounter));
			files[i].setCrcValue(archive.subStreamsInfo.crcs[nonEmptyFileCounter]);
			files[i].setSize(
				archive.subStreamsInfo.unpackSizes[nonEmptyFileCounter]);
			++nonEmptyFileCounter;
		} else {
...
private void calculateStreamMap(final Archive archive) throws IOException {
	...
	final int numPackSizes =
		archive.packSizes != null ? archive.packSizes.length : 0;
	...
	for (int i = 0; i < archive.files.length; i++) {
		...
		if (nextFolderUnpackStreamIndex >=
		    archive.folders[nextFolderIndex].numUnpackSubStreams) {
			++nextFolderIndex;
...
private void buildDecodingStream() throws IOException {
	...
	if (currentFolderIndex == folderIndex) {
		...
		// but don't do it eagerly -- if the user skips over the entire folder
		// nothing is effectively decompressed.

		file.setContentMethods(
			archive.files[currentEntryIndex - 1].getContentMethods());
	} else {
		...
		final long folderOffset =
			SIGNATURE_HEADER_SIZE + archive.packPos +
	...
	InputStream fileStream =
		new BoundedInputStream(currentFolderInputStream, file.getSize());
	if (file.getHasCrc()) {
		fileStream = new CRC32VerifyingInputStream(fileStream, file.getSize(),
		                                           file.getCrcValue());
	}
...
private InputStream buildDecoderStack(final Folder folder,
                                      final long folderOffset,
                                      final int firstPackStreamIndex,
...
throws IOException {
	...
	InputStream inputStreamStack = new FilterInputStream(
		new BufferedInputStream(new BoundedFileChannelInputStream(
						channel, archive.packSizes[firstPackStreamIndex]))) {
		@Override
	...
	for (final Coder coder : folder.getOrderedCoders()) {
		...
		inputStreamStack = Coders.addDecoder(fileName, inputStreamStack,
		                                     ...
		                                     coder, password);
		methods.addFirst(new SevenZMethodConfiguration(
					 method, Coders.findByMethod(method).getOptionsFromCoder(
						 coder, inputStreamStack)));
	}
...
private static long skipBytesFully(final ByteBuffer input,
