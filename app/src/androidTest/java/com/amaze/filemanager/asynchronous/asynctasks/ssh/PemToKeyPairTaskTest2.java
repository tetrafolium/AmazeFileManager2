...
public class PemToKeyPairTaskTest2 {
  ...
  // AAAAC3NzaC1lZDI1NTE5AAAAIGxJHFewxU9tJn9hUq9e2C/+ELFw83NpmJ5NLFOzU7O3
  // test-openssh-key
  ...
  // Passphrase = 12345678
  // public key for authorized_keys: ssh-ed25519
  // AAAAC3NzaC1lZDI1NTE5AAAAIHio1/33U0XoewL1qGLmTzxyVNeYP5b0Tunv/SQrQi92
  // test-openssh-key
  ...
  public void testUnencryptedKeyToKeyPair() throws InterruptedException {
    ...
    PemToKeyPairTask task =
        new PemToKeyPairTask(unencryptedOpenSshKey, result -> {
  ...
  public void testEncryptedKeyToKeyPair()
    ...
    CountDownLatch waiter = new CountDownLatch(1);
    PemToKeyPairTask task =
        new PemToKeyPairTask(encryptedOpenSshKey, result -> {
          assertNotNull(result);
          assertNotNull(result.getPublic());
          assertNotNull(result.getPrivate());
          waiter.countDown();
