...
private File tarballfile = new File(Environment.getExternalStorageDirectory(),
                                    "test-archive.tar.gz");
private File tarLzmafile = new File(Environment.getExternalStorageDirectory(),
                                    "test-archive.tar.lzma");
private File tarXzfile = new File(Environment.getExternalStorageDirectory(),
                                  "test-archive.tar.xz");
private File tarBz2file = new File(Environment.getExternalStorageDirectory(),
                                   "test-archive.tar.bz2");
private File sevenZipfile =
	new File(Environment.getExternalStorageDirectory(), "test-archive.7z");
private File passwordProtectedZipfile = new File(
	Environment.getExternalStorageDirectory(), "test-archive-encrypted.zip");
private File passwordProtected7Zipfile = new File(
	Environment.getExternalStorageDirectory(), "test-archive-encrypted.7z");
...
private void performTest(final @NonNull File archiveFile) {
	Intent intent =
		...
		.putExtra(ExtractService.KEY_PATH_EXTRACT,
		          new File(Environment.getExternalStorageDirectory(),
		                   "test-archive")
		          .getAbsolutePath());
