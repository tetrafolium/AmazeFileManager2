...
public class ListFilesOnSshdTest extends AbstractSftpServerTest {
  ...
  public void testNormalListDirs() throws InterruptedException {
    for (String s :
         new String[] {"sysroot", "srv", "var", "tmp", "bin", "lib", "usr"}) {
  ...
  public void testListDirsAndSymlinks() throws Exception {
    ...
    for (String s : new String[] {"srv", "var", "tmp"}) {
      ...
      Files.createSymbolicLink(
          Paths.get(new File(Environment.getExternalStorageDirectory(), s)
                        .getAbsolutePath()),
  ...
  private void performVerify() throws InterruptedException {
    ...
    HybridFile file = new HybridFile(
        OpenMode.SFTP, "ssh://testuser:testpassword@127.0.0.1:22222");
    CountDownLatch waiter = new CountDownLatch(7);
    file.forEachChildrenFile(
        RuntimeEnvironment.application, false, (fileFound) -> {
          assertTrue(fileFound.getPath() + " not seen as directory",
    ...
    assertThat(result,
               hasItems("sysroot", "srv", "var", "tmp", "bin", "lib", "usr"));
  }
  ...
  public void testListDirsAndFilesAndSymlinks() throws Exception {
    ...
    for (int i = 1; i <= 4; i++) {
      ...
      out.close();
      Files.createSymbolicLink(
          Paths.get(new File(Environment.getExternalStorageDirectory(),
                             "symlink" + i + ".txt")
                        .getAbsolutePath()),
    ...
    file.forEachChildrenFile(
        RuntimeEnvironment.application, false, (fileFound) -> {
          if (!fileFound.getName().endsWith(".txt")) {
            ...
            assertFalse(fileFound.getPath() + " not seen as file",
                        fileFound.isDirectory());
    ...
    assertThat(dirs,
               hasItems("sysroot", "srv", "var", "tmp", "bin", "lib", "usr"));
    assertThat(files,
               hasItems("1.txt", "2.txt", "3.txt", "4.txt", "symlink1.txt",
                        "symlink2.txt", "symlink3.txt", "symlink4.txt"));
  }
}
