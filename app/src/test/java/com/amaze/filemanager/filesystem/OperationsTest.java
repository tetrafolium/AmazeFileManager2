...
@Config(maxSdk = 27, constants = BuildConfig.class,
...
public class OperationsTest {
  ...
  public void testMkdir() throws InterruptedException {
    ...
    HybridFile newFolderHF =
        new HybridFile(OpenMode.FILE, newFolder.getAbsolutePath());

    CountDownLatch waiter = new CountDownLatch(1);
    Operations.mkdir(newFolderHF, RuntimeEnvironment.application, false,
  ...
  @Test
  public void testMkdirNewFolderSameNameAsCurrentFolder()
    ...
    HybridFile newFolder2HF =
        new HybridFile(OpenMode.FILE, newFolder2.getAbsolutePath());
    CountDownLatch waiter2 = new CountDownLatch(1);
    Operations.mkdir(newFolder2HF, RuntimeEnvironment.application, false,
                     new AbstractErrorCallback() {
                       @Override
                       public void done(final HybridFile hFile,
                                        final boolean b) {
  ...
  public void testRename() throws InterruptedException {
    ...
    HybridFile oldFolderHF =
        new HybridFile(OpenMode.FILE, oldFolder.getAbsolutePath());
    File newFolder = new File(storageRoot, "test2");
    HybridFile newFolderHF =
        new HybridFile(OpenMode.FILE, newFolder.getAbsolutePath());

    CountDownLatch waiter1 = new CountDownLatch(1);
    Operations.mkdir(oldFolderHF, RuntimeEnvironment.application, false,
                     new AbstractErrorCallback() {
                       @Override
                       public void done(final HybridFile hFile,
                                        final boolean b) {
    ...
    CountDownLatch waiter2 = new CountDownLatch(1);
    Operations.rename(
        oldFolderHF, newFolderHF, false, RuntimeEnvironment.application,
        new AbstractErrorCallback() {
          @Override
  ...
  public void testRenameSameName() throws InterruptedException {
    ...
    HybridFile folderHF =
        new HybridFile(OpenMode.FILE, folder.getAbsolutePath());

    CountDownLatch waiter1 = new CountDownLatch(1);
    Operations.mkdir(folderHF, RuntimeEnvironment.application, false,
                     new AbstractErrorCallback() {
                       @Override
                       public void done(final HybridFile hFile,
                                        final boolean b) {
    ...
    CountDownLatch waiter2 = new CountDownLatch(1);
    AtomicBoolean assertFlag = new AtomicBoolean(false);
    Operations.rename(folderHF, folderHF, false, RuntimeEnvironment.application,
                      new AbstractErrorCallback() {
                        @Override
                        public void exists(final HybridFile file) {
                          assertFlag.set(true);
  ...
  public void testRenameSameName2() throws InterruptedException {
    ...
    HybridFile folder2HF =
        new HybridFile(OpenMode.FILE, folder2.getAbsolutePath());

    CountDownLatch waiter2 = new CountDownLatch(1);
    Operations.mkdir(folder2HF, RuntimeEnvironment.application, false,
                     new AbstractErrorCallback() {
                       @Override
                       public void done(final HybridFile hFile,
                                        final boolean b) {
    ...
    CountDownLatch waiter3 = new CountDownLatch(1);
    AtomicBoolean assertFlag = new AtomicBoolean(false);
    Operations.rename(folderHF, folder2HF, false,
  ...
  public void testRenameSameName3() throws InterruptedException {
    ...
    HybridFile folder3HF =
        new HybridFile(OpenMode.FILE, folder3.getAbsolutePath());

    CountDownLatch waiter3 = new CountDownLatch(1);
    AtomicBoolean assertFlag = new AtomicBoolean(false);
    Operations.rename(
        folder2HF, folder3HF, false, RuntimeEnvironment.application,
        new AbstractErrorCallback() {
          @Override
  ...
  private abstract class AbstractErrorCallback
      implements Operations.ErrorCallBack {
    @Override
    public void exists(final HybridFile file) {}
