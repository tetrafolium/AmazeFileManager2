...
public class RootHelperTest {
  ...
  private static final List<String> expected = Arrays.asList(
      "srv", "var", "tmp", "bin", "lib", "usr", "1.txt", "2.txt", "3.txt",
      "4.txt", "symlink1.txt", "symlink2.txt", "symlink3.txt", "symlink4.txt");
  ...
  public void testRoot()
      throws InterruptedException, SecurityException, IllegalArgumentException {
    MainActivity.shellInteractive =
        new Shell.Builder().setShell("/bin/false").open();
    runVerify(true);
  }

  private void runVerify(final boolean root) throws InterruptedException {
    List<String> result = new ArrayList<>();
    CountDownLatch waiter = new CountDownLatch(expected.size());
    RootHelper.getFiles(
        Environment.getExternalStorageDirectory().getAbsolutePath(), root, true,
        mode -> {}, file -> {
