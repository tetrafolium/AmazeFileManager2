 ...
 * http://iki.fi/elonen/) and Copyright &copy; 2010 Konstantinos Togias
 * (info@ktogias.gr, http://ktogias.gr)
 *
 ...
 * yourself if you need them.) </li> <li> Supports parameter parsing of GET and
 * POST methods </li> <li> Supports both dynamic content and file serving </li>
 *    <li> Supports file upload (since version 1.2, 2010) </li>
 *    <li> Supports partial content (streaming)</li>
 *    <li> Supports ETags</li>
 *    <li> Never caches anything </li>
 *    <li> Doesn't limit bandwidth, request time or simultaneous connections
 * </li> <li> Default code serves files and shows all HTTP parameters and
 * headers</li> <li> File server supports directory listing, index.html and
 * index.htm</li> <li> File server supports partial content (streaming)</li>
 *    <li> File server supports ETags</li>
 *    <li> File server does the 301 redirection trick for directories without
 * '/'</li> <li> File server supports simple skipping for files (continue
 * download) </li> <li> File server serves also very long files without memory
 * overhead </li> <li> Contains a built-in list of most common mime types </li>
...
public abstract class CloudStreamServer {
  ...
  public class Response {
     ...
     */
    public Response() { this.status = HTTP_OK; }

    /**
     * Basic constructor.
     */
    public Response(final String status, final String mimeType,
                    final CloudStreamSource data) {
      this.status = status;
  ...
  public CloudStreamServer(final int port, final File wwwroot)
      throws IOException {
    myTcpPort = port;
  ...
  /**
   * Since CloudStreamServer and Streamer both uses the same port, shutdown the
   * Streamer before acquiring the port.
   *
   * @return ServerSocket
   */
  ...
  //      {
  //              System.out.println( "NanoHTTPD 1.24 (C) 2001,2005-2011 Jarno
  //              Elonen and (C) 2010 Konstantinos Togias\n" +
  //                                                      "(Command line
  //                                                      options: [-p port] [-d
  //                                                      root-dir]
  //                                                      [--licence])\n" );
  ...
  //              from \"" + wwwroot + "\"" ); System.out.println( "Hit Enter to
  //              stop.\n" );
  //
  ...
  private class HTTPSession implements Runnable {
    ...
    private void handleResponse(final Socket socket) {
      try {
        ...
        BufferedReader hin =
            new BufferedReader(new InputStreamReader(hbis, "utf-8"));
        ...
        if (r == null)
          sendError(socket, HTTP_INTERNALERROR,
     ...
     **/
    private void decodeHeader(final BufferedReader in, final Properties pre,
                              final Properties parms, final Properties header)
      ...
      try {
        ...
        if (!st.hasMoreTokens())
          sendError(socket, HTTP_BADREQUEST,
     ...
     **/
    private void decodeMultipartData(final String boundary, final byte[] fbuf,
          ...
          if (mpline != null) {
            ...
            if (item.getProperty("content-type") == null) {
              ...
              String path = saveTmpFile(
                  fbuf, offset, bpositions[boundarycount - 1] - offset - 4);
     ...
     * identical keys due to the simplicity of Properties -- if you need
     * multiples, you might want to replace the Properties with a Hashtable of
     * Vectors or such.
     */
     ...
     */
    private void sendError(final Socket socket, final String status,
                           final String msg) throws InterruptedException {
     ...
     */
    private void sendResponse(final Socket socket, final String status,
                              final String mime, final Properties header,
                              final CloudStreamSource data) {
      try {
        if (status == null)
        ...
        if (data != null) {
          // long pending = data.availableExact();      // This is to support
          // partial sends, see serveFile()
          data.open();
          byte[] buff = new byte[8192];
          int read = 0;
          while ((read = data.read(buff)) > 0) {
            // if(SolidExplorer.LOG)Log.d(CloudUtil.TAG, "Read: "+ read +",
            // pending: "+ data.availableExact());
  ...
  //                      MIME_PLAINTEXT,
  //                              "INTERNAL ERRROR: serveFile(): given homeDir
  //                              is not a directory." );
  //
  //              if ( res == null )
  //              {
  ...
  //                              readable else if ( allowDirectoryListing &&
  //                              f.canRead() )
  //                              {
  //                                      String[] files = f.list();
  //                                      String msg =
  ...
  //                                      if ( uri.length() > 1 )
  //                                      {
  //                                              String u = uri.substring( 0,
  //                                              uri.length()-1 ); int slash =
  //                                              u.lastIndexOf( '/' ); if (
  //                                              slash >= 0 && slash  <
  //                                              u.length())
  //                                                      msg += "<b><a href=\""
  ...
  //                              possible String mime = null; int dot =
  //                              f.getCanonicalPath().lastIndexOf( '.' ); if (
  //                              dot >= 0 )
  //                                      mime = (String)theMimeTypes.get(
  //                                      f.getCanonicalPath().substring( dot +
  //                                      1 ).toLowerCase());
  //                              if ( mime == null )
  ...
  //                              // Calculate etag
  //                              String etag =
  //                              Integer.toHexString((f.getAbsolutePath() +
  //                              f.lastModified() + "" +
  //                              f.length()).hashCode());
  //
  ...
  //                              header when skipping is requested long fileLen
  //                              = f.length(); if (range != null && startFrom
  //                              >= 0)
  //                              {
  ...
  //                                      FileInputStream( f )); res.addHeader(
  ...
  //              }
  //
  //              res.addHeader( "Accept-Ranges", "bytes"); // Announce that the
  //              file server accepts partial content requestes return res;
  //      }
