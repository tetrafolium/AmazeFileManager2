...
public class OTGUtil {
  ...
  getDocumentFilesList(final String path, final Context context) {
   ...
   * @param context context for loading
   */
  public static void getDocumentFiles(final String path, final Context context,
                                      final OnFileFound fileFound) {
    ...
    for (DocumentFile file : rootUri.listFiles()) {
      if (file.exists()) {
        ...
        HybridFileParcelable baseFile = new HybridFileParcelable(
            path + "/" + file.getName(),
            RootHelper.parseDocumentFilePermission(file), file.lastModified(),
   ...
   *
   * @param createRecursive flag used to determine whether to create new file
   *     while traversing to path,
   *                        in case path is not present. Notably useful in
   * opening an output stream.
   */
  public static DocumentFile getDocumentFile(final String path,
                                             final Context context,
                                             final boolean createRecursive) {
    Uri rootUriString = SingletonUsbOtg.getInstance().getUsbOtgRoot();
    if (rootUriString == null)
      throw new NullPointerException("USB OTG root not set!");
    ...
    for (String part : parts) {
      ...
      if (createRecursive && (nextDocument == null || !nextDocument.exists())) {
        nextDocument =
            rootUri.createFile(part.substring(part.lastIndexOf(".")), part);
      }
  ...
  /**
   * Checks if there is at least one USB device connected with class MASS
   * STORAGE.
   */
  @NonNull
  public static List<UsbOtgRepresentation>
  getMassStorageDevicesConnected(@NonNull final Context context) {
    ...
    for (String deviceName : devices.keySet()) {
      ...
      for (int i = 0; i < device.getInterfaceCount(); i++) {
        if (device.getInterface(i).getInterfaceClass() ==
          ...
          UsbOtgRepresentation usb = new UsbOtgRepresentation(
              device.getProductId(), device.getVendorId(), serial);
