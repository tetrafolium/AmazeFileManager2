 ...
 *
 * Copyright © 2017-2018 Vishal Nehra <vishalmeham2@gmail.com>, Emmanuel
 ...
 *
 * Class provide helper methods to encrypt/decrypt various type of files, or
 * passwords We take the password from user before encrypting file. First, the
 * password is encrypted against the key created in keystore in android {@see
 * #encryptPassword(String)}. We're using AES encryption with GCM as the
 * processor algorithm. The encrypted password is mapped against the file path
 * to be encrypted in database for later use. This is handled by the service
 * invoking this instance. The service then calls the constructor which fires up
 * the subsequent encryption/decryption process.
 *
 * We differentiate between already encrypted files from <i>new ones</i> by
 * encrypting the plaintext
 * {@link PrefFrag#ENCRYPT_PASSWORD_MASTER}
 * and {@link PrefFrag#ENCRYPT_PASSWORD_FINGERPRINT}
 * against the path in database. At the time of decryption, we check for these
 * values and either retrieve master password from preferences or fire up the
 * fingerprint sensor authentication.
 *
 * From <i>new ones</i> we mean the ones when were encrypted after user changed
 * preference for master password/fingerprint sensor from settings.
 *
 * We use buffered streams to process files, usage of NIO will probably mildly
 * effect the performance.
 *
 * Be sure to use constructors to encrypt/decrypt files only, and to call
 * service through
...
public class CryptUtil {
  ...
  // TODO: Generate a random IV every time, and keep track of it (in database
  ...
  /**
   * Constructor will start encryption process serially. Make sure to call with
   * background thread. The result file of encryption will be in the same
   * directory with a {@link #CRYPT_EXTENSION} extension
   *
   * Make sure you're done with encrypting password for this file and map it
   * with this file in database
   *
   * Be sure to use constructors to encrypt/decrypt files only, and to call
   * service through
   * {@link ServiceWatcherUtil} and to initialize watchers beforehand
   ...
   */
  public CryptUtil(final Context context, final HybridFileParcelable sourceFile,
                   final ProgressHandler progressHandler,
                   final ArrayList<HybridFile> failedOps,
    ...
    HybridFile hFile =
        new HybridFile(sourceFile.getMode(), sourceFile.getParent(context));
    encrypt(context, sourceFile, hFile, targetFilename);
  }

  /**
   * Decrypt the file in specified path. Can be used to open the file (decrypt
   * in cache) or simply decrypt the file in the same (or in a custom
   * preference) directory Make sure to decrypt and check user provided
   * passwords beforehand from database
   *
   * Be sure to use constructors to encrypt/decrypt files only, and to call
   * service through
   * {@link ServiceWatcherUtil} and to initialize watchers beforehand
   ...
   */
  public CryptUtil(final Context context, final HybridFileParcelable baseFile,
   ...
   * @param targetDirectory   the target directory inside which we're going to
   *     decrypt
   */
  private void decrypt(final Context context,
                       final HybridFileParcelable sourceFile,
                       final HybridFile targetDirectory)
      throws GeneralSecurityException, IOException {
    ...
    if (sourceFile.isDirectory()) {
      ...
      final HybridFile hFile =
          new HybridFile(targetDirectory.getMode(), targetDirectory.getPath(),
                         sourceFile.getName().replace(CRYPT_EXTENSION, ""),
      ...
      sourceFile.forEachChildrenFile(context, sourceFile.isRoot(), file -> {
        try {
          ...
          throw new IllegalStateException(
              e); // throw unchecked exception, no throws needed
        }
      });
    } else {
      ...
      BufferedInputStream inputStream =
          new BufferedInputStream(sourceFile.getInputStream(context),
      ...
      BufferedOutputStream outputStream =
          new BufferedOutputStream(targetFile.getOutputStream(context),
                                   GenericCopyUtil.DEFAULT_BUFFER_SIZE);
   ...
   * @param targetDirectory   the target directory in which we're going to
   *     encrypt
   */
  private void
  encrypt(final Context context, final HybridFileParcelable sourceFile,
          final HybridFile targetDirectory, final String targetFilename)
      throws GeneralSecurityException, IOException {
    ...
    if (sourceFile.isDirectory()) {
      ...
      final HybridFile hFile =
          new HybridFile(targetDirectory.getMode(), targetDirectory.getPath(),
                         targetFilename, sourceFile.isDirectory());
      FileUtil.mkdirs(context, hFile);
   ...
   */
  @RequiresApi(api = Build.VERSION_CODES.M)
  private void aesEncrypt(final BufferedInputStream inputStream,
   ...
   */
  @RequiresApi(api = Build.VERSION_CODES.M)
  private void aesDecrypt(final BufferedInputStream inputStream,
                          final BufferedOutputStream outputStream)
      throws GeneralSecurityException, IOException {
  ...
  private static Key getSecretKey()
    ...
    if (!keyStore.containsAlias(KEY_ALIAS_AMAZE)) {
      KeyGenerator keyGenerator = KeyGenerator.getInstance(
          KeyProperties.KEY_ALGORITHM_AES, KEY_STORE_ANDROID);
  ...
  private void rsaEncrypt(final Context context,
  ...
  @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN_MR2)
  private void rsaDecrypt(final Context context,
                          final BufferedInputStream inputStream,
                          final BufferedOutputStream outputStream)
      throws GeneralSecurityException, IOException {
  ...
  @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN_MR2)
  private static String rsaEncryptPassword(final Context context,
  ...
  @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN_MR2)
  private static String rsaDecryptPassword(final Context context,
   ...
   */
  public static Cipher initCipher(final Context context)
      throws GeneralSecurityException, IOException {
    Cipher cipher = null;
  ...
  static class RSAKeygen {
     ...
     */
    @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN_MR2)
    private void generateKeyPair(final Context context)
        throws GeneralSecurityException, IOException {
      ...
      if (!keyStore.containsAlias(KEY_ALIAS_AMAZE)) {
        ...
        KeyPairGenerator keyPairGenerator =
            KeyPairGenerator.getInstance("RSA", KEY_STORE_ANDROID);
     ...
     */
    private byte[] encryptAESKey(final byte[] secretKey)
        throws GeneralSecurityException, IOException {
      ...
      cipher.init(Cipher.ENCRYPT_MODE,
                  keyEntry.getCertificate().getPublicKey());
    ...
    /**
     * Decodes encrypted AES key from preference and decrypts using RSA private
     * key
     */
    @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN_MR2)
     ...
     */
    private byte[] decryptAESKey(final byte[] encodedBytes)
        throws GeneralSecurityException, IOException {
