 ...
 *
 * Helper class providing helper methods to manage Service startup and it's
 * progress Be advised - this class can only handle progress with one object at
 * a time. Hence, class also provides convenience methods to serialize the
 * service startup.
 */
...
public class ServiceWatcherUtil {
   ...
   * This variable should only be updated from an {@link
  ...
  private static ConcurrentLinkedQueue<Intent> pendingIntents =
  ...
  /**
   * Watches over the service progress without interrupting the worker thread in
   * respective services Method frees up all the resources and handlers after
   * operation completes.
   */
  public void
  watch(final ServiceWatcherInteractionInterface interactionInterface) {
    runnable = new Runnable() {
      ...
      public void run() {
        ...
        if (position == progressHandler.getWrittenSize() &&
          ...
          // new position is same as the last second position, and halt counter
          // is past threshold

          String writtenSize = Formatter.formatShortFileSize(
              interactionInterface.getApplicationContext(),
              progressHandler.getWrittenSize());
          String totalSize = Formatter.formatShortFileSize(
              interactionInterface.getApplicationContext(),
              progressHandler.getTotalSize());

          if (interactionInterface.isDecryptService() &&
              writtenSize.equals(totalSize)) {
            // workaround for decryption when we have a length retrieved by
            // CipherInputStream less than the original stream, and hence the
            // total size we passed at the beginning is never reached we try to
            // get a less precise size and make our decision based on that
        ...
        if (position == progressHandler.getTotalSize() ||
  ...
  /**
   * Manually call runnable, before the delay. Fixes race condition which can
   * arise when service has finished execution and stopping self, but the
   * runnable is yet scheduled to be posted. Thus avoids posting any callback
  ...
  /**
   * Convenience method to check whether another service is working in
   * background If a service is found working (by checking {@link
   * #handlerThread} for it's state) then we wait for an interval of 5 secs,
   * before checking on it again.
   *
   * Be advised - this method is not sure to start a new service, especially
   * when app has been closed as there are higher chances for android system to
   * GC the thread when it is running low on memory
   */
  public static synchronized void runService(final Context context,
                                             final Intent intent) {
    ...
    case 2:
      ...
      pendingIntents.add(intent);
      notificationManager.notify(NotificationConstants.WAIT_ID,
                                 builder.build());
      break;
    default:
      pendingIntents.add(intent);
  ...
  private static synchronized void postWaiting(final Context context) {
    ...
    builder = new NotificationCompat
                  .Builder(context, NotificationConstants.CHANNEL_NORMAL_ID)
      ...
      public void run() {
        if (handlerThread == null || !handlerThread.isAlive()) {
          ...
          if (pendingIntents.size() == 0) {
            // we've done all the work, free up resources (if not already killed
            // by system)
        ...
        Log.d(getClass().getSimpleName(),
              "Processes in progress, delay the check");
