...
public ShareTask(final Activity context, final ArrayList<Uri> arrayList,
...
protected Void doInBackground(final String... strings) {
	...
	if (!resInfos.isEmpty()) {
		for (ResolveInfo resInfo : resInfos) {
			...
			System.out.println(resInfo.activityInfo.packageName + "\t" +
	...
	if (!bluetooth_present &&
	    appInstalledOrNot("com.android.bluetooth", packageManager)) {
		Intent intent = new Intent();
		...
		arrayList2.add(contextc.getResources().getDrawable(
...
public void onPostExecute(final Void v) {
	if (!targetShareIntents.isEmpty()) {
		...
		ShareAdapter shareAdapter = new ShareAdapter(contextc, targetShareIntents,
