...
/**
 * Created by root on 11/22/15, modified by Emmanuel
 * Messulam<emmanuelbendavid@gmail.com>
 */
public class MainActivityHelper {

  public static final int NEW_FOLDER = 0, NEW_FILE = 1, NEW_SMB = 2,
                          NEW_CLOUD = 3;
  ...
  /*
   * A static string which saves the last searched query. Used to retain search
   * task after user presses back button from pressing on any list item of
   * search results
   */
  ...
  public void
  showFailedOperationDialog(final ArrayList<HybridFileParcelable> failedOps,
        ...
        public void onReceive(final Context context, final Intent intent) {
          if (intent != null) {
            if (intent.getAction().equals(Intent.ACTION_MEDIA_MOUNTED)) {
              Toast.makeText(mainActivity, "Media Mounted", Toast.LENGTH_SHORT)
                  .show();
              String a = intent.getData().getPath();
              if (a != null && a.trim().length() != 0 && new File(a).exists() &&
   ...
   */
  void mkdir(final OpenMode openMode, final String path,
             final MainFragment ma) {
    mk(R.string.newfolder, "",
       (dialog, which)
           -> {
         EditText textfield =
             dialog.getCustomView().findViewById(R.id.singleedittext_input);
         mkDir(new HybridFile(openMode,
   ...
   * @param ma       {@link MainFragment} current fragment
   */
  void mkfile(final OpenMode openMode, final String path,
              final MainFragment ma) {
    mk(R.string.newfile, NEW_FILE_TXT_EXTENSION,
       (dialog, which)
           -> {
         EditText textfield =
             dialog.getCustomView().findViewById(R.id.singleedittext_input);
         mkFile(new HybridFile(openMode,
                               path + "/" + textfield.getText().toString()),
                ma);
         dialog.dismiss();
       },
       (text) -> {
         ...
         // The redundant equalsIgnoreCase() is needed since ".txt" itself does
         // not end with .txt (i.e. recommended as ".txt.txt"
  ...
  private void mk(final @StringRes int newText, final String prefill,
    ...
    GeneralDialogCreation
        .showNameDialog(
            mainActivity,
            mainActivity.getResources().getString(R.string.entername), prefill,
            mainActivity.getResources().getString(newText),
            mainActivity.getResources().getString(R.string.create),
            mainActivity.getResources().getString(R.string.cancel), null,
            onPositiveAction, validator)
        .show();
  }

  public void add(final int pos) {
    final MainFragment ma =
        (MainFragment)((TabFragment)mainActivity.getSupportFragmentManager()
  ...
  public void guideDialogForLEXA(final String path) {
    ...
    LayoutInflater layoutInflater =
        (LayoutInflater)mainActivity.getSystemService(
            Context.LAYOUT_INFLATER_SERVICE);
    View view = layoutInflater.inflate(R.layout.lexadrawer, null);
    x.customView(view, true);
    // textView
    TextView textView = view.findViewById(R.id.description);
    textView.setText(mainActivity.getString(R.string.needsaccesssummary) +
                     path +
                     mainActivity.getString(R.string.needsaccesssummary1));
    ...
    x.positiveText(R.string.open)
        ...
        .onNegative((dialog, which)
                        -> Toast
                               .makeText(mainActivity, R.string.error,
                                         Toast.LENGTH_SHORT)
                               .show());
  ...
  public void rename(final OpenMode mode, final String oldPath,
                     final String newPath, final Activity context,
                     final boolean rootmode) {
    final Toast toast = Toast.makeText(
        context, context.getString(R.string.renaming), Toast.LENGTH_SHORT);
    toast.show();
    Operations.rename(
        new HybridFile(mode, oldPath), new HybridFile(mode, newPath), rootmode,
        context, new Operations.ErrorCallBack() {
          @Override
          public void exists(final HybridFile file) {
            context.runOnUiThread(() -> {
              if (toast != null)
                toast.cancel();
              Toast
                  .makeText(mainActivity, context.getString(R.string.fileexist),
                            Toast.LENGTH_SHORT)
                  .show();
            });
          }
            ...
            context.runOnUiThread(() -> {
              if (b) {
                ...
                intent.putExtra(MainActivity.KEY_INTENT_LOAD_LIST_FILE,
                                hFile.getParent(context));
                mainActivity.sendBroadcast(intent);

                // update the database entry to reflect rename for encrypted
                // file
                ...
                Toast
                    .makeText(context,
                              context.getString(R.string.operationunsuccesful),
                              Toast.LENGTH_SHORT)
                    .show();
            });
          }
   ...
   */
  public void compressFiles(final File file,
  ...
  public void mkFile(final HybridFile path, final MainFragment ma) {
    final Toast toast =
        Toast.makeText(ma.getActivity(), ma.getString(R.string.creatingfile),
                       Toast.LENGTH_SHORT);
    toast.show();
    Operations.mkfile(
        path, ma.getActivity(), mainActivity.isRootExplorer(),
          ...
          public void done(final HybridFile hFile, final boolean b) {
            ma.getActivity().runOnUiThread(() -> {
              if (b) {
                ...
                Toast
                    .makeText(ma.getActivity(),
                              ma.getString(R.string.operationunsuccesful),
                              Toast.LENGTH_SHORT)
                    .show();
              }
            });
  ...
  public void mkDir(final HybridFile path, final MainFragment ma) {
    final Toast toast =
        Toast.makeText(ma.getActivity(), ma.getString(R.string.creatingfolder),
                       Toast.LENGTH_SHORT);
    toast.show();
    Operations.mkdir(
        path, ma.getActivity(), mainActivity.isRootExplorer(),
  ...
  public void deleteFiles(final ArrayList<HybridFileParcelable> files) {
    ...
    int mode = checkFolder(new File(files.get(0).getPath()).getParentFile(),
                           mainActivity);
    if (mode == 2) {
  ...
  public void extractFile(final File file) {
    ...
    if (mode == 2) {
      ...
      Decompressor decompressor =
          CompressedHelper.getCompressorInstance(mainActivity, file);
  ...
  public String parseCloudPath(final OpenMode serviceType, final String path) {
    switch (serviceType) {
    case DROPBOX:
      if (path.contains(CloudHandler.CLOUD_PREFIX_DROPBOX))
        return path;
      else
        return CloudHandler.CLOUD_PREFIX_DROPBOX +
            path.substring(path.indexOf(":") + 1, path.length());
    case BOX:
      if (path.contains(CloudHandler.CLOUD_PREFIX_BOX))
        return path;
      else
        return CloudHandler.CLOUD_PREFIX_BOX +
            path.substring(path.indexOf(":") + 1, path.length());
    case GDRIVE:
      if (path.contains(CloudHandler.CLOUD_PREFIX_GOOGLE_DRIVE))
        return path;
      else
        return CloudHandler.CLOUD_PREFIX_GOOGLE_DRIVE +
            path.substring(path.indexOf(":") + 1, path.length());
    case ONEDRIVE:
      if (path.contains(CloudHandler.CLOUD_PREFIX_ONE_DRIVE))
        return path;
      else
  ...
  /**
   * Creates a fragment which will handle the search AsyncTask {@link
   * SearchWorkerFragment}
   *
  ...
  public void search(final SharedPreferences sharedPrefs, final String query) {
    ...
    SearchWorkerFragment fragment = (SearchWorkerFragment)fm.findFragmentByTag(
    ...
    addSearchFragment(
        fm, new SearchWorkerFragment(), fpath, query, ma.openMode,
  ...
  public static void addSearchFragment(
      ...
      final String path, final String input, final OpenMode openMode,
      final boolean rootMode, final boolean regex, final boolean matches) {
    ...
    fragmentManager.beginTransaction()
        .add(fragment, MainActivity.TAG_ASYNC_HELPER)
        .commit();
  }

  /**
   * Check whether creation of new directory is inside the same directory with
   * the same name or not Directory inside the same directory with similar
   * filename shall not be allowed Doesn't work at an OTG path
   */
