...
public class EncryptDecryptUtils {
   ...
   */
  public static void startEncryption(final Context c, final String path,
    ...
    // EncryptService.TAG_ENCRYPT_TARGET already has the .aze extension, no need
    // to append again
  ...
  public static void
  decryptFile(final Context c, final MainActivity mainActivity,
              final MainFragment main, final OpenMode openMode,
    ...
    try {
      encryptedEntry =
          findEncryptedEntry(main.getContext(), sourceFile.getPath());
    ...
    DecryptButtonCallbackInterface decryptButtonCallbackInterface =
        new DecryptButtonCallbackInterface() {
          ...
          public void failed() {
            Toast
                .makeText(main.getContext(),
                          main.getActivity().getString(
                              R.string.crypt_decryption_fail_password),
                          Toast.LENGTH_LONG)
                .show();
          }
        };
      ...
      try {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
          GeneralDialogCreation.showDecryptFingerprintDialog(
              c, mainActivity, decryptIntent, utilsProvider.getAppTheme(),
    ...
    default:
      GeneralDialogCreation.showDecryptDialog(
          c, mainActivity, decryptIntent, utilsProvider.getAppTheme(),
          encryptedEntry.getPassword(), decryptButtonCallbackInterface);
      break;
    }
  }
  ...
  public interface EncryptButtonCallbackInterface {
    ...
    /**
     * Callback fired when we've just gone through warning dialog before
     * encryption
     */
     ...
     * Not called when we've a master password set or enable fingerprint
     * authentication
     *
