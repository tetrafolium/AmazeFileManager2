...
public void setStreamSrc(final InputStream inputStream, final String fileName,
                         final long length) {
...
@Override
public CloudStreamServer.Response
serve(final String uri, final String method, final Properties header,
	...
	if (inputStream == null)
		...
		Log.d(CloudUtil.TAG,
		      "Request: " + range + " from: " + startFrom + ", to: " + endAt);

		// Change return code and add Content-Range header when skipping
		// is requested
		// source.open();
		final CloudStreamSource source =
			new CloudStreamSource(fileName, length, inputStream);
		long fileLen = source.length();
		if (range != null && startFrom > 0) {
			if (startFrom >= fileLen) {
				res = new CloudStreamServer.Response(HTTP_RANGE_NOT_SATISFIABLE,
				                                     MIME_PLAINTEXT, null);
				...
				if (newLen < 0)
					newLen = 0;
				Log.d(CloudUtil.TAG, "start=" + startFrom + ", endAt=" + endAt +
				      ", newLen=" + newLen);
				...
				res =
					new CloudStreamServer.Response(HTTP_PARTIALCONTENT, null, source);
