...
public class ProgressHandler {
   ...
   * Volatile because non volatile long r/w are not atomic (see Java Language
   * Specification 17.7)
   */
  ...
  /**
   * callback interface to interact with process viewer fragment and
   * notification
   */
   ...
   * @param sourceFiles the total number of source files selected by the user
   *     for operation
   */
  ...
  public int getSourceFilesProcessed() { return sourceFilesProcessed; }
  ...
  public void setTotalSize(final long totalSize) { this.totalSize = totalSize; }
  ...
  public synchronized float getPercentProgress() {
    if (totalSize == 0)
      return 0f; // Sometimes the total size is 0, because of metadata not being
                 // measured
