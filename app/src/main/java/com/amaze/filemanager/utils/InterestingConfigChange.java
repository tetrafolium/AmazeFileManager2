 ...
 *
 * Supposed to be used to determine recursive callbacks to
 * fragment/activity/loader Make sure to recycle after you're done
 */
 ...
 */
public static boolean isConfigChanged(final Resources resources) {
	int changedFieldsMask =
		lastConfiguration.updateFrom(resources.getConfiguration());
	boolean densityChanged =
		lastDensity != resources.getDisplayMetrics().densityDpi;
	int mode = ActivityInfo.CONFIG_SCREEN_LAYOUT | ActivityInfo.CONFIG_UI_MODE |
	           ActivityInfo.CONFIG_LOCALE;
	return densityChanged || (changedFieldsMask & mode) != 0;
}

/**
 * Recycle after usage, to avoid getting inconsistent result because of static
 * modifiers
 */
