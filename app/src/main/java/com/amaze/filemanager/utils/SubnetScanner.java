...
public class SubnetScanner extends Thread {
  ...
  class Task implements Callable<ComputerParcelable> {
    ...
    public Task(final String str) { this.addr = str; }
  ...
  public void run() {
    ...
    if (ipAddress != 0) {
      ...
      String substring =
          formatIpAddress.substring(0, formatIpAddress.lastIndexOf(46) + 1);
      if (!isInterrupted()) {
        for (ipAddress = 0; ipAddress < 100; ipAddress++) {
          this.tasks.add(this.pool.submit(new Task(substring + ipAddress)));
          this.tasks.add(
              this.pool.submit(new Task(substring + (ipAddress + 100))));
          if (ipAddress < 56) {
            this.tasks.add(
                this.pool.submit(new Task(substring + (ipAddress + 200))));
          }
        }
            ...
            if (!isInterrupted()) {
              try {
                ComputerParcelable computer =
                    (ComputerParcelable)((Future)this.tasks.get(i))
          ...
          try {
            ...
            for (SmbFile smbFile2 : listFiles) {
              ...
              for (SmbFile files : listFiles2) {
                try {
                  String substring = files.getName().substring(
                      0, files.getName().length() - 1);
                  UniAddress byName = UniAddress.getByName(substring);
                  if (byName != null) {
                    SubnetScanner.this.onFound(new ComputerParcelable(
                        substring, byName.getHostAddress()));
                  }
                } catch (Throwable e) {
                }
              }
  ...
  public void setObserver(final ScanObserver scanObserver) {
    synchronized (this.mLock) { this.observer = scanObserver; }
  }
