 ...
 * <vishalmeham2@gmail.com>, Marcin Zasuwa <marcinadd@gmail.com>
 *
...
public class FileUtils {
  ...
  public static long folderSize(final File directory,
                                final OnProgressUpdate<Long> updateState) {
    long length = 0;
    try {
      for (File file : directory.listFiles()) {
        if (file.isFile())
          ...
          length += folderSize(file, null); // null because updateState would be
                                            // called for children dirs
  ...
  public static long folderSize(final HybridFile directory,
                                final OnProgressUpdate<Long> updateState) {
   ...
   *
   * It is slow, it is stupid, and may be inaccurate (because of permission
   * problems). Only for fallback use when <code>du</code> is not available.
   *
   * @see HybridFile#folderSize(Context)
   * @return Folder size in bytes
   */
  public static Long folderSizeSftp(final SFTPClient client,
  ...
  public static long folderSizeCloud(final OpenMode openMode,
                                     final CloudMetaData sourceFileMeta) {

    DataUtils dataUtils = DataUtils.getInstance();
    long length = 0;
    CloudStorage cloudStorage = dataUtils.getAccount(openMode);
    for (CloudMetaData metaData : cloudStorage.getChildren(
             CloudUtil.stripPath(openMode, sourceFileMeta.getPath()))) {
  ...
  public static long otgFolderSize(final String path, final Context context) {
    ...
    OTGUtil.getDocumentFiles(
        path, context,
        file -> totalBytes.addAndGet(getBaseFileSize(file, context)));
   ...
   */
  public static long getTotalBytes(final ArrayList<HybridFileParcelable> files,
  ...
  private static long getBaseFileSize(final HybridFileParcelable baseFile,
                                      final Context context) {
   ...
   */
  public static void scanFile(final @NonNull File file,
                              final @NonNull Context c) {
    scanFile(Uri.fromFile(file), c);
  }

  /**
   * Triggers {@link Intent#ACTION_MEDIA_SCANNER_SCAN_FILE} intent to refresh
   * the media store.
   *
   * @param uri File's {@link Uri}
   * @param c {@link Context}
   */
  private static void scanFile(final @NonNull Uri uri,
                               final @NonNull Context c) {
   ...
   * Don't use filesystem API directly as files might not be present anymore
   * (eg. move/rename) which may lead to {@link java.io.FileNotFoundException}
   * @param hybridFiles
   * @param context
   */
  public static void scanFile(final @NonNull Context context,
                              final @NonNull HybridFile[] hybridFiles) {
    AsyncTask.execute(() -> {
   ...
   * @param context
   */
  public static void scanFile(final @NonNull HybridFile hybridFile,
                              final Context context) {

    if ((hybridFile.isLocal() || hybridFile.isOtgFile()) &&
        hybridFile.exists(context)) {

      DocumentFile documentFile =
          FileUtil.getDocumentFile(hybridFile.getFile(), false, context);
      // If FileUtil.getDocumentFile() returns null, fall back to
  ...
  public static void crossfade(final View buttons, final View pathbar) {
    ...
    pathbar.animate().alpha(0f).setDuration(100).setListener(
  ...
  public static void crossfadeInverse(final View buttons, final View pathbar) {
    ...
    buttons.animate().alpha(0f).setDuration(500).setListener(
        new AnimatorListenerAdapter() {
          @Override
          public void onAnimationEnd(final Animator animation) {
  ...
  public static void shareCloudFile(final String path, final OpenMode openMode,
                                    final Context context) {
    new AsyncTask<String, Void, String>() {
      ...
      protected String doInBackground(final String... params) {
        ...
        CloudStorage cloudStorage =
            DataUtils.getInstance().getAccount(openMode);
        return cloudStorage.createShareLink(
            CloudUtil.stripPath(openMode, shareFilePath));
      }
      ...
      protected void onPostExecute(final String s) {
        ...
        Toast
            .makeText(context, context.getString(R.string.cloud_share_copied),
                      Toast.LENGTH_LONG)
            .show();
      }
    }.execute(path);
  ...
  public static void shareFiles(final ArrayList<File> a, final Activity c,
                                final AppTheme appTheme, final int fab_skin) {
    ...
    String mime =
        MimeTypes.getMimeType(a.get(0).getPath(), a.get(0).isDirectory());
    if (a.size() > 1)
      for (File f : a) {
   ...
   */
  public static void
  installApk(final @NonNull File f,
             final @NonNull PermissionsActivity permissionsActivity) {
    ...
    try {
      ...
      e.printStackTrace();
      Toast.makeText(permissionsActivity, R.string.error, Toast.LENGTH_SHORT)
          .show();
    }
  }
   ...
   *
   * @param f the file
   * @param forcechooser force the chooser to show up even when set default by
   *     user
   */
  public static void openunknown(final File f, final Context c,
                                 final boolean forcechooser,
    ...
    if (type != null && type.trim().length() != 0 && !type.equals("*/*")) {
      ...
      if (forcechooser) {
        ...
        activityIntent =
            Intent.createChooser(chooserIntent, c.getString(R.string.openwith));
      } else {
   ...
   * Open file from OTG
   */
  public static void openunknown(final DocumentFile f, final Context c,
                                 final boolean forcechooser,
                                 final boolean useNewStack) {
    Intent chooserIntent = new Intent();
    chooserIntent.setAction(Intent.ACTION_VIEW);
  ...
  public static Uri fileToContentUri(final Context context, final File file) {
    ...
    Uri uri = fileToContentUri(context, normalizedPath, file.isDirectory(),
                               EXTERNAL_VOLUME);
    if (uri != null) {
      return uri;
    }
    uri = fileToContentUri(context, normalizedPath, file.isDirectory(),
                           INTERNAL_VOLUME);
    if (uri != null) {
      return uri;
    }
  ...
  private static Uri fileToContentUri(final Context context, final String path,
    ...
    try {
      if (c != null && c.moveToNext()) {
        ...
        if (mimeType == Icons.IMAGE || mimeType == Icons.VIDEO ||
            mimeType == Icons.AUDIO) {
          isValid = true;
        } else {
          int type = c.getInt(
  ...
  /**
   * Method supports showing a UI to ask user to open a file without any
   * extension/mime
   */
  public static void openWith(final File f, final Context c,
    ...
    String[] items = new String[] {
        c.getString(R.string.text),     c.getString(R.string.image),
        c.getString(R.string.video),    c.getString(R.string.audio),
        c.getString(R.string.database), c.getString(R.string.other)};
   ...
   */
  public static boolean canGoBack(final Context context,
                                  final HybridFile currentFile) {
    ...
    case SFTP:
      return true;
    default:
      return true; // TODO: 29/9/2017 there might be nothing to go back to
                   // (check parent)
    }
  }

  public static long[] getSpaces(final HybridFile hFile, final Context context,
                                 final OnProgressUpdate<Long[]> updateState) {
  ...
  public static boolean copyToClipboard(final Context context,
                                        final String text) {
    try {
  ...
  public static void openFile(final File f, final MainActivity m,
                              final SharedPreferences sharedPrefs) {
    boolean useNewStack = sharedPrefs.getBoolean(
        PreferencesConstants.PREFERENCE_TEXTEDITOR_NEWSTACK, false);
    ...
    if (f.getName().toLowerCase().endsWith(".apk")) {
      GeneralDialogCreation.showPackageDialog(f, m);
               ...
               CompressedHelper.isFileExtractable(f.getPath())) {
    ...
    } else if (Icons.getTypeOfFile(f.getPath(), f.isDirectory()) ==
               Icons.AUDIO) {
      ...
      try {
        ...
        Toast.makeText(m, m.getString(R.string.noappfound), Toast.LENGTH_LONG)
            .show();
  ...
  private static boolean isSelfDefault(final File f, final Context c) {
    ...
    intent.setDataAndType(Uri.fromFile(f),
                          MimeTypes.getMimeType(f.getPath(), f.isDirectory()));
    ResolveInfo info = c.getPackageManager().resolveActivity(
   ...
   */
  public static void openFile(final DocumentFile f, final MainActivity m,
                              final SharedPreferences sharedPrefs) {
    boolean useNewStack = sharedPrefs.getBoolean(
        PreferencesConstants.PREFERENCE_TEXTEDITOR_NEWSTACK, false);
    try {
  ...
  public static ArrayList<HybridFile>
  toHybridFileConcurrentRadixTree(final ConcurrentRadixTree<VoidValue> a) {
  ...
  public static ArrayList<HybridFile>
  toHybridFileArrayList(final LinkedList<String> a) {
    ArrayList<HybridFile> b = new ArrayList<>();
    for (String s : a) {
  ...
  public static HybridFileParcelable parseName(final String line) {
    ...
    long Size =
        (size == null || size.trim().length() == 0) ? -1 : Long.parseLong(size);
    if (date.trim().length() > 0) {
      ...
      HybridFileParcelable baseFile = new HybridFileParcelable(
          name.toString(), array[0], stringDate.getTime(), Size, true);
  ...
  public static ArrayList<Boolean[]> parse(final String permLine) {
    ...
    Boolean[] read =
        new Boolean[] {permLine.charAt(1) == 'r', permLine.charAt(4) == 'r',
                       permLine.charAt(7) == 'r'};

    Boolean[] write =
        new Boolean[] {permLine.charAt(2) == 'w', permLine.charAt(5) == 'w',
                       permLine.charAt(8) == 'w'};

    Boolean[] execute =
        new Boolean[] {permLine.charAt(3) == 'x', permLine.charAt(6) == 'x',
                       permLine.charAt(9) == 'x'};
  ...
  public static boolean isPathAccessible(final String dir,
                                         final SharedPreferences pref) {
    File f = new File(dir);
    boolean showIfHidden = pref.getBoolean(
                PreferencesConstants.PREFERENCE_SHOW_HIDDENFILES, false),
            isDirSelfOrParent = dir.endsWith("/.") || dir.endsWith("/.."),
            showIfRoot = pref.getBoolean(
                PreferencesConstants.PREFERENCE_ROOTMODE, false);
  ...
  isRoot(final String dir) { // TODO: 5/5/2017 hardcoding root might lead to
  ...
  hybridListToFileArrayList(final ArrayList<HybridFileParcelable> a) {
   ...
   */
  public static void checkForPath(final Context context, final String path,
                                  final boolean isRootExplorer) {
    // TODO: Add support for SMB and OTG in this function
    if (!new File(path).exists()) {
      Toast
          .makeText(context, context.getString(R.string.bookmark_lost),
                    Toast.LENGTH_SHORT)
          .show();
      Operations.mkdir(RootHelper.generateBaseFile(new File(path), true),
                       context, isRootExplorer, new Operations.ErrorCallBack() {
                         // TODO empty
                         @Override
                         ...
                         @Override
                         public void launchSAF(final HybridFile file,
                                               final HybridFile file1) {}

                         @Override
                         public void done(final HybridFile hFile,
                                          final boolean b) {}
