...
/**
 * This class uses two sections, so that there doesn't need to be two different
 * Fragments. For sections info check switchSections() below.
 *
 * Created by Arpit on 21-06-2015 edited by Emmanuel Messulam
...
private static final String[] PREFERENCE_KEYS_SECTION_0 = {
	KEY_COLOREDNAV, "selectcolorconfig"
};
private static final String[] PREFERENCE_KEYS_SECTION_1 = {
	KEY_PRESELECTED_CONFIGS, PreferencesConstants.PREFERENCE_SKIN,
...
private void colorChangeDialog(final String colorPrefKey) {
	final UserColorPreferences userColorPreferences =
	...
	if (userColorPreferences != null) {
		...
		ColorAdapter adapter = new ColorAdapter(
			getActivity(), ColorPreference.availableColors, currentColor,
			(selectedColor)->{
				@ColorInt
				...
				activity.getColorPreference().saveColorPreferences(
					sharedPref, new UserColorPreferences(
						primaryFirst, primarySecond, accent, iconSkin));
				if (dialog != null)
					dialog.dismiss();
				invalidateEverything();
			});

		GridView v = (GridView)getActivity().getLayoutInflater().inflate(
			R.layout.dialog_grid, null);
		...
		dialog = new MaterialDialog.Builder(getActivity())
				...
				public void onNeutral(final MaterialDialog dialog) {
				        ...
				        activity.getColorPreference().saveColorPreferences(
						sharedPref, userColorPreferences);
				        invalidateEverything();
				}
			})
		         .customView(v, false)
...
private void loadSection1() {
	...
	selectedColors.setListener(()->{
			...
			int colorPickerPref =
				sharedPref.getInt(PreferencesConstants.PREFERENCE_COLOR_CONFIG,
				                  ColorPickerDialog.NO_DATA);
			if (colorPickerPref == ColorPickerDialog.RANDOM_INDEX) {
			        Toast.makeText(getActivity(), R.string.setRandom, Toast.LENGTH_LONG)
			        .show();
			}
		});

	((InvalidablePreferenceCategory)findPreference("category"))
	.invalidate(activity.getAccent());
...
private void checkCustomization() {
	...
	findPreference(PreferencesConstants.PREFERENCE_SKIN)
	.setEnabled(enableCustomization);
	findPreference(PreferencesConstants.PREFERENCE_SKIN_TWO)
	.setEnabled(enableCustomization);
	findPreference(PreferencesConstants.PREFERENCE_ACCENT)
	.setEnabled(enableCustomization);
	findPreference(PreferencesConstants.PREFERENCE_ICON_SKIN)
	.setEnabled(enableCustomization);
}

private void reloadListeners() {
	for (final String PREFERENCE_KEY :
	     (currentSection == SECTION_0 ? PREFERENCE_KEYS_SECTION_0
...
private void
invalidateColorPreference(final ColorPickerDialog selectedColors) {
	int colorPickerPref =
		sharedPref.getInt(PreferencesConstants.PREFERENCE_COLOR_CONFIG,
		                  ColorPickerDialog.NO_DATA);
