 ...
 *
 * Copyright © 2016-2018 Yashwanth Reddy Gondi, Vishal Nehra
...
public View onCreateView(final LayoutInflater inflater,
	...
	case LIGHT:
		startDividerView.setBackgroundColor(
			Utils.getColor(getContext(), R.color.divider));
	...
	ftpBtn.setOnClickListener(v->{
			if (!FtpService.isRunning()) {
					...
					startServer();
			                ...
			                // no wifi and no eth, we shouldn't be here in the first place,
...
public void onActivityCreated(final Bundle savedInstanceState) {
	...
	int skinTwoColor =
		mainActivity.getCurrentColorPreference().primarySecondTab;
...
public boolean onOptionsItemSelected(final MenuItem item) {
	...
	case R.id.choose_ftp_port:
		...
		.input(getString(R.string.ftp_port_edit_menu_title),
		       Integer.toString(currentFtpPort), true, (dialog, input)->{})
		...
		.onPositive((dialog, which)->{
				...
				if (editText != null) {
				        ...
				        if (portNumber < 1024) {
				                ...
				                Toast
				                .makeText(getActivity(), R.string.ftp_port_change_success,
				                          Toast.LENGTH_SHORT)
				                .show();
					}
				}
	...
	case R.id.ftp_path:
		...
		dialogBuilder.input(getString(R.string.ftp_path_hint),
		                    getDefaultPathFromPreferences(), false,
		                    (dialog, input)->{});
		dialogBuilder.onPositive((dialog, which)->{
				...
				if (editText != null) {
				        ...
				        if (pathFile.exists() && pathFile.isDirectory()) {
				                ...
				                Toast
				                .makeText(getActivity(), R.string.ftp_path_change_success,
				                          Toast.LENGTH_SHORT)
				                .show();
					} else {
				                // try to get parent
		...
		loginDialogBuilder.onPositive((dialog, which)->{
				if (mAnonymousCheckBox.isChecked()) {
				        ...
				        if (passwordEditText.getText().toString().equals("")) {
				                ...
				                // password and username field not empty, let's set them to
				                // preferences
	...
	case R.id.ftp_timeout:
		MaterialDialog.Builder timeoutBuilder =
		...
		timeoutBuilder.input(
			String.valueOf(FtpService.DEFAULT_TIMEOUT + " " +
			               getResources().getString(R.string.ftp_seconds)),
			String.valueOf(getFTPTimeout()), true, (dialog, input)->{
...
private BroadcastReceiver mWifiReceiver = new BroadcastReceiver() {
	...
	public void onReceive(final Context context, final Intent intent) {
		...
		if ((netInfo != null &&
			...
			ftpBtn.setText(
				getResources().getString(R.string.start_ftp).toUpperCase());
		}
	}
};
...
private BroadcastReceiver ftpReceiver = new BroadcastReceiver() {
	...
	public void onReceive(final Context context, final Intent intent) {
		...
		case FtpService.ACTION_STARTED:
			...
			ftpBtn.setText(
				getResources().getString(R.string.stop_ftp).toUpperCase());
			break;
...
private void updateSpans() {
	...
	String statusHead =
		getResources().getString(R.string.ftp_status_title) + ": ";

	spannedStatusConnected = Html.fromHtml(
		statusHead + "<b>&nbsp;&nbsp;"
		+ "<font color='" + accentColor + "'>" +
		getResources().getString(R.string.ftp_status_running) + "</font></b>");
	spannedStatusUrl = Html.fromHtml("URL:&nbsp;" + ftpAddress);
	spannedStatusNoConnection = Html.fromHtml(
		statusHead + "<b>&nbsp;&nbsp;&nbsp;&nbsp;"
		+ "<font color='" +
	...
	spannedStatusNotRunning = Html.fromHtml(
		statusHead + "<b>&nbsp;&nbsp;&nbsp;&nbsp;" +
		getResources().getString(R.string.ftp_status_not_running) + "</b>");
	spannedStatusSecure = Html.fromHtml(
		statusHead + "<b>&nbsp;&nbsp;&nbsp;&nbsp;"
		+ "<font color='" +
...
private void initLoginDialogViews(final View loginDialogView) {

	usernameEditText =
		loginDialogView.findViewById(R.id.edit_text_dialog_ftp_username);
	passwordEditText =
		loginDialogView.findViewById(R.id.edit_text_dialog_ftp_password);
	usernameTextInput =
		loginDialogView.findViewById(R.id.text_input_dialog_ftp_username);
	passwordTextInput =
		loginDialogView.findViewById(R.id.text_input_dialog_ftp_password);
	mAnonymousCheckBox =
		loginDialogView.findViewById(R.id.checkbox_ftp_anonymous);
...
private String getFTPAddressString() {
	...
	return (getSecurePreference() ? FtpService.INITIALS_HOST_SFTP
	                          : FtpService.INITIALS_HOST_FTP) +
	       ia.getHostAddress() + ":" + getDefaultPortFromPreferences();
}

private int getDefaultPortFromPreferences() {
	return mainActivity.getPrefs().getInt(FtpService.PORT_PREFERENCE_KEY,
	                                      FtpService.DEFAULT_PORT);
}

private String getUsernameFromPreferences() {
	return mainActivity.getPrefs().getString(FtpService.KEY_PREFERENCE_USERNAME,
	                                         FtpService.DEFAULT_USERNAME);
}

private String getPasswordFromPreferences() {
	try {
		String encryptedPassword = mainActivity.getPrefs().getString(
		...
		Toast
		.makeText(getContext(), getResources().getString(R.string.error),
		          Toast.LENGTH_SHORT)
		.show();
		// can't decrypt the password saved in preferences, remove the preference
		// altogether
		mainActivity.getPrefs()
		.edit()
		.putString(FtpService.KEY_PREFERENCE_PASSWORD, "")
		.apply();
		return "";
	}
}
...
private String getDefaultPathFromPreferences() {
	...
	return preferences.getString(FtpService.KEY_PREFERENCE_PATH,
	                             FtpService.DEFAULT_PATH);
}

private void changeFTPServerPort(final int port) {
	mainActivity.getPrefs()
	.edit()
...
private void changeFTPServerPath(final String path) {
	SharedPreferences preferences =
		PreferenceManager.getDefaultSharedPreferences(getActivity());
...
private void setFTPUsername(final String username) {
	mainActivity.getPrefs()
	.edit()
...
private void setFTPPassword(final String password) {
	try {
		mainActivity.getPrefs()
		.edit()
		.putString(FtpService.KEY_PREFERENCE_PASSWORD,
...
private int getFTPTimeout() {
	return mainActivity.getPrefs().getInt(FtpService.KEY_PREFERENCE_TIMEOUT,
	                                      FtpService.DEFAULT_TIMEOUT);
}

private void setFTPTimeout(final int seconds) {
	mainActivity.getPrefs()
	.edit()
	.putInt(FtpService.KEY_PREFERENCE_TIMEOUT, seconds)
	.apply();
}

private boolean getSecurePreference() {
	return mainActivity.getPrefs().getBoolean(FtpService.KEY_PREFERENCE_SECURE,
	                                          FtpService.DEFAULT_SECURE);
}

private void setSecurePreference(final boolean isSecureEnabled) {
	mainActivity.getPrefs()
	.edit()
