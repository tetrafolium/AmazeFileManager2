...
public class CloudSheetFragment
	extends BottomSheetDialogFragment implements View.OnClickListener {

private View rootView;
private LinearLayout mSmbLayout, mScpLayout, mDropboxLayout, mBoxLayout,
                     mGoogleDriveLayout, mOnedriveLayout, mGetCloudLayout;
...
public void setupDialog(final Dialog dialog, final int style) {
	...
	if (((MainActivity)getActivity()).getAppTheme().equals(AppTheme.DARK)) {
		...
		rootView.setBackgroundColor(
			Utils.getColor(getContext(), android.R.color.white));
	}
...
public static final boolean isCloudProviderAvailable(final Context context) {
	...
	try {
		pm.getPackageInfo(CloudContract.APP_PACKAGE_NAME,
...
public void onClick(final View v) {
	...
	case R.id.linear_layout_get_cloud:
		...
		cloudPluginIntent.setData(
			Uri.parse("market://details?id=com.filemanager.amazecloud"));
