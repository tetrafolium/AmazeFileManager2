...
public void onActivityCreated(final Bundle savedInstanceState) {
	...
	ListPreloader<String> preloader =
		new ListPreloader<>(GlideApp.with(app), modelProvider, sizeProvider,
		                    GlideConstants.MAX_PRELOAD_APPSADAPTER);

	adapter = new AppsAdapter(getContext(), (ThemedActivity)getActivity(),
	                          utilsProvider, modelProvider, sizeProvider,
...
/**
 * Assigns sort modes
 * A value from 0 to 2 defines sort mode as name/last modified/size in
 * ascending order Values from 3 to 5 defines sort mode as name/last
 * modified/size in descending order <p> Final value of {@link #sortby} varies
 * from 0 to 2
 */
...
@Override
public Loader<AppListLoader.AppsDataPair> onCreateLoader(final int id,
                                                         final Bundle args) {
	return new AppListLoader(getContext(), sortby, asc);
}

@Override
public void onLoadFinished(final Loader<AppListLoader.AppsDataPair> loader,
                           final AppListLoader.AppsDataPair data) {
