...
public class ProcessViewerFragment extends Fragment {
  ...
  /**
   * Helps defining the result type for {@link
   * #processResults(DatapointParcelable, int)} to process
   */
  private static final int SERVICE_COPY = 0, SERVICE_EXTRACT = 1,
                           SERVICE_COMPRESS = 2, SERVICE_ENCRYPT = 3,
                           SERVICE_DECRYPT = 4;
  ...
  private TextView mProgressTypeText, mProgressFileNameText, mProgressBytesText,
      mProgressFileText, mProgressSpeedText, mProgressTimer;

  private ServiceConnection mCopyConnection, mExtractConnection,
      mCompressConnection, mEncryptConnection, mDecryptConnection;
  ...
  public View onCreateView(final LayoutInflater inflater,
    ...
    mProgressFileNameText =
        rootView.findViewById(R.id.text_view_progress_file_name);
    ...
    mCopyConnection =
        new CustomServiceConnection(this, mLineChart, SERVICE_COPY);
    mExtractConnection =
        new CustomServiceConnection(this, mLineChart, SERVICE_EXTRACT);
    mCompressConnection =
        new CustomServiceConnection(this, mLineChart, SERVICE_COMPRESS);
    mEncryptConnection =
        new CustomServiceConnection(this, mLineChart, SERVICE_ENCRYPT);
    mDecryptConnection =
        new CustomServiceConnection(this, mLineChart, SERVICE_DECRYPT);
  ...
  public void processResults(final DatapointParcelable dataPackage,
    ...
    if (dataPackage != null) {
      ...
      Spanned bytesText = Html.fromHtml(
          getResources().getString(R.string.written) + " <font color='" +
          accentColor + "'><i>" +
          Formatter.formatFileSize(getContext(), doneBytes) + " </font></i>" +
      ...
      Spanned fileProcessedSpan =
          Html.fromHtml(getResources().getString(R.string.processing_file) +
                        " <font color='" + accentColor + "'><i>" +
                        (dataPackage.sourceProgress) + " </font></i>" +
                        getResources().getString(R.string.of) + " <i>" +
                        dataPackage.sourceFiles + "</i>");
      mProgressFileText.setText(fileProcessedSpan);

      Spanned speedSpan = Html.fromHtml(
          getResources().getString(R.string.current_speed) + ": <font color='" +
          accentColor + "'><i>" +
          Formatter.formatFileSize(getContext(), dataPackage.speedRaw) +
          "/s</font></i>");
      mProgressSpeedText.setText(speedSpan);

      Spanned timerSpan = Html.fromHtml(
          getResources().getString(R.string.service_timer) + ": <font color='" +
          accentColor + "'><i>" + Utils.formatTimer(++looseTimeInSeconds) +
          "</font></i>");
  ...
  private void setupDrawables(final int serviceType, final boolean isMove) {
    ...
    case SERVICE_COPY:
      ...
      mProgressTypeText.setText(
          isMove ? getResources().getString(R.string.moving)
    ...
    case SERVICE_EXTRACT:
      if (mainActivity.getAppTheme().equals(AppTheme.DARK) ||
        ...
        mProgressImage.setImageDrawable(
            getResources().getDrawable(R.drawable.ic_zip_box_white));
      } else {
        mProgressImage.setImageDrawable(
            getResources().getDrawable(R.drawable.ic_zip_box_grey));
      }
    ...
    case SERVICE_ENCRYPT:
      ...
      mProgressTypeText.setText(
          getResources().getString(R.string.crypt_encrypting));
    ...
    case SERVICE_DECRYPT:
      ...
      mProgressTypeText.setText(
          getResources().getString(R.string.crypt_decrypting));
      cancelBroadcast(new Intent(EncryptService.TAG_BROADCAST_CRYPT_CANCEL));
      break;
    }
  }
  ...
  private void cancelBroadcast(final Intent intent) {
    ...
    mCancelButton.setOnClickListener(v -> {
      Toast
          .makeText(getActivity(), getResources().getString(R.string.stopping),
                    Toast.LENGTH_LONG)
          .show();
  ...
  /**
   * Add a new entry dynamically to the chart, initializes a {@link LineDataSet}
   * if not done so
  ...
  private void chartInit(final long totalBytes) {
    ...
    yAxisLeft.setGridColor(
        Utils.getColor(getContext(), R.color.white_translucent));
  ...
  private static class CustomServiceConnection implements ServiceConnection {
    ...
    public CustomServiceConnection(final ProcessViewerFragment frag,
    ...
    public void onServiceConnected(final ComponentName name,
      ...
      ObtainableServiceBinder<? extends AbstractProgressiveService> binder =
          (ObtainableServiceBinder<? extends AbstractProgressiveService>)
              service;
