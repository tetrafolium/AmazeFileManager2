...
public class MainFragment extends Fragment implements BottomBarButtonPath {
  ...
  private HybridFileParcelable
      encryptBaseFile; // the cached base file which we're to open, delete it
                       // later
  ...
  /*
   * boolean identifying if the search task should be re-run on back press after
   * pressing on any of the search result
   */
  ...
  public void onCreate(final Bundle savedInstanceState) {
    ...
    if (ordinalValue != -1)
      openMode = OpenMode.getOpenMode(ordinalValue);

    IS_LIST = dataUtils.getListOrGridForPath(CURRENT_PATH, DataUtils.LIST) ==
              DataUtils.LIST;

    accentColor = getMainActivity().getAccent();
    primaryColor =
        getMainActivity().getCurrentColorPreference().primaryFirstTab;
    primaryTwoColor =
        getMainActivity().getCurrentColorPreference().primarySecondTab;
  }
  ...
  public View onCreateView(final LayoutInflater inflater,
    ...
    // String itemsstring = res.getString(R.string.items);// TODO: 23/5/2017 use
    // or delete
  ...
  public void onActivityCreated(final Bundle savedInstanceState) {
    ...
    if (utilsProvider.getAppTheme().equals(AppTheme.LIGHT) && !IS_LIST) {
      listView.setBackgroundColor(
          Utils.getColor(getContext(), R.color.grid_background_light));
    } else {
    ...
    columns =
        Integer.parseInt(sharedPref.getString(PREFERENCE_GRID_COLUMNS, "-1"));
    if (IS_LIST) {
    ...
    // View footerView =
    // getActivity().getLayoutInflater().inflate(R.layout.divider, null);// TODO:
    // 23/5/2017 use or delete
    dividerItemDecoration = new DividerItemDecoration(
        getActivity(), false, getBoolean(PREFERENCE_SHOW_DIVIDERS));
    listView.addItemDecoration(dividerItemDecoration);
    mSwipeRefreshLayout.setColorSchemeColors(accentColor);
    DefaultItemAnimator animator = new DefaultItemAnimator();
    listView.setItemAnimator(animator);
    mToolbarContainer.getViewTreeObserver().addOnGlobalLayoutListener(
  ...
  void setGridLayoutSpanSizeLookup(final GridLayoutManager mLayoutManagerGrid) {
  ...
  public void switchView() {
    boolean isPathLayoutGrid =
        dataUtils.getListOrGridForPath(CURRENT_PATH, DataUtils.LIST) ==
        DataUtils.GRID;
  ...
  public void onSaveInstanceState(final Bundle outState) {
    ...
    if (listView != null) {
      ...
      if (selection) {
        outState.putIntegerArrayList("position",
                                     adapter.getCheckedItemsIndex());
      }
  ...
  void onSavedInstanceState(final Bundle savedInstanceState) {
    ...
    if (cur != null) {
      ...
      getMainActivity().getAppbar().getBottomBar().updatePath(
          CURRENT_PATH, results, MainActivityHelper.SEARCH_TEXT, openMode,
          folder_count, file_count, this);
      reloadListElements(true, results, !IS_LIST);
      if (savedInstanceState.getBoolean("selection")) {
        for (Integer index :
             savedInstanceState.getIntegerArrayList("position")) {
  ...
  public ActionMode.Callback mActionModeCallback = new ActionMode.Callback() {
    ...
    public boolean onCreateActionMode(final ActionMode mode, final Menu menu) {
      ...
      // getMainActivity().translateDrawerList(true);
      ...
      getMainActivity().updateViews(
          new ColorDrawable(res.getColor(R.color.holo_dark_action_mode)));

      // do not allow drawer to open when item gets selected
      if (!getMainActivity().getDrawer().isLocked()) {
        getMainActivity().getDrawer().lockIfNotOnTablet(
    ...
    public boolean onPrepareActionMode(final ActionMode mode, final Menu menu) {
      ...
      menu.findItem(R.id.all).setTitle(
          positions.size() == folder_count + file_count ? R.string.deselect_all
                                                        : R.string.selectall);
    ...
    public boolean onActionItemClicked(final ActionMode mode,
      ...
      case R.id.openmulti:
        ...
        try {
          ...
          intent_result.putParcelableArrayListExtra(Intent.EXTRA_STREAM,
                                                    resulturis);
          getActivity().finish();
          // mode.finish();
        } catch (Exception e) {
      ...
      case R.id.about:
        ...
        GeneralDialogCreation.showPropertiesDialogWithPermissions(
            (x).generateBaseFile(), x.permissions,
            (ThemedActivity)getActivity(), getMainActivity().isRootExplorer(),
            utilsProvider.getAppTheme());
        mode.finish();
        return true;
      case R.id.delete:
        GeneralDialogCreation.deleteFilesDialog(getContext(), LIST_ELEMENTS,
                                                getMainActivity(), checkedItems,
                                                utilsProvider.getAppTheme());
        return true;
      case R.id.share:
        ...
        if (arrayList.size() > 100)
          ...
          case ONEDRIVE:
            FileUtils.shareCloudFile(LIST_ELEMENTS.get(0).desc,
                                     LIST_ELEMENTS.get(0).getMode(),
                                     getContext());
            break;
          default:
            FileUtils.shareFiles(arrayList, getActivity(),
                                 utilsProvider.getAppTheme(), accentColor);
            break;
          }
        }
        return true;
      case R.id.openparent:
        loadlist(new File(checkedItems.get(0).desc).getParent(), false,
                 OpenMode.FILE);
        return true;
      case R.id.all:
      ...
      case R.id.hide:
        ...
        return true;
      case R.id.ex:
        getMainActivity().mainActivityHelper.extractFile(
            new File(checkedItems.get(0).desc));
        mode.finish();
        return true;
      case R.id.cpy:
      case R.id.cut: {
        HybridFileParcelable[] copies =
            new HybridFileParcelable[checkedItems.size()];
        for (int i = 0; i < checkedItems.size(); i++) {
          copies[i] = checkedItems.get(i).generateBaseFile();
        }
        int op = item.getItemId() == R.id.cpy ? PasteHelper.OPERATION_COPY
      ...
      case R.id.compress:
        ...
        GeneralDialogCreation.showCompressDialog((MainActivity)getActivity(),
                                                 copies1, CURRENT_PATH);
        mode.finish();
        return true;
      case R.id.openwith:
        boolean useNewStack =
            sharedPref.getBoolean(PREFERENCE_TEXTEDITOR_NEWSTACK, false);
        FileUtils.openunknown(new File(checkedItems.get(0).desc), getActivity(),
                              true, useNewStack);
        return true;
    ...
    public void onDestroyActionMode(final ActionMode mode) {
      ...
      // if (getMainActivity().isDrawerLocked)
      // getMainActivity().translateDrawerList(false);
  ...
  private BroadcastReceiver receiver2 = new BroadcastReceiver() {
    ...
    public void onReceive(final Context context, final Intent intent) {
      ...
      case ROOT:
      case FILE:
        // local file system don't need an explicit load, we've set an observer
        // to take actions on creation/moving/deletion/modification of file on
        // current path

        // run media scanner
        String[] path = new String[1];
        String arg =
            intent.getStringExtra(MainActivity.KEY_INTENT_LOAD_LIST_FILE);

        // run media scanner for only one context
        if (arg != null &&
            getMainActivity().getCurrentMainFragment() == MainFragment.this) {
          ...
          if (Build.VERSION.SDK_INT >= 19) {
            ...
            mediaScannerConnection = new MediaScannerConnection(
                context, mediaScannerConnectionClient);
  ...
  private BroadcastReceiver decryptReceiver = new BroadcastReceiver() {
    ...
    public void onReceive(final Context context, final Intent intent) {
      ...
      if (isEncryptOpen && encryptBaseFile != null) {
        FileUtils.openFile(new File(encryptBaseFile.getPath()),
                           getMainActivity(), sharedPref);
        isEncryptOpen = false;
      }
    }
  };

  public void home() { ma.loadlist((ma.home), false, OpenMode.FILE); }
   ...
   */
  public void onListItemClicked(final boolean isBackButton, final int position,
                                final LayoutElementParcelable e,
    ...
    if (results) {
      ...
      SearchWorkerFragment fragment =
          (SearchWorkerFragment)fragmentManager.findFragmentByTag(
    ...
    if (selection) {
      ...
      if (isBackButton) {
        ...
        if (e.isDirectory) {
          ...
          encryptBaseFile = new HybridFileParcelable(
          ...
          EncryptDecryptUtils.decryptFile(
              getContext(), getMainActivity(), ma, openMode,
              e.generateBaseFile(),
              getActivity().getExternalCacheDir().getPath(), utilsProvider,
              true);
        } else {
          if (getMainActivity().mReturnIntent) {
            ...
            case SFTP:
              Toast
                  .makeText(getContext(),
                            ...
                            Toast.LENGTH_LONG)
                  .show();
              SshClientUtils.launchSftp(e.generateBaseFile(),
                                        getMainActivity());
              break;
            case OTG:
              FileUtils.openFile(
            ...
            case ONEDRIVE:
              Toast
                  .makeText(getContext(),
                            ...
                            Toast.LENGTH_LONG)
                  .show();
              CloudUtil.launchCloud(e.generateBaseFile(), openMode,
                                    getMainActivity());
              break;
            default:
              FileUtils.openFile(new File(e.desc), (MainActivity)getActivity(),
                                 sharedPref);
              break;
            }
   ...
   * @return the entry
   */
  private static EncryptedEntry findEncryptedEntry(final Context context,
                                                   final String path)
      throws Exception {
  ...
  public void updateTabWithDb(final Tab tab) {
  ...
  /**
   * Returns the intent with uri corresponding to specific {@link
   * HybridFileParcelable} back to external app
   */
  public void returnIntentResults(final HybridFileParcelable baseFile) {
    ...
    if (getMainActivity().mRingtonePickerIntent) {
      ...
      Uri mediaStoreUri =
          MediaStoreHack.getUriFromFile(baseFile.getPath(), getActivity());
      Log.d(getClass().getSimpleName(),
            mediaStoreUri.toString() + "\t" +
                MimeTypes.getMimeType(baseFile.getPath(),
      ...
      if (resultUri != null)
        intentresult.setDataAndType(resultUri,
                                    MimeTypes.getExtension(baseFile.getPath()));
   ...
   * @param back if we're coming back from any directory and want the scroll to
   *     be restored
   * @param openMode the mode in which the directory should be opened
   */
  public void loadlist(final String path, final boolean back,
    ...
    if (loadFilesListTask != null &&
        loadFilesListTask.getStatus() == AsyncTask.Status.RUNNING) {
      loadFilesListTask.cancel(true);
    }

    loadFilesListTask = new LoadFilesListTask(
        ma.getActivity(), path, ma, openMode, getBoolean(PREFERENCE_SHOW_THUMB),
        getBoolean(PREFERENCE_SHOW_HIDDENFILES), (data) -> {
          if (data != null && data.second != null) {
            boolean isPathLayoutGrid =
                dataUtils.getListOrGridForPath(path, DataUtils.LIST) ==
                DataUtils.GRID;
            setListElements(data.second, back, path, data.first, false,
                            isPathLayoutGrid);
  ...
  void initNoFileLayout() {
    ...
    if (utilsProvider.getAppTheme().equals(AppTheme.LIGHT)) {
      ...
      nofilesview.setBackgroundColor(
          Utils.getColor(getContext(), android.R.color.black));
      ((TextView)nofilesview.findViewById(R.id.nofiletext))
          .setTextColor(Color.WHITE);
    } else {
      nofilesview.setBackgroundColor(
          Utils.getColor(getContext(), R.color.holo_dark_background));
      ((TextView)nofilesview.findViewById(R.id.nofiletext))
          .setTextColor(Color.WHITE);
    }
  }
   ...
   * @param back     if we're coming back from any directory and want the scroll
   *     to be restored
   ...
   */
  public void setListElements(final ArrayList<LayoutElementParcelable> bitmap,
                              final boolean back, final String path,
                              final OpenMode openMode, final boolean results,
                              final boolean grid) {
    if (bitmap != null) {
  ...
  public void reloadListElements(final boolean back, final boolean results,
                                 final boolean grid) {
    if (isAdded()) {
      ...
      if (adapter == null) {
        adapter = new RecyclerAdapter(getMainActivity(), ma, utilsProvider,
                                      sharedPref, listView, LIST_ELEMENTS,
                                      ma.getActivity());
      } else {
      ...
      if (addheader && IS_LIST) {
        dividerItemDecoration = new DividerItemDecoration(
            getActivity(), true, getBoolean(PREFERENCE_SHOW_DIVIDERS));
        listView.addItemDecoration(dividerItemDecoration);
        addheader = false;
      }
  ...
  private LayoutElementParcelable getBackElement() {
    if (back == null) {
      back = new LayoutElementParcelable(getContext(), true,
  ...
  private void startFileObserver() {
    ...
    case FILE:
      ...
      if (file.isDirectory() && file.canRead()) {
        ...
        customFileObserver = new CustomFileObserver(
            CURRENT_PATH,
  ...
  public void rename(final HybridFileParcelable f) {
    MaterialDialog renameDialog = GeneralDialogCreation.showNameDialog(
        getMainActivity(), "", f.getName(),
        getResources().getString(R.string.rename),
        getResources().getString(R.string.save), null,
    ...
    // place cursor at the starting of edit text by posting a runnable to edit
    // text this is done because in case android has not populated the edit text
    // layouts yet, it'll reset calls to selection if not posted in message
    // queue
    EditText textfield =
  ...
  public void goBack() {
    ...
    if (!results) {
      if (!mRetainSearchTask) {
        ...
        // case when we had pressed on an item from search results and wanna go
        // back leads to resuming the search task

        if (MainActivityHelper.SEARCH_TEXT != null) {

          // starting the search query again :O
          getMainActivity().mainFragment = (MainFragment)getMainActivity()
                                               .getTabFragment()
          ...
          String parentPath =
              new HybridFile(openMode, CURRENT_PATH).getParent(getActivity());
          ...
          MainActivityHelper.addSearchFragment(
              fm, new SearchWorkerFragment(), parentPath,
              MainActivityHelper.SEARCH_TEXT, openMode,
              getMainActivity().isRootExplorer(),
              sharedPref.getBoolean(SearchWorkerFragment.KEY_REGEX, false),
              sharedPref.getBoolean(SearchWorkerFragment.KEY_REGEX_MATCHES,
                                    false));
        } else
      ...
      try {
        getMainActivity().runOnUiThread(() -> {
          ...
          if ((i = dataUtils.containsServer(smbPath)) != -1) {
            getMainActivity().showSMBDialog(dataUtils.getServers().get(i)[0],
                                            smbPath, true);
          }
        });
      } catch (Exception e) {
  ...
  /**
   * Assigns sort modes
   * A value from 0 to 3 defines sort mode as name/last modified/size/type in
   * ascending order Values from 4 to 7 defines sort mode as name/last
   * modified/size/type in descending order <p> Final value of {@link #sortby}
   * varies from 0 to 3
   */
  public void getSortModes() {
    ...
    dsort = Integer.parseInt(
        sharedPref.getString(PREFERENCE_DIRECTORY_SORT_MODE, "0"));
  }
  ...
  public ArrayList<LayoutElementParcelable>
  addToSmb(final SmbFile[] mFile, final String path,
           final boolean showHiddenFiles) throws SmbException {
    ArrayList<LayoutElementParcelable> smbFileList = new ArrayList<>();
    if (searchHelper.size() > 500)
      searchHelper.clear();
    for (SmbFile aMFile : mFile) {
      if ((dataUtils.isFileHidden(aMFile.getPath()) || aMFile.isHidden()) &&
          !showHiddenFiles) {
        continue;
      }
      ...
      if (aMFile.isDirectory()) {
        ...
        LayoutElementParcelable layoutElement = new LayoutElementParcelable(
            getContext(), name, aMFile.getPath(), "", "", "", 0, false,
            aMFile.lastModified() + "", true, getBoolean(PREFERENCE_SHOW_THUMB),
            OpenMode.SMB);
        ...
        file_count++;
        LayoutElementParcelable layoutElement = new LayoutElementParcelable(
            getContext(), name, aMFile.getPath(), "", "",
            Formatter.formatFileSize(getContext(), aMFile.length()),
            aMFile.length(), false, aMFile.lastModified() + "", false,
  ...
  private LayoutElementParcelable addTo(final HybridFileParcelable mFile) {
    ...
    if (!dataUtils.isFileHidden(mFile.getPath())) {
      if (mFile.isDirectory()) {
        size = "";
        LayoutElementParcelable layoutElement = new LayoutElementParcelable(
            getContext(), f.getPath(), mFile.getPermission(), mFile.getLink(),
            size, 0, true, mFile.getDate() + "", false,
        ...
        try {
          LayoutElementParcelable layoutElement = new LayoutElementParcelable(
              getContext(), f.getPath(), mFile.getPermission(), mFile.getLink(),
              size, longSize, false, mFile.getDate() + "", false,
              getBoolean(PREFERENCE_SHOW_THUMB), mFile.getMode());
          LIST_ELEMENTS.add(layoutElement);
          file_count++;
          return layoutElement;
  ...
  public void hide(final String path) {
    ...
    if (file.isDirectory()) {
      ...
      if (!f1.exists()) {
        try {
          getMainActivity().mainActivityHelper.mkFile(
              new HybridFile(OpenMode.FILE, f1.getPath()), this);
        } catch (Exception e) {
          e.printStackTrace();
        }
      }
  ...
  public String getCurrentPath() { return CURRENT_PATH; }

  private void addShortcut(final LayoutElementParcelable path) {
  ...
  public void onSearchPreExecute(final String query) {
    getMainActivity().getAppbar().getBottomBar().setPathText("");
    getMainActivity().getAppbar().getBottomBar().setFullPathText(
        getString(R.string.searching, query));
  }

  // adds search results based on result boolean. If false, the adapter is
  // initialised with initial values, if true, new values are added to the
  // adapter.
  public void addSearchResult(final HybridFileParcelable a,
                              final String query) {
    if (listView != null) {
  ...
  public void onSearchCompleted(final String query) {
    ...
    new AsyncTask<Void, Void, Void>() {
      ...
      public void onPostExecute(final Void c) {
        reloadListElements(
            true, true, !IS_LIST); // TODO: 7/7/2017 this is really inneffient,
                                   // use RecycleAdapter's createHeaders()
        getMainActivity().getAppbar().getBottomBar().setPathText("");
        getMainActivity().getAppbar().getBottomBar().setFullPathText(
            getString(R.string.searchresults, query));
      }
    }.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
  }

  public static void launchSMB(final HybridFileParcelable baseFile,
      ...
      public void run() {
        try {
          ...
          for (Layoutelements layoutelement : LIST_ELEMENTS) {
              ...
              if (smbFile.getName().contains(smbFile.getName()))
          subtitleFiles.add(smbFile);
          }
          */
          ...
          activity.runOnUiThread(() -> {
            try {
              Uri uri = Uri.parse(
                  Streamer.URL +
                  Uri.fromFile(
                         new File(Uri.parse(baseFile.getPath()).getPath()))
                      .getEncodedPath());
