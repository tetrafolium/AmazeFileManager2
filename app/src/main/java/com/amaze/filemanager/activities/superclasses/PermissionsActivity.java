...
public class PermissionsActivity extends ThemedActivity
  ...
  private OnPermissionGranted[] permissionCallbacks =
  ...
  public void requestStoragePermission(
    ...
    final MaterialDialog materialDialog = GeneralDialogCreation.showBasicDialog(
        this, R.string.grant_storage_permission, R.string.grantper,
        R.string.grant, R.string.cancel);
    materialDialog.getActionButton(DialogAction.NEGATIVE)
        .setOnClickListener(v -> finish());
    materialDialog.setCancelable(false);

    requestPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE,
                      STORAGE_PERMISSION, materialDialog, onPermissionGranted);
  }
  ...
  public void requestInstallApkPermission(
      @NonNull final OnPermissionGranted onPermissionGranted) {
    final MaterialDialog materialDialog = GeneralDialogCreation.showBasicDialog(
        this, R.string.grant_apkinstall_permission, R.string.grantper,
        R.string.grant, R.string.cancel);
    materialDialog.getActionButton(DialogAction.NEGATIVE)
    ...
    requestPermission(Manifest.permission.REQUEST_INSTALL_PACKAGES,
                      INSTALL_APK_PERMISSION, materialDialog,
                      onPermissionGranted);
  }

  /**
   * Requests permission, overrides {@param rationale}'s POSITIVE button dialog
   * action.
   * @param permission The permission to ask for
   * @param code {@link #STORAGE_PERMISSION} or {@link #INSTALL_APK_PERMISSION}
   * @param rationale MaterialLayout to provide an additional rationale to the
   *     user if the permission was not granted
   *                  and the user would benefit from additional context for the
   * use of the permission. For example, if the request has been denied
   * previously.
   */
  public void
  requestPermission(final String permission, final int code,
  ...
  public interface OnPermissionGranted { void onPermissionGranted(); }
}
