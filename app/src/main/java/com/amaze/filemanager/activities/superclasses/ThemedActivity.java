...
public class ThemedActivity extends PreferenceActivity {
  ...
  public void onCreate(final Bundle savedInstanceState) {
    ...
    int colorPickerPref =
        getPrefs().getInt(PreferencesConstants.PREFERENCE_COLOR_CONFIG,
                          ColorPickerDialog.NO_DATA);
    if (colorPickerPref == ColorPickerDialog.RANDOM_INDEX) {
      getColorPreference().saveColorPreferences(
          getPrefs(), ColorPreferenceHelper.randomize(this));
    }
  ...
  public UserColorPreferences getCurrentColorPreference() {
    return getColorPreference().getCurrentUserColorPreferences(this,
                                                               getPrefs());
  }

  public @ColorInt int getAccent() {
    return getColorPreference()
        .getCurrentUserColorPreferences(this, getPrefs())
        .accent;
  }
  ...
  void setTheme() {
    ...
    if (Build.VERSION.SDK_INT >= 21) {
      ...
      String stringRepresentation =
          String.format("#%06X", (0xFFFFFF & getAccent()));
