 ...
 * MainActivity.java
 *
 * Copyright (C) 2014-2018 Arpit Khurana <arpitkh96@gmail.com>, Vishal Nehra
...
public class MainActivity extends PermissionsActivity
    implements SmbConnectionListener, DataChangeListener, BookmarkCallback,
  ...
  // oppathList - the paths at which certain operation needs to be performed
  // (pairs with oparrayList)
  ...
  public static final String TAG_INTENT_FILTER_GENERAL =
      "general_communications";
  public static final String ARGS_KEY_LOADER = "loader_cloud_args_service";

  /**
   * Broadcast which will be fired after every file operation, will denote list
   * loading Registered by {@link MainFragment}
   */
  public static final String KEY_INTENT_LOAD_LIST = "loadlist";

  /**
   * Extras carried by the list loading intent
   * Contains path of parent directory in which operation took place, so that we
   * can run media scanner on it
   */
  public static final String KEY_INTENT_LOAD_LIST_FILE = "loadlist_file";

  /**
   * Mime type in intent that apps need to pass when trying to open file manager
   * from a specific directory Should be clubbed with {@link Intent#ACTION_VIEW}
   * and send in path to open in intent data field
   */
  public static final String ARGS_INTENT_ACTION_VIEW_MIME_FOLDER =
      "resource/folder";

  public static final String CLOUD_AUTHENTICATOR_GDRIVE =
      "android.intent.category.BROWSABLE";
  public static final String CLOUD_AUTHENTICATOR_REDIRECT_URI =
      "com.amaze.filemanager:/auth";
  ...
  private static final String DEFAULT_FALLBACK_STORAGE_PATH =
      "/storage/sdcard0";
  ...
  public void onCreate(final Bundle savedInstanceState) {
    ...
    if (CloudSheetFragment.isCloudProviderAvailable(this)) {
      ...
      getSupportLoaderManager().initLoader(REQUEST_CODE_CLOUD_LIST_KEYS, null,
                                           this);
    }

    path = getIntent().getStringExtra("path");
    openProcesses =
        getIntent().getBooleanExtra(KEY_INTENT_PROCESS_VIEWER, false);
    intent = getIntent();
    ...
    if (savedInstanceState != null) {
      drawer.setSomethingSelected(
          savedInstanceState.getBoolean(KEY_DRAWER_SELECTED));
    }

    // setting window background color instead of each item, in order to reduce
    // pixel overdraw
    ...
    /*findViewById(R.id.drawer_buttton).setOnClickListener(new
    ...
    if (SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      ActivityManager.TaskDescription taskDescription =
          new ActivityManager.TaskDescription(
              "Amaze",
              ((BitmapDrawable)getResources().getDrawable(R.mipmap.ic_launcher))
                  .getBitmap(),
              ColorPreferenceHelper.getPrimary(getCurrentColorPreference(),
    ...
    AppConfig.runInParallel(new AppConfig.CustomAsyncCallbacks<Void, Void>(
      ...
      public Void doInBackground() {
        ...
        dataUtils.setHiddenFiles(
            utilsHandler.getHiddenFilesConcurrentRadixTree());
      ...
      public void onPostExecute(final Void result) {
        ...
        if (savedInstanceState == null) {
          if (openProcesses) {
            ...
            transaction.replace(R.id.content_frame, new ProcessViewerFragment(),
                                KEY_INTENT_PROCESS_VIEWER);
            ...
            if (path != null && path.length() > 0) {
              ...
              if (file.isDirectory(MainActivity.this))
                ...
                goToMain(null);
                FileUtils.openFile(new File(path), MainActivity.this,
                                   getPrefs());
              }
            } else {
              goToMain(null);
          ...
          oparrayList = savedInstanceState.getParcelableArrayList(
              KEY_OPERATIONS_PATH_LIST);
          operation = savedInstanceState.getInt(KEY_OPERATION);
          // mainFragment = (Main)
          // savedInstanceState.getParcelable("main_fragment");
        }
      }
    });
  }
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M &&
      ...
      requestStoragePermission(() -> {
        ...
        if (b) {
          ...
          getPrefs()
              .edit()
              .putBoolean(PREFERENCE_NEED_TO_SET_HOME, false)
              .commit();
        } else {
          // just refresh list
          if (tabFragment != null) {
  ...
  /**
   * Checks for the action to take when Amaze receives an intent from external
   * source
   */
  private void checkForExternalIntent(final Intent intent) {
    ...
    if (actionIntent != null) {
      if (actionIntent.equals(Intent.ACTION_GET_CONTENT)) {
        // file picker intent
        mReturnIntent = true;
        Toast.makeText(this, getString(R.string.pick_a_file), Toast.LENGTH_LONG)
            .show();
        ...
        Utils.disableScreenRotation(this);
      } else if (actionIntent.equals(Intent.ACTION_SEND_MULTIPLE) &&
                 type != null) {
  ...
  /**
   * Initializes the floating action button to act as to save data from an
   * external intent
   */
  private void initFabToSave(final ArrayList<Uri> uris) {
    ...
    floatingActionButton.setMenuButtonIcon(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
          ...
          if (result == MainActivityHelper.WRITABLE_OR_ON_SDCARD) {
            FileUtil.writeUriToStorage(
                MainActivity.this, uris, getContentResolver(),
          ...
          FileUtil.writeUriToStorage(MainActivity.this, uris,
                                     getContentResolver(),
                                     getCurrentMainFragment().getCurrentPath());
          Toast
  ...
  /**
   * Initializes an interactive shell, which will stay throughout the app
   * lifecycle The shell is associated with a handler thread which maintain the
   * message queue from the callbacks of shell as we certainly cannot allow the
   * callbacks to run on same thread because of possible deadlock situation and
   * the asynchronous behaviour of LibSuperSU
   */
  private void initializeInteractiveShell() {
    // only one looper can be associated to a thread. So we are making sure not
    // to create new handler threads every time the code relaunch.
    if (isRootExplorer()) {
      handlerThread = new HandlerThread("handler");
      handlerThread.start();
      handler = new Handler(handlerThread.getLooper());
      shellInteractive =
          (new Shell.Builder()).useSU().setHandler(handler).open();

      // TODO: check for busybox
      /*try {
          if (!RootUtils.isBusyboxAvailable()) {
              Toast.makeText(this, getString(R.string.error_busybox),
      Toast.LENGTH_LONG).show(); closeInteractiveShell();
              sharedPref.edit().putBoolean(PreferenceUtils.KEY_ROOT,
      false).apply();
          }
  ...
  public synchronized ArrayList<String> getStorageDirectories() {
    ...
    final String rawEmulatedStorageTarget =
      ...
      if (compressedExplorerFragment.mActionMode == null) {
        if (compressedExplorerFragment.canGoBack()) {
          ...
          FragmentTransaction fragmentTransaction =
              getSupportFragmentManager().beginTransaction();
          fragmentTransaction.setCustomAnimations(R.anim.slide_out_bottom,
  ...
  public void invalidatePasteButton(final MenuItem paste) {
    if (pasteHelper != null) {
  ...
  public void goToMain(final String path) {
    FragmentTransaction transaction =
        getSupportFragmentManager().beginTransaction();
  ...
  public boolean onCreateOptionsMenu(final Menu menu) {
    ...
    MenuItemCompat.setOnActionExpandListener(search, new
  ...
  public boolean onPrepareOptionsMenu(final Menu menu) {
    ...
    if (fragment instanceof TabFragment) {
      ...
      try {
        ...
        appbar.getBottomBar().updatePath(
            ma.getCurrentPath(), ma.results, MainActivityHelper.SEARCH_TEXT,
            ma.openMode, ma.folder_count, ma.file_count, ma);
      } catch (Exception e) {
      }
  ...
  public boolean onOptionsItemSelected(final MenuItem item) {
    ...
    case R.id.history:
      if (ma != null)
        GeneralDialogCreation.showHistoryDialog(dataUtils, getPrefs(), ma,
                                                getAppTheme());
      break;
    case R.id.sethome:
      if (ma == null)
      ...
      if (main.openMode != OpenMode.FILE && main.openMode != OpenMode.ROOT) {
        Toast.makeText(mainActivity, R.string.not_allowed, Toast.LENGTH_SHORT)
            .show();
        break;
      }
      final MaterialDialog dialog = GeneralDialogCreation.showBasicDialog(
          mainActivity, R.string.questionset, R.string.setashome, R.string.yes,
          R.string.no);
    ...
    case R.id.sort:
      ...
      if (fragment instanceof AppsListFragment) {
        GeneralDialogCreation.showSortDialog((AppsListFragment)fragment,
                                             getAppTheme());
      }
      break;
    case R.id.sortby:
    ...
    case R.id.dsort:
      ...
      int current = Integer.parseInt(getPrefs().getString(
          PreferencesConstants.PREFERENCE_DIRECTORY_SORT_MODE, "0"));

      final MainFragment mainFrag = ma;

      builder.items(sort).itemsCallbackSingleChoice(
          current, (dialog1, view, which, text) -> {
            getPrefs()
                .edit()
                .putString(PreferencesConstants.PREFERENCE_DIRECTORY_SORT_MODE,
                           "" + which)
                .commit();
    ...
    case R.id.hiddenitems:
      GeneralDialogCreation.showHiddenDialog(dataUtils, getPrefs(), ma,
                                             getAppTheme());
      break;
    case R.id.view:
      final MainFragment mainFragment = ma;
      int pathLayout =
          dataUtils.getListOrGridForPath(ma.getCurrentPath(), DataUtils.LIST);
      if (ma.IS_LIST) {
        if (pathLayout == DataUtils.LIST) {
          AppConfig.runInBackground(() -> {
            utilsHandler.removeFromDatabase(new OperationData(
                UtilsHandler.Operation.LIST, mainFragment.getCurrentPath()));
          });
        }
        utilsHandler.saveToDatabase(new OperationData(
  ...
  public void onRestoreInstanceState(Bundle savedInstanceState){
      ...
      oppathe1 = savedInstanceState.getString(KEY_OPERATED_ON_PATH);
      oparrayList =
  savedInstanceState.getStringArrayList(KEY_OPERATIONS_PATH_LIST);
  ...
  private void updateUsbInformation() {
    ...
    List<UsbOtgRepresentation> connectedDevices =
        OTGUtil.getMassStorageDevicesConnected(this);
    ...
    if (!connectedDevices.isEmpty()) {
      ...
      if (!isInformationUpdated) {
        SingletonUsbOtg.getInstance().setConnectedDevice(
  ...
  /**
   * Receiver to check if a USB device is connected at the runtime of
   * application
   */
  ...
  protected void onDestroy() {
    super.onDestroy();
    // TODO: 6/5/2017 Android may choose to not call this method before
    // destruction
    // TODO:
  ...
  public void openCompressed(final String path) {
    appBarLayout.animate()
        .translationY(0)
        .setInterpolator(new DecelerateInterpolator(2))
        .start();
    FragmentTransaction fragmentTransaction =
        getSupportFragmentManager().beginTransaction();
    fragmentTransaction.setCustomAnimations(R.anim.slide_in_top,
  ...
  public File getUsbDrive() {
    ...
    try {
      for (File f : parent.listFiles())
        if (f.exists() && f.getName().toLowerCase().contains("usb") &&
            f.canExecute())
          return f;
    } catch (Exception e) {
    }
  ...
  public FABsMenu getFAB() { return floatingActionButton; }

  public AppBar getAppbar() { return appbar; }
    ...
    if (requestCode == Drawer.image_selector_request_code) {
      ...
      if (responseCode == Activity.RESULT_OK) {
        ...
        if (treeUri != null)
          getPrefs()
              .edit()
              .putString(PreferencesConstants.PREFERENCE_URI,
                         treeUri.toString())
              .commit();
      } else {
        // If not confirmed SAF, or if still not writable, then revert settings.
        /* DialogUtil.displayError(getActivity(),
           R.string.message_dialog_cannot_write_to_folder_saf, false,
           currentFolder);
      ...
      case DataUtils.RENAME:
        MainFragment ma = getCurrentMainFragment();
        mainActivityHelper.rename(ma.openMode, (oppathe), (oppathe1),
                                  mainActivity, isRootExplorer());
        ma.updateList();
        break;
      case DataUtils.NEW_FILE:
        mainActivityHelper.mkFile(new HybridFile(OpenMode.FILE, oppathe),
                                  getCurrentMainFragment());
        break;
      ...
      case DataUtils.SAVE_FILE:
        FileUtil.writeUriToStorage(this, urisToBeSaved, getContentResolver(),
                                   getCurrentMainFragment().getCurrentPath());
        urisToBeSaved = null;
        finish();
        break;
      ...
      if (responseCode == Activity.RESULT_OK && intent.getData() != null) {
        ...
        getCurrentMainFragment().loadlist(OTGUtil.PREFIX_OTG, false,
                                          OpenMode.OTG);
  ...
  void initialisePreferences() {
    currentTab = getPrefs().getInt(PreferencesConstants.PREFERENCE_CURRENT_TAB,
                                   PreferenceUtils.DEFAULT_CURRENT_TAB);
    @ColorInt
    int currentPrimary = ColorPreferenceHelper.getPrimary(
  ...
  void initialiseViews() {
    ...
    // buttonBarFrame.setBackgroundColor(Color.parseColor(currentTab==1 ?
    // skinTwo : skin));
    ...
    // ColorDrawable(Color.parseColor((currentTab==1 ? skinTwo : skin))));
    ...
    if (SDK_INT == Build.VERSION_CODES.KITKAT_WATCH ||
      ...
      // tintManager.setStatusBarTintColor(Color.parseColor((currentTab==1 ?
      // skinTwo : skin)));
      FrameLayout.MarginLayoutParams p =
          (ViewGroup.MarginLayoutParams)findViewById(R.id.drawer_layout)
              .getLayoutParams();
  ...
  /**
   * Call this method when you need to update the MainActivity view components'
   * colors based on update in the {@link MainActivity#currentTab} Warning - All
   * the variables should be initialised before calling this method!
   */
  public void updateViews(final ColorDrawable colorDrawable) {
    // appbar view color
    ...
    if (SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      ...
      if (getBoolean(PREFERENCE_COLORED_NAVIGATION))
        mainActivity.getWindow().setNavigationBarColor(
            PreferenceUtils.getStatusColor(colorDrawable.getColor()));
  ...
  void initialiseFab() {
    ...
    floatingActionButton.getMenuButton().setRippleColor(
        Utils.getColor(this, R.color.white_translucent));
    floatingActionButton.setAnimationDuration(500);
    floatingActionButton.setMenuListener(new FABsMenuListener() {
      @Override
      public void onMenuExpanded(final FABsMenu fabsMenu) {
        showSmokeScreen();
      }

      @Override
      public void onMenuCollapsed(final FABsMenu fabsMenu) {
        hideSmokeScreen();
      }
    });
    ...
    initFabTitle(findViewById(R.id.menu_new_folder),
                 MainActivityHelper.NEW_FOLDER);
    initFabTitle(findViewById(R.id.menu_new_file), MainActivityHelper.NEW_FILE);
    initFabTitle(findViewById(R.id.menu_new_cloud),
                 MainActivityHelper.NEW_CLOUD);
  }

  private void initFabTitle(final TitleFAB fabTitle, final int type) {
    ...
    switch (getAppTheme().getSimpleTheme()) {
    case DARK:
      fabTitle.setTitleBackgroundColor(
  ...
  public boolean copyToClipboard(final Context context, final String text) {
    try {
      android.content.ClipboardManager clipboard =
          (android.content.ClipboardManager)context.getSystemService(
              CLIPBOARD_SERVICE);
      android.content.ClipData clip = android.content.ClipData.newPlainText(
          "Path copied to clipboard", text);
  ...
  public void renameBookmark(final String title, final String path) {
    if (dataUtils.containsBooks(new String[] {title, path}) != -1) {
      RenameBookmark renameBookmark =
          RenameBookmark.getInstance(title, path, getAccent());
      if (renameBookmark != null)
        renameBookmark.show(getFragmentManager(), "renamedialog");
    }
  }

  public PasteHelper getPaste() { return pasteHelper; }

  public void setPaste(final PasteHelper p) {
    pasteHelper = p;
    supportInvalidateOptionsMenu();
  }
  ...
  public void onNewIntent(final Intent i) {
    ...
    if (path != null) {
    ...
    } else if (i.getStringArrayListExtra(TAG_INTENT_FILTER_FAILED_OPS) !=
               null) {
      ArrayList<HybridFileParcelable> failedOps =
      ...
      // we used an external authenticator instead of APIs. Probably for Google
      // Drive
      CloudRail.setAuthenticationResponse(intent);
    } else if ((openProcesses =
                    i.getBooleanExtra(KEY_INTENT_PROCESS_VIEWER, false))) {
      FragmentTransaction transaction =
          getSupportFragmentManager().beginTransaction();
      transaction.replace(R.id.content_frame, new ProcessViewerFragment(),
                          KEY_INTENT_PROCESS_VIEWER);
  ...
  public void showSMBDialog(final String name, final String path,
                            final boolean edit) {
  ...
  public void showSftpDialog(final String name, final String path,
                             final boolean edit) {
    if (path.length() > 0 && name.length() == 0) {
      int i = dataUtils.containsServer(new String[] {name, path});
      if (i != -1)
    ...
    if (userinfo.indexOf(':') < 0) {
      ...
      bundle.putString("keypairName",
                       utilsHandler.getSshAuthPrivateKeyName(path));
    } else {
  ...
  /**
   * Shows a view that goes from white at it's lowest part to transparent a the
   * top. It covers the fragment.
   */
  public void showSmokeScreen() { FileUtils.revealShow(fabBgView, true); }

  public void hideSmokeScreen() { FileUtils.revealShow(fabBgView, false); }

  @Override
  public void addConnection(final boolean edit, @NonNull final String name,
    ...
    if (!edit) {
      if ((dataUtils.containsServer(path)) == -1) {
        ...
        Snackbar
            .make(findViewById(R.id.navigation),
                  getString(R.string.connection_exists), Snackbar.LENGTH_SHORT)
            .show();
      }
    } else {
  ...
  @Override
  public void onHiddenFileAdded(final String path) {
    utilsHandler.saveToDatabase(
        new OperationData(UtilsHandler.Operation.HIDDEN, path));
  }

  @Override
  public void onHiddenFileRemoved(final String path) {
    utilsHandler.removeFromDatabase(
        new OperationData(UtilsHandler.Operation.HIDDEN, path));
  }

  @Override
  public void onHistoryAdded(final String path) {
    utilsHandler.saveToDatabase(
        new OperationData(UtilsHandler.Operation.HISTORY, path));
  }

  @Override
  public void onBookAdded(final String[] path, final boolean refreshdrawer) {
    utilsHandler.saveToDatabase(
  ...
  @Override
  public void delete(final String title, final String path) {
    utilsHandler.removeFromDatabase(
  ...
  @Override
  public void modify(final String oldpath, final String oldname,
                     final String newPath, final String newname) {
  ...
  @Override
  public void onPostExecute(final String query) {
  ...
  @Override
  public void addConnection(final OpenMode service) {
    try {
      if (cloudHandler.findEntry(service) != null) {
        // cloud entry already exists
        Toast
            .makeText(this,
                      getResources().getString(R.string.connection_exists),
                      Toast.LENGTH_LONG)
            .show();
      } else {
        Toast
        ...
        Loader loader =
            getSupportLoaderManager().getLoader(REQUEST_CODE_CLOUD_LIST_KEY);
        ...
        getSupportLoaderManager().initLoader(REQUEST_CODE_CLOUD_LIST_KEY, args,
                                             this);
      }
    } catch (CloudPluginException e) {
      e.printStackTrace();
      Toast
          .makeText(this, getResources().getString(R.string.cloud_error_plugin),
                    Toast.LENGTH_LONG)
          .show();
    }
  }
  ...
  @Override
  public void deleteConnection(final OpenMode service) {
  ...
  public Loader<Cursor> onCreateLoader(final int id, final Bundle args) {
    Uri uri = Uri.withAppendedPath(
        Uri.parse("content://" + CloudContract.PROVIDER_AUTHORITY),
        "/keys.db/secret_keys");

    String[] projection =
        new String[] {CloudContract.COLUMN_ID, CloudContract.COLUMN_CLIENT_ID,
    ...
    case REQUEST_CODE_CLOUD_LIST_KEY:
      ...
      return new CursorLoader(this, uriAppendedPath, projection, null, null,
                              null);
    case REQUEST_CODE_CLOUD_LIST_KEYS:
      ...
      try {
        ...
        // we want keys for services saved in database, and the cloudrail app
        // key which is at index 1
        ...
        return new CursorLoader(this, uri, projection, CloudContract.COLUMN_ID,
                                ids, null);
      } catch (CloudPluginException e) {
        e.printStackTrace();
        ...
        Toast
            .makeText(this,
                      getResources().getString(R.string.cloud_error_plugin),
                      Toast.LENGTH_LONG)
            .show();
      }
    default:
      Uri undefinedUriAppendedPath = ContentUris.withAppendedId(uri, 7);
      return new CursorLoader(this, undefinedUriAppendedPath, projection, null,
                              null, null);
    }
  }
  ...
  public void onLoadFinished(final Loader<Cursor> loader, final Cursor data) {
    ...
    cloudLoaderAsyncTask =
        new CloudLoaderAsyncTask(this, cloudHandler, cloudCursorData);
