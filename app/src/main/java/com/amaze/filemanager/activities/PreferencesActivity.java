...
public class PreferencesActivity extends ThemedActivity {
  ...
  private Parcelable[] fragmentsListViewParcelables =
      new Parcelable[NUMBER_OF_PREFERENCES];
  ...
  /**
   * This is a hack, each PreferenceFragment has a ListView that loses it's
   * state (specifically the scrolled position) when the user accesses another
   * PreferenceFragment. To prevent this, the Activity saves the ListView's
   * state, so that it can be restored when the user returns to the
   * PreferenceFragment.
   *
   * We cannot use the normal save/restore state functions because they only get
   * called when the OS kills the fragment, not the user. See
   * https://stackoverflow.com/a/12793395/3124150 for a better explanation.
   *
   * We cannot save the Parcelable in the fragment because the fragment is
   * destroyed.
   */
  public void saveListViewState(final int prefFragment,
                                final Parcelable listViewState) {
    fragmentsListViewParcelables[prefFragment] = listViewState;
  }

  /**
   * This is a hack see {@link PreferencesActivity#saveListViewState(int,
   * Parcelable)}
   */
  ...
  public void setRestartActivity() { restartActivity = true; }

  public boolean getRestartActivity() { return restartActivity; }
  ...
  public void invalidateNavBar() {
    ...
    if (SDK_INT == 20 || SDK_INT == 19) {
      ...
      FrameLayout.MarginLayoutParams p =
          (ViewGroup.MarginLayoutParams)findViewById(R.id.preferences)
              .getLayoutParams();
      SystemBarTintManager.SystemBarConfig config = tintManager.getConfig();
      p.setMargins(0, config.getStatusBarHeight(), 0, 0);
    } else if (SDK_INT >= 21) {
      boolean colourednavigation =
  ...
  /**
   * This 'elegantly' destroys the activity and recreates it so that the
   * different widgets and texts change their inner states's colors.
   */
  ...
  /**
   * When a Preference (that requires an independent fragment) is selected this
   * is called.
  ...
  private void loadPrefFragment(final PreferenceFragment fragment,
                                final @StringRes int titleBarName) {
