...
public class CircularColorsView extends View {
  ...
  public void setDividerColor(final int color) { dividerPaint.setColor(color); }
  ...
  protected void onDraw(final Canvas canvas) {
    ...
    if (isInEditMode())
      setColors(Color.CYAN, Color.RED, Color.GREEN, Color.BLUE);
    if (!paintInitialized)
    ...
    float diameterByWidth =
        (getWidth() - distance * 2) / 3f * DIAMETER_PERCENTUAL;
    ...
    float[] positionX = {
        ...
        getWidth() - diameter - distance - radius, getWidth() - radius};
    semicicleRect.set(positionX[0] - radius, centerY - radius,
                      positionX[0] + radius, centerY + radius);

    canvas.drawArc(semicicleRect, 90, 180, true, colors[0]);
    canvas.drawArc(semicicleRect, 270, 180, true, colors[1]);

    canvas.drawLine(semicicleRect.centerX(), semicicleRect.top,
                    semicicleRect.centerX(), semicicleRect.bottom,
                    dividerPaint);
