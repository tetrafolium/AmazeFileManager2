 ...
 * @param path the full path to the server. Includes an un-encrypted
 *     password to support
 *             runtime loading without reloading stuff from database.
 * @param encryptedPath the full path to the server. Includes encrypted
 *     password to save in
 *                      database. Later be decrypted at every boot when we
 * read from db entry.
 * @param oldname the old name of connection if we're here to edit
 * @param oldPath the old full path (un-encrypted as we read from existing
 *     entry in db, which
 *                we decrypted beforehand).
 */
void addConnection(boolean edit, String name, String path,
                   String encryptedPath, String oldname, String oldPath);

/**
 * Callback denoting a connection been deleted from dialog
 * @param name name of connection as in navigation drawer and in database
 *     entry
 * @param path the full path to server. Includes an un-encrypted password as
 *     we decrypted it
 *             beforehand while reading from database before coming here to
 * delete. We'll later have to encrypt the password back again in order to
 * match entry from db and to successfully delete it. If we don't want this
 * behaviour, then we'll have to not allow duplicate connection name, and
 * delete entry based on the name only. But that is not supported as of now.
 *             See {@link
 * com.amaze.filemanager.database.UtilsHandler#removeSmbPath(String,
 * String)}
 */
...
public Dialog onCreateDialog(final Bundle savedInstanceState) {
	...
	emptyName =
		getString(R.string.cantbeempty, getString(R.string.connectionname));
	invalidDomain = getString(R.string.invalid, getString(R.string.domain));
	invalidUsername =
		getString(R.string.invalid, getString(R.string.username).toLowerCase());
	...
	ba3.onPositive((dialog, which)->{
			...
			if (ch.isChecked())
				smbFile = createSMBPath(new String[] {ipa, "", "", domaind}, true);
			        ...
			        smbFile =
					createSMBPath(new String[] {ipa, useru, passp, domaind}, false);
			}
			...
			try {
			        ...
			        e.printStackTrace();
			        Toast
			        .makeText(getActivity(), getString(R.string.error),
			                  Toast.LENGTH_LONG)
			        .show();
			        return;
			}
			...
			if (smbConnectionListener != null) {
			        ...
			        smbConnectionListener.addConnection(edit, s[0], smbFile.getPath(), s[1],
			                                            name, path);
			}
			dismiss();
		});
...
private SmbFile createSMBPath(final String[] auth, final boolean anonym) {
	try {
...
private static ColorStateList createEditTextColorStateList(final int color) {
...
private static void setTint(final EditText editText, final int color) {
