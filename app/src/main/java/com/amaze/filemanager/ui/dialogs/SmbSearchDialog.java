...
public class SmbSearchDialog extends DialogFragment {
        ...
        if (getActivity() != null) {
          getActivity().runOnUiThread(() -> {
            if (computers.size() == 1) {
              dismiss();
              Toast
                  .makeText(getActivity(), getString(R.string.nodevicefound),
                            Toast.LENGTH_SHORT)
                  .show();
              MainActivity mainActivity = (MainActivity)getActivity();
              mainActivity.showSMBDialog("", "", false);
  ...
  private class ListViewAdapter
      extends RecyclerView.Adapter<ElementViewHolder> {
    ...
    public ListViewAdapter(final Context context,
                           final List<ComputerParcelable> objects) {
    ...
    @Override
    public ElementViewHolder onCreateViewHolder(final ViewGroup parent,
                                                final int viewType) {
      View view;
      switch (viewType) {
    ...
    @Override
    public void onBindViewHolder(final ElementViewHolder holder,
                                 final int position) {
