...
public class Indicator
  ...
  public Indicator(final Context context) { this(context, null, 0); }
  ...
  public Indicator(final Context context, final AttributeSet attrs,
    ...
    unselectedColour =
        Color.argb(80, Color.red(selectedColour), Color.green(selectedColour),
  ...
  public void onPageScrolled(final int position, final float positionOffset,
    ...
    if (isAttachedToWindow) {
      ...
      // when swiping from #2 to #1 ViewPager reports position as 1 and a
      // descending offset need to convert this into our left-dot-based
      // 'coordinate space'
      if (currentPosition != position) {
        fraction = 1f - positionOffset;

        // if user scrolls completely to next page then the position param
        // updates to that new page but we're not ready to switch our 'current'
        // page yet so adjust for that
        if (fraction == 1f) {
  ...
  private void drawUnselected(final Canvas canvas) {
    ...
    for (int page = 0; page < pageCount; page++) {
      ...
      combinedUnselectedPath.op(
          getUnselectedPath(page, dotCenterX[page], dotCenterX[nextXIndex],
   ...
   *
   * It can also be in a combination of these states e.g. joining one neighbour
   * while retreating from another.  We therefore create a Path so that we can
   * examine each dot pair separately and later take the union for these cases.
   *
   * This function returns a path for the given dot **and any action to it's
   * right** e.g. joining or retreating from it's neighbour
   */
  private Path getUnselectedPath(final int page, final float centerX,
    ...
    if ((joiningFraction == 0f || joiningFraction == INVALID_FRACTION) &&
      ...
      // case #1 – At rest
      unselectedDotPath.addCircle(dotCenterX[page], dotCenterY, dotRadius,
                                  Path.Direction.CW);
    }
    ...
    if (joiningFraction > 0f && joiningFraction <= 0.5f &&
      ...
      unselectedDotLeftPath.cubicTo(controlX1, controlY1, controlX2, controlY2,
                                    endX1, endY1);
      ...
      // semi circle to the top center
      rectF.set(nextCenterX - dotRadius, dotTopY, nextCenterX + dotRadius,
                dotBottomY);
      ...
      controlX2 = endX1;
      controlY2 = endY1 - halfDotRadius;
      unselectedDotRightPath.cubicTo(controlX1, controlY1, controlX2, controlY2,
                                     endX1, endY1);
    ...
    if (joiningFraction > 0.5f && joiningFraction < 1f &&
      ...
      // adjust the fraction so that it goes from 0.3 -> 1 to produce a more
      // realistic 'join'
      ...
      controlY2 = endY1;
      unselectedDotPath.cubicTo(controlX1, controlY1, controlX2, controlY2,
                                endX1, endY1);
    ...
    if (joiningFraction == 1 && retreatingJoinX1 == INVALID_FRACTION) {
      ...
      // case #4 Joining neighbour, combined straight technically we could use
      // case 3 for this situation as well but assume that this is an
      // optimization rather than faffing around with beziers just to draw a
      // rounded rect
      rectF.set(centerX - dotRadius, dotTopY, nextCenterX + dotRadius,
                dotBottomY);
    ...
    // this is done separately so that we can have a single retreating path
    // spanning multiple dots and therefore animate it's movement smoothly
  ...
  private ValueAnimator createMoveSelectedAnimator(final float moveTo,
                                                   final int was, final int now,
                                                   final int steps) {

    // create the actual move animator
    ValueAnimator moveSelected = ValueAnimator.ofFloat(selectedDotX, moveTo);

    // also set up a pending retreat anim – this starts when the move is 75%
    // complete
    retreatAnimation = new PendingRetreatAnimator(
        was, now, steps,
        now > was ? new RightwardStartPredicate(
    ...
    moveSelected.addListener(new AnimatorListenerAdapter() {
      ...
      public void onAnimationStart(final Animator animation) {
        // set a flag so that we continue to draw the unselected dot in the
        // target position until the selected dot has finished moving into place
        selectedDotInPosition = false;
      }

      @Override
      public void onAnimationEnd(final Animator animation) {
        // set a flag when anim finishes so that we don't draw both selected &
        // unselected page dots
        selectedDotInPosition = true;
      }
    });
  ...
  /**
   * An Animator that shows and then shrinks a retreating join between the
   * previous and newly selected pages.  This also sets up some pending dot
   * reveals – to be started when the retreat has passed the dot to be revealed.
   */
  private class PendingRetreatAnimator extends PendingStartAnimator {

    PendingRetreatAnimator(final int was, final int now, final int steps,
                           final StartPredicate predicate) {
      super(predicate);
      setDuration(animHalfDuration);
      setInterpolator(interpolator);

      // work out the start/end values of the retreating join from the direction
      // we're travelling in.  Also look at the current selected dot position,
      // i.e. we're moving on before a prior anim has finished.
      final float initialX1 =
          now > was ? Math.min(dotCenterX[was], selectedDotX) - dotRadius
                    : dotCenterX[now] - dotRadius;
      final float finalX1 =
          now > was ? dotCenterX[now] - dotRadius : dotCenterX[now] - dotRadius;
      ...
      // hold on to the indexes of the dots that will be hidden by the retreat
      // so that we can initialize their revealFraction's i.e. make sure they're
      // hidden while the reveal animation runs
      final int[] dotsToHide = new int[steps];
      if (initialX1 != finalX1) { // rightward retreat
        setFloatValues(initialX1, finalX1);
        // create the reveal animations that will run when the retreat passes
        // them
        for (int i = 0; i < steps; i++) {
          revealAnimations[i] = new PendingRevealAnimator(
              was + i, new RightwardStartPredicate(dotCenterX[was + i]));
          dotsToHide[i] = was + i;
        }
        ...
        for (int i = 0; i < steps; i++) {
          revealAnimations[i] = new PendingRevealAnimator(
              was - i, new LeftwardStartPredicate(dotCenterX[was - i]));
          dotsToHide[i] = was - i;
        }
        addUpdateListener(valueAnimator -> {
          // todo avoid autoboxing
      ...
      addListener(new AnimatorListenerAdapter() {
        ...
        public void onAnimationStart(final Animator animation) {
          ...
          // we need to set this so that the dots are hidden until the reveal
          // anim runs
  ...
  private class PendingRevealAnimator extends PendingStartAnimator {
    ...
    public PendingRevealAnimator(final int dot,
                                 final StartPredicate predicate) {
      super(predicate);
  ...
  /**
   * A predicate used to start an animation when a given value is greater than a
   * threshold
   */
  ...
  /**
   * A predicate used to start an animation then a given value is less than a
   * threshold
   */
