...
public class SearchView {
  ...
  public SearchView(final AppBar appbar, final MainActivity a,
    ...
    backImageView.setOnClickListener(
        v -> appbar.getSearchView().hideSearchView());
    ...
    // searchViewEditText.setTextColor(Utils.getColor(this,
  ...
  public void revealSearchView() {
    ...
    int endRadius = Math.max(appbar.getToolbar().getWidth(),
                             appbar.getToolbar().getHeight());

    Animator animator;
    if (SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
      int[] searchCoords = new int[2];
      View searchItem = appbar.getToolbar().findViewById(
          R.id.search); // It could change position, get it every time
      searchViewEditText.setText("");
      searchItem.getLocationOnScreen(searchCoords);
      animator = ViewAnimationUtils.createCircularReveal(
          searchViewLayout, searchCoords[0] + 32, searchCoords[1] - 16,
          START_RADIUS, endRadius);
    } else {
    ...
    animator.addListener(new Animator.AnimatorListener() {
      ...
      public void onAnimationEnd(final Animator animation) {
        ...
        inputMethodManager.hideSoftInputFromWindow(
            searchViewEditText.getWindowToken(),
  ...
  public boolean isShown() { return searchViewLayout.isShown(); }

  public interface SearchListener { void onSearch(String queue); }
}
