 ...
 * @param appTheme current theme light/dark which will determine the boundary
 *     color
 * @param metrics  to convert the boundary width for {@link #setStroke} method
 *     from dp to px
 */
public CircleGradientDrawable(final String color, final AppTheme appTheme,
 ...
 *     from dp to px
 */
public CircleGradientDrawable(final @ColorInt int color,
...
public CircleGradientDrawable(final AppTheme appTheme,
                              final DisplayMetrics metrics) {
	this.mDisplayMetrics = metrics;

	setShape(OVAL);
	setSize(1, 1);
	setStroke(dpToPx(STROKE_WIDTH), (appTheme.equals(AppTheme.DARK) ||
	                                 appTheme.equals(AppTheme.BLACK))
