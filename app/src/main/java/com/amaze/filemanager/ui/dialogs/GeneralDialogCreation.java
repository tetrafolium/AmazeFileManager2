 ...
 *
 * Copyright © 2017-2019 Vishal Nehra <vishalmeham2@gmail.com>, Emmanuel
 * Messulam <emmanuelbendavid@gmail.com>, Raymond Lai <airwave209gt at
...
public class GeneralDialogCreation {
  ...
  showBasicDialog(final ThemedActivity themedActivity,
                  final @StringRes int content, final @StringRes int title,
                  final @StringRes int postiveText,
                  final @StringRes int negativeText) {
  ...
  public static MaterialDialog
  showNameDialog(final MainActivity m, final String hint, final String prefill,
                 final String title, final String positiveButtonText,
                 final String neutralButtonText,
                 final String negativeButtonText,
                 final MaterialDialog.SingleButtonCallback positiveButtonAction,
    ...
    View dialogView =
        m.getLayoutInflater().inflate(R.layout.dialog_singleedittext, null);
    ...
    WarnableTextInputValidator textInputValidator =
        new WarnableTextInputValidator(
            builder.getContext(), textfield, tilTextfield,
  ...
  public static void deleteFilesDialog(
      final Context c, final ArrayList<LayoutElementParcelable> layoutElements,
      final MainActivity mainActivity,
      final List<LayoutElementParcelable> positions, final AppTheme appTheme) {
    ...
    MaterialDialog dialog =
        new MaterialDialog.Builder(c)
            ...
            .onPositive((dialog1, which) -> {
              Toast
                  .makeText(c, c.getString(R.string.deleting),
                            Toast.LENGTH_SHORT)
                  .show();
    ...
    new AsyncTask<Void, Object, Void>() {
      ...
      protected Void doInBackground(final Void... params) {
        ...
        for (int i = 0; i < positions.size(); i++) {
          ...
          publishProgress(sizeTotal, counterFiles, counterDirectories, files,
                          directories);
        }
        return null;
      }
      ...
      @Override
      protected void onProgressUpdate(final Object... result) {
        ...
        updateViews(tempSizeTotal, tempFilesStringBuilder,
                    tempDirectoriesStringBuilder, tempCounterFiles,
      ...
      protected void onPostExecute(final Void aVoid) {
        ...
        updateViews(sizeTotal, files, directories, counterFiles,
                    counterDirectories);
      }

      private void updateViews(
          final long tempSizeTotal, final StringBuilder filesStringBuilder,
          final StringBuilder directoriesStringBuilder, final int... values) {
        ...
        if (tempCounterDirectories != 0 || tempCounterFiles != 0) {
          ...
          if (listDirectories.getVisibility() != View.VISIBLE &&
              tempCounterDirectories != 0)
            listDirectories.setVisibility(View.VISIBLE);
          listFiles.setText(filesStringBuilder);
          if (listFiles.getVisibility() != View.VISIBLE &&
              tempCounterFiles != 0)
            listFiles.setVisibility(View.VISIBLE);

          if (categoryDirectories.getVisibility() != View.VISIBLE &&
              tempCounterDirectories != 0)
            categoryDirectories.setVisibility(View.VISIBLE);
          if (categoryFiles.getVisibility() != View.VISIBLE &&
              tempCounterFiles != 0)
            categoryFiles.setVisibility(View.VISIBLE);
        }

        // Show total size with at least one directory or file and size is not
        // zero.
        if (tempCounterFiles + tempCounterDirectories > 1 &&
            tempSizeTotal > 0) {
  ...
  public static void showPropertiesDialogWithPermissions(
      final HybridFileParcelable baseFile, final String permissions,
      final ThemedActivity activity, final boolean isRoot,
      final AppTheme appTheme) {
    showPropertiesDialog(baseFile, permissions, activity, isRoot, appTheme,
                         true, false);
  }

  public static void
  showPropertiesDialogWithoutPermissions(final HybridFileParcelable f,
  ...
  public static void
  showPropertiesDialogForStorage(final HybridFileParcelable f,
                                 final ThemedActivity activity,
                                 final AppTheme appTheme) {
  ...
  private static void showPropertiesDialog(
      final HybridFileParcelable baseFile, final String permissions,
      final ThemedActivity base, final boolean isRoot, final AppTheme appTheme,
      final boolean showPermissions, final boolean forStorage) {
    ...
    /*View setup*/
    {
      ...
      LinearLayout mNameLinearLayout =
          v.findViewById(R.id.properties_dialog_name);
      LinearLayout mLocationLinearLayout =
          v.findViewById(R.id.properties_dialog_location);
      LinearLayout mSizeLinearLayout =
          v.findViewById(R.id.properties_dialog_size);
      LinearLayout mDateLinearLayout =
          v.findViewById(R.id.properties_dialog_date);
    ...
    /*Chart creation and data loading*/
    {
      boolean isRightToLeft =
          c.getResources().getBoolean(R.bool.is_right_to_left);
      ...
      if (forStorage) {
        final String[] LEGENDS = new String[] {c.getString(R.string.used),
                                               c.getString(R.string.free)};
        final int[] COLORS = {Utils.getColor(c, R.color.piechart_red),
      ...
      if (isRoot && permissions.length() > 6) {
        ...
        appCompatButton.setOnClickListener(v15 -> {
          if (permissionsTable.getVisibility() == View.GONE) {
            ...
            setPermissionsDialog(permissionsTable, button, baseFile,
                                 permissions, c, main);
          } else {
  ...
  public static class SizeFormatter implements IValueFormatter {
    ...
    public SizeFormatter(final Context c) { context = c; }
  ...
  public static void showCloudDialog(final MainActivity mainActivity,
                                     final AppTheme appTheme,
    ...
    builder.onPositive(
        (dialog, which) -> mainActivity.deleteConnection(openMode));
  ...
  public static void showEncryptWarningDialog(
      final Intent intent, final MainFragment main, final AppTheme appTheme,
    ...
    final MaterialDialog.Builder builder =
        new MaterialDialog.Builder(main.getActivity());
    ...
    builder.onNegative((dialog, which) -> {
      preferences.edit()
          .putBoolean(PreferencesConstants.PREFERENCE_CRYPT_WARNING_REMEMBER,
                      true)
          .apply();
      try {
        encryptButtonCallbackInterface.onButtonPressed(intent);
  ...
  public static void showEncryptWithPresetPasswordSaveAsDialog(
      @NonNull final Context c, @NonNull final MainActivity main,
      final @NonNull String password, @NonNull final Intent intent) {

    HybridFileParcelable intentParcelable =
        intent.getParcelableExtra(EncryptService.TAG_SOURCE);
    MaterialDialog saveAsDialog = showNameDialog(
        main, "", intentParcelable.getName().concat(CryptUtil.CRYPT_EXTENSION),
        c.getString(intentParcelable.isDirectory()
                        ? R.string.encrypt_folder_save_as
                        : R.string.encrypt_file_save_as),
        c.getString(R.string.ok), null, c.getString(R.string.cancel),
        (dialog, which)
            -> {
          EditText textfield =
              dialog.getCustomView().findViewById(R.id.singleedittext_input);
          intent.putExtra(EncryptService.TAG_ENCRYPT_TARGET,
                          textfield.getText().toString());
          try {
            EncryptDecryptUtils.startEncryption(c, intentParcelable.getPath(),
                                                password, intent);
          } catch (GeneralSecurityException | IOException e) {
            e.printStackTrace();
            Toast
                .makeText(c, c.getString(R.string.crypt_encryption_fail),
                          Toast.LENGTH_LONG)
                .show();
          } finally {
            dialog.dismiss();
  ...
  public static void showEncryptAuthenticateDialog(
      final Context c, final Intent intent, final MainActivity main,
      final AppTheme appTheme,
      final EncryptDecryptUtils
          .EncryptButtonCallbackInterface encryptButtonCallbackInterface) {
    ...
    builder.customView(rootView, true)
        ...
        .onPositive((dialog, which) -> {
          intent.putExtra(EncryptService.TAG_ENCRYPT_TARGET,
                          encryptSaveAsEditText.getText().toString());
    ...
    new WarnableTextInputValidator(
        c, passwordEditText, textInputLayoutPassword, btnOK, (text) -> {
          if (text.length() < 1) {
            return new WarnableTextInputValidator.ReturnState(
                WarnableTextInputValidator.ReturnState.STATE_ERROR,
                R.string.field_empty);
    ...
    new WarnableTextInputValidator(
        c, passwordConfirmEditText, textInputLayoutPasswordConfirm, btnOK,
        (text) -> {
    ...
    new WarnableTextInputValidator(
        c, encryptSaveAsEditText, textInputLayoutEncryptSaveAs, btnOK,
        (text) -> {
          if (text.length() < 1) {
            return new WarnableTextInputValidator.ReturnState(
                WarnableTextInputValidator.ReturnState.STATE_ERROR,
  ...
  public static void showDecryptFingerprintDialog(
      final Context c, final MainActivity main, final Intent intent,
      final AppTheme appTheme,
      final EncryptDecryptUtils
    ...
    View rootView = View.inflate(
        c, R.layout.dialog_decrypt_fingerprint_authentication, null);

    Button cancelButton =
        rootView.findViewById(R.id.button_decrypt_fingerprint_cancel);
  ...
  public static void showDecryptDialog(
      final Context c, final MainActivity main, final Intent intent,
      final AppTheme appTheme, final String password,
      final EncryptDecryptUtils
          .DecryptButtonCallbackInterface decryptButtonCallbackInterface) {

    showPasswordDialog(c, main, appTheme, R.string.crypt_decrypt,
                       R.string.authenticate_password, ((dialog, which) -> {
                         EditText editText = dialog.getView().findViewById(
  ...
  public static void showPasswordDialog(
      final @NonNull Context c, @NonNull final MainActivity main,
      final @NonNull AppTheme appTheme, final @StringRes int titleText,
      final @StringRes int promptText,
      final @NonNull MaterialDialog.SingleButtonCallback positiveCallback,
      final @Nullable MaterialDialog.SingleButtonCallback negativeCallback) {
    int accentColor = main.getAccent();

    MaterialDialog.Builder builder = new MaterialDialog.Builder(c);
    View dialogLayout =
        View.inflate(main, R.layout.dialog_singleedittext, null);
  ...
  public static void showSMBHelpDialog(final Context m, final int accentColor) {
  ...
  public static void showPackageDialog(final File f, final MainActivity m) {
    ...
    mat.title(R.string.packageinstaller)
        .content(R.string.pitext)
  ...
  public static void showArchiveDialog(final File f, final MainActivity m) {
    ...
    mat.title(R.string.archive)
        ...
        .onNegative(
            (dialog, which) -> m.openCompressed(Uri.fromFile(f).toString()));
    if (m.getAppTheme().equals(AppTheme.DARK) ||
        m.getAppTheme().equals(AppTheme.BLACK))
  ...
  public static void showCompressDialog(final MainActivity m,
                                        final ArrayList<HybridFileParcelable> b,
    ...
    new WarnableTextInputValidator(
        a.getContext(), etFilename, tilFilename,
        materialDialog.getActionButton(DialogAction.POSITIVE), (text) -> {
  ...
  public static void showSortDialog(final MainFragment m,
                                    final AppTheme appTheme,
    ...
    a.items(sort).itemsCallbackSingleChoice(
        current > 3 ? current - 4 : current,
        (dialog, view, which, text) -> true);
    final Set<String> sortbyOnlyThis = sharedPref.getStringSet(
        PREFERENCE_SORTBY_ONLY_THIS, Collections.emptySet());
  ...
  private static void onSortTypeSelected(final MainFragment m,
                                         final SharedPreferences sharedPref,
                                         final Set<String> onlyThisFloders,
                                         final MaterialDialog dialog,
                                         final boolean desc) {
    final int sortType =
        desc ? dialog.getSelectedIndex() + 4 : dialog.getSelectedIndex();
    ...
    sharedPref.edit()
        .putStringSet(PREFERENCE_SORTBY_ONLY_THIS, onlyThisFloders)
        .apply();
    m.updateList();
    dialog.dismiss();
  }

  public static void showSortDialog(final AppsListFragment m,
                                    final AppTheme appTheme) {
    ...
    a.theme(appTheme.getMaterialDialogTheme());
    a.items(sort).itemsCallbackSingleChoice(
        current > 2 ? current - 3 : current,
        (dialog, view, which, text) -> true);
    a.negativeText(R.string.ascending).positiveColor(accentColor);
    a.positiveText(R.string.descending).negativeColor(accentColor);
    a.onNegative((dialog, which) -> {
      m.Sp.edit()
          .putString("sortbyApps", "" + dialog.getSelectedIndex())
          .commit();
      m.getSortModes();
      m.getLoaderManager().restartLoader(AppsListFragment.ID_LOADER_APP_LIST,
                                         null, m);
      dialog.dismiss();
    });
    ...
    a.onPositive((dialog, which) -> {
      m.Sp.edit()
          .putString("sortbyApps", "" + (dialog.getSelectedIndex() + 3))
          .commit();
      m.getSortModes();
      m.getLoaderManager().restartLoader(AppsListFragment.ID_LOADER_APP_LIST,
                                         null, m);
  ...
  public static void showHistoryDialog(final DataUtils dataUtils,
                                       final SharedPreferences sharedPrefs,
                                       final MainFragment m,
                                       final AppTheme appTheme) {
    int accentColor = m.getMainActivity().getAccent();
    final MaterialDialog.Builder a =
        new MaterialDialog.Builder(m.getActivity());
  ...
  public static void showHiddenDialog(final DataUtils dataUtils,
                                      final SharedPreferences sharedPrefs,
                                      final MainFragment m,
                                      final AppTheme appTheme) {
    int accentColor = m.getMainActivity().getAccent();
  ...
  public static void setPermissionsDialog(final View v, final View but,
                                          final HybridFile file, final String f,
    ...
    but.setOnClickListener(v1 -> {
      int perms = RootUtils.permissionsToOctalString(
          readown.isChecked(), writeown.isChecked(), exeown.isChecked(),
      ...
      try {
        RootUtils.changePermissions(
            file.getPath(), perms, file.isDirectory(context), isSuccess -> {
              if (isSuccess) {
                Toast
                    .makeText(context, mainFrag.getString(R.string.done),
                              Toast.LENGTH_LONG)
                    .show();
              } else {
                Toast
                    .makeText(context,
                              mainFrag.getString(R.string.operationunsuccesful),
                              Toast.LENGTH_LONG)
                    .show();
              }
            });
        ...
        Toast
            .makeText(context, mainFrag.getString(R.string.rootfailure),
                      Toast.LENGTH_LONG)
            .show();
        e.printStackTrace();
      }
  ...
  public static void showChangePathsDialog(final MainActivity mainActivity,
    ...
    a.input(null, mainActivity.getCurrentMainFragment().getCurrentPath(), false,
            (dialog, charSequence) -> {
              boolean isAccessible =
                  FileUtils.isPathAccessible(charSequence.toString(), prefs);
              dialog.getActionButton(DialogAction.POSITIVE)
                  .setEnabled(isAccessible);
            });
    ...
    a.onPositive((dialog, which) -> {
      mainActivity.getCurrentMainFragment().loadlist(
          dialog.getInputEditText().getText().toString(), false,
          OpenMode.UNKNOWN);
    });
  ...
  public static MaterialDialog
  showOtgSafExplanationDialog(final ThemedActivity themedActivity) {
    return GeneralDialogCreation.showBasicDialog(
        themedActivity, R.string.saf_otg_explanation, R.string.otg_access,
