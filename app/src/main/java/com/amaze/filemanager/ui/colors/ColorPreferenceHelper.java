...
public class ColorPreferenceHelper {
  ...
  public static final
      @ColorRes int DEFAULT_PRIMARY_FIRST_TAB = R.color.primary_indigo,
  ...
  private static final @ColorRes int[][] RANDOM_COMBINATIONS = new int[][] {
      ...
      {R.color.primary_deep_purple, R.color.primary_pink,
       R.color.primary_deep_purple},
      {R.color.primary_blue_grey, R.color.primary_brown,
       R.color.primary_blue_grey},
      {R.color.primary_pink, R.color.primary_orange, R.color.primary_pink},
      {R.color.primary_blue_grey, R.color.primary_red,
       R.color.primary_blue_grey},
      {R.color.primary_red, R.color.primary_orange, R.color.primary_red},
      {R.color.primary_light_blue, R.color.primary_pink,
  ...
  public static UserColorPreferences randomize(final Context c) {
    @ColorRes
    int[] colorPos =
        RANDOM_COMBINATIONS[new Random().nextInt(RANDOM_COMBINATIONS.length)];

    return new UserColorPreferences(
        Utils.getColor(c, colorPos[0]), Utils.getColor(c, colorPos[0]),
        Utils.getColor(c, colorPos[1]), Utils.getColor(c, colorPos[2]));
  }

  /**
   * Eases the retrieval of primary colors ColorUsage.
   * If the index is out of bounds, the first primary color is returned as
   * default.
   *
  ...
  int getPrimary(final UserColorPreferences currentColors, final int num) {
    return num == 1 ? currentColors.primarySecondTab
                    : currentColors.primaryFirstTab;
  }

  private UserColorPreferences currentColors;

  public UserColorPreferences
  getCurrentUserColorPreferences(final Context context,
  ...
  public void saveColorPreferences(final SharedPreferences prefs,
                                   final UserColorPreferences userPrefs) {
    SharedPreferences.Editor editor = prefs.edit();
    editor.putInt(PreferencesConstants.PREFERENCE_SKIN,
                  userPrefs.primaryFirstTab);
    editor.putInt(PreferencesConstants.PREFERENCE_SKIN_TWO,
                  userPrefs.primarySecondTab);
    editor.putInt(PreferencesConstants.PREFERENCE_ACCENT, userPrefs.accent);
    editor.putInt(PreferencesConstants.PREFERENCE_ICON_SKIN,
                  userPrefs.iconSkin);
    editor.apply();
  ...
  private UserColorPreferences
  getColorPreferences(final Context c, final SharedPreferences prefs) {
    if (isUsingOldColorsSystem(prefs))
      correctToNewColorsSystem(c, prefs);

    int tabOne = prefs.getInt(PreferencesConstants.PREFERENCE_SKIN,
                              Utils.getColor(c, DEFAULT_PRIMARY_FIRST_TAB));
    int tabTwo = prefs.getInt(PreferencesConstants.PREFERENCE_SKIN_TWO,
                              Utils.getColor(c, DEFAULT_PRIMARY_SECOND_TAB));
    int accent = prefs.getInt(PreferencesConstants.PREFERENCE_ACCENT,
                              Utils.getColor(c, DEFAULT_ACCENT));
    int iconSkin = prefs.getInt(PreferencesConstants.PREFERENCE_ICON_SKIN,
                                Utils.getColor(c, DEFAULT_ICON_SKIN));

    return new UserColorPreferences(tabOne, tabTwo, accent, iconSkin);
  }

  /**
   * The old system used indexes, from here on in this file a correction is made
   * so that the indexes are converted into ColorInts
   */
  ...
  private static final List<Integer> OLD_SYSTEM_LIST = Arrays.asList(
      R.color.primary_red, R.color.primary_pink, R.color.primary_purple,
      R.color.primary_deep_purple, R.color.primary_indigo, R.color.primary_blue,
      R.color.primary_light_blue, R.color.primary_cyan, R.color.primary_teal,
      R.color.primary_green, R.color.primary_light_green, R.color.primary_amber,
      R.color.primary_orange, R.color.primary_deep_orange,
      R.color.primary_brown, R.color.primary_grey_900,
      R.color.primary_blue_grey, R.color.primary_teal_900, R.color.accent_pink,
      R.color.accent_amber, R.color.accent_light_blue,
      R.color.accent_light_green);

  private void correctToNewColorsSystem(final Context c,
                                        final SharedPreferences prefs) {
    ...
    SharedPreferences.Editor editor = prefs.edit();
    editor.putInt(PreferencesConstants.PREFERENCE_SKIN,
                  correctForIndex(c, tabOne));
    editor.putInt(PreferencesConstants.PREFERENCE_SKIN_TWO,
                  correctForIndex(c, tabTwo));
    editor.putInt(PreferencesConstants.PREFERENCE_ACCENT,
                  correctForIndex(c, accent));
    editor.putInt(PreferencesConstants.PREFERENCE_ICON_SKIN,
                  correctForIndex(c, iconSkin));
    editor.apply();
  }

  private @ColorInt int correctForIndex(final Context c, final int color) {
    if (color != -1)
      return Utils.getColor(c, OLD_SYSTEM_LIST.get(color));
    else
      return Utils.getColor(c, R.color.primary_indigo);
  }
}
