...
public class Bzip2Extractor extends Extractor {
  ...
  public Bzip2Extractor(final @NonNull Context context,
  ...
  protected void extractWithFilter(final @NonNull Filter filter)
    ...
    while ((tarArchiveEntry = inputStream.getNextTarEntry()) != null) {
      if (filter.shouldExtract(tarArchiveEntry.getName(),
  ...
  private void extractEntry(@NonNull final Context context,
                            final TarArchiveInputStream inputStream,
                            final TarArchiveEntry entry, final String outputDir)
      throws IOException {
