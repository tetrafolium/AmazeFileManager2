...
/**
 * Wrapper for manipulating files via the Android Media Content Provider. As of
 * Android 4.4 KitKat, applications can no longer write to the "secondary
 * storage" of a device. Write operations using the java.io.File API will thus
 * fail. This class restores access to those write operations by way of the
 * Media Content Provider.</p>
 *
 * Note that this class relies on the internal operational characteristics of
 * the media content provider API, and as such is not guaranteed to be
 * future-proof. Then again, we did all think the java.io.File API was going to
 * be future-proof for media card access, so all bets are off.</p>
 *
 * If you're forced to use this class, it's because Google/AOSP made a very poor
 * API decision in Android 4.4 KitKat. Read more at
 * https://plus.google.com/+TodLiebeck/posts/gjnmuaDM8sn</p>
 *
 * Your application must declare the permission
 * "android.permission.WRITE_EXTERNAL_STORAGE".</p>
 *
 * Adapted from:
 * http://forum.xda-developers.com/showpost.php?p=52151865&postcount=20</p>
 *
...
/**
 * Deletes the file. Returns true if the file has been successfully deleted or
 * otherwise does not exist. This operation is not recursive.
 */
public static boolean delete(final Context context, final File file) {
	...
	// Delete the entry from the media database. This will actually delete media
	// files.
...
public static OutputStream getOutputStream(final Context context,
                                           final String str) {
...
public static Uri getUriFromFile(final String path, final Context context) {
	...
	if (filecursor.isAfterLast()) {
		...
		return resolver.insert(MediaStore.Files.getContentUri("external"),
		                       values);
	} else {
		int imageId =
...
private static File installTemporaryTrack(final Context context)
throws IOException {
...
throws IOException {
	...
	final Uri albumUri =
		Uri.parse(String.format(Locale.US, ALBUM_ART_URI + "/%d", albumId));
	final ContentValues values = new ContentValues();
