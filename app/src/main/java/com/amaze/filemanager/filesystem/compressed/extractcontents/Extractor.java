...
public Extractor(final @NonNull Context context,
...
public void extractFiles(final String[] files) throws IOException {
	...
	extractWithFilter((relativePath, isDir)->{
			if (filesToExtract.contains(relativePath)) {
			        ...
			        return true;
			} else { // header to be extracted is at least the entry path (may be
			         // more, when it is a directory)
...
protected abstract void extractWithFilter(@NonNull Filter filter)
throws IOException;
...
protected String fixEntryName(final String entryName) {
	if (entryName.indexOf('\\') >= 0) {
		return fixEntryName(entryName.replaceAll("\\\\", SEPARATOR));
	} else if (entryName.indexOf(SEPARATOR_CHAR) == 0) {
		// if entryName starts with "/" (e.g. "/test.txt"), strip the prefixing
		// "/"s
