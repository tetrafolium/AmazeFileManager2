...
public abstract class SshClientUtils {
   ...
   *
   * This template pattern is borrowed from Spring Framework, to simplify code
   * on operations using SftpClientTemplate.
   *
  ...
  public static final <T>
      T execute(@NonNull final SshClientSessionTemplate template) {
   ...
   */
  public static final <T>
      T execute(@NonNull final SFtpClientTemplate template) {
    return execute(new SshClientTemplate(template.url, false) {
      @Override
      public T execute(final SSHClient client) {
  ...
  /**
   * Convenience method to call {@link SmbUtil#getSmbEncryptedPath(Context,
   * String)} if the given SSH URL contains the password (assuming the password
   * is encrypted).
   *
  ...
  encryptSshPathAsNecessary(final @NonNull String fullUri) {
  ...
  /**
   * Convenience method to call {@link SmbUtil#getSmbDecryptedPath(Context,
   * String)} if the given SSH URL contains the password (assuming the password
   * is encrypted).
   *
   * @param fullUri SSH URL
   ...
   */
  public static final String
  decryptSshPathAsNecessary(final @NonNull String fullUri) {
    String uriWithoutProtocol =
        fullUri.substring(SSH_URI_PREFIX.length(), fullUri.lastIndexOf('@'));
    try {
      return (uriWithoutProtocol.lastIndexOf(':') > 0)
   ...
   *
   * For example, given
   * <code>ssh://user:password@127.0.0.1:22/home/user/foo/bar</code>, this
  ...
  public static final String extractBaseUriFrom(final @NonNull String fullUri) {
    ...
    return uriWithoutProtocol.indexOf('/') == -1
        ? fullUri
        : fullUri.substring(0, uriWithoutProtocol.indexOf('/') +
                                   SSH_URI_PREFIX.length());
  }
   ...
   * @return The remote path part of the full SSH URL
   */
  public static final String extractRemotePathFrom(final
                                                   @NonNull String fullUri) {
    String uriWithoutProtocol = fullUri.substring(SSH_URI_PREFIX.length());
    return uriWithoutProtocol.indexOf('/') == -1
        ? "/"
  ...
  /**
   * Disconnects the given {@link SSHClient} but wrap all exceptions beneath, so
   * callers are free from the hassles of handling thrown exceptions.
   *
  ...
  public static void launchSftp(final HybridFileParcelable baseFile,
    ...
    new Thread(() -> {
      try {
        ...
        activity.runOnUiThread(() -> {
          try {
            File file = new File(
                SshClientUtils.extractRemotePathFrom(baseFile.getPath()));
  ...
  public static String deriveSftpPathFrom(final String hostname, final int port,
    ...
    return (selectedParsedKeyPair != null || password == null)
        ...
        : String.format("ssh://%s:%s@%s:%d", username, password, hostname,
                        port);
  }
}
