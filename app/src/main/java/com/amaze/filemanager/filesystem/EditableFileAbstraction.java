...
/**
 * This is a special representation of a file that is to be used so that uris
 * can be loaded as editable files.
 *
  ...
  public EditableFileAbstraction(final Context context, final Uri uri) {
    ...
    case "content":
      ...
      if (c != null) {
        ...
        try {
          /*
           The result and whether [Cursor.getString()] throws an exception when
           the column value is null or the column type is not a string type is
    ...
    case "file":
      ...
      if (path == null)
        throw new NullPointerException("Uri '" + uri.toString() +
                                       "' is not hierarchical!");
      ...
      break;
    default:
      throw new IllegalArgumentException("The scheme '" + uri.getScheme() +
                                         "' cannot be processed!");
    }
  }
}
