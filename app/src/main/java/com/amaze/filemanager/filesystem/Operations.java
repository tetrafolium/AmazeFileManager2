...
/**
 * Created by arpitkh996 on 13-01-2016, modified by Emmanuel
...
/**
 * Callback fired when creating new file/directory and required storage
 * access framework permission to access SD Card is not available
 */
void launchSAF(HybridFile file);

/**
 * Callback fired when renaming file and required storage access framework
...
public static void mkdir(@NonNull final HybridFile file,
                         final Context context, final boolean rootMode,
	...
	new AsyncTask<Void, Void, Void>() {
		...
		protected Void doInBackground(final Void... params) {
			...
			if (file.isSmb()) {
				...
				try {
					cloudStorageBox.createFolder(
						CloudUtil.stripPath(OpenMode.BOX, file.getPath()));
					errorCallBack.done(file, true);
				} catch (Exception e) {
					e.printStackTrace();
					errorCallBack.done(file, false);
...
public static void mkfile(@NonNull final HybridFile file,
                          final Context context, final boolean rootMode,
                          @NonNull final ErrorCallBack errorCallBack) {
			...
			if (file.isSmb()) {
				...
				try {
					...
					ByteArrayInputStream byteArrayInputStream =
						new ByteArrayInputStream(tempBytes);
				...
				DocumentFile parentDirectory =
					OTGUtil.getDocumentFile(file.getParent(), context, false);
				if (parentDirectory.isDirectory()) {
					parentDirectory.createFile(file.getName(context).substring(
...
public static void rename(final HybridFile oldFile, final HybridFile newFile,
                          final boolean rootMode, final Context context,
	...
	new AsyncTask<Void, Void, Void>() {
		...
		protected Void doInBackground(final Void... params) {
			...
			if (oldFile.isSmb()) {
				...
				errorCallBack.done(
					newFile, oldDocumentFile.renameTo(newFile.getName(context)));
				return null;
			} else {
...
isCopyLoopPossible(final HybridFileParcelable sourceFile,
...
/**
 * Validates file name
 * special reserved characters shall not be allowed in the file names on FAT
 * filesystems
 *
