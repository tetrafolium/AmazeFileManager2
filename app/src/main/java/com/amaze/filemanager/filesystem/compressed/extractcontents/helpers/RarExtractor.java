...
public RarExtractor(final @NonNull Context context,
	...
	try {
		...
		for (FileHeader header : rarFile.getFileHeaders()) {
			if (CompressedHelper.isEntryPathValid(header.getFileNameString())) {
				if (filter.shouldExtract(header.getFileNameString(),
				                         header.isDirectory())) {
					// header to be extracted is at least the entry path (may be more,
...
private void extractEntry(@NonNull final Context context,
                          final Archive zipFile, final FileHeader entry,
