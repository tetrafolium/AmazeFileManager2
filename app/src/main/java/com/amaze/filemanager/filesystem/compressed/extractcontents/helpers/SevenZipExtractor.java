 ...
 * Emmanuel Messulam<emmanuelbendavid@gmail.com>, Raymond Lai
 * <airwave209gt@gmail.com> and Contributors.
 *
...
public SevenZipExtractor(final @NonNull Context context,
...
throws IOException {
	...
	for (SevenZArchiveEntry entry : sevenzFile.getEntries()) {
		if (filter.shouldExtract(entry.getName(), entry.isDirectory())) {
			// Entry to be extracted is at least the entry path (may be more, when
			// it is a directory)
			arrayList.add(entry);
