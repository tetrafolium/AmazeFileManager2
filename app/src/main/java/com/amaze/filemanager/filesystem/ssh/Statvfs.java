 ...
 *
 * This is not specified in official SSH protocol; it is implemented by OpenSSH
 * (which is the most used SSH server implementation around.
 */
public class Statvfs {
  private static final String STATVFS_OPENSSH_COM = "statvfs@openssh.com";

  /**
   * Convenience method for creating <code>statvfs@openssh.com</code> {@link
   * Request}s.
   *
   ...
   * @return {@link Request}
   */
  public static final Request request(final SFTPClient sftpClient,
                                      final String path) {
    return sftpClient.getSFTPEngine()
        .newRequest(PacketType.EXTENDED)
        .putString(STATVFS_OPENSSH_COM)
        .putString(
            path, sftpClient.getSFTPEngine().getSubsystem().getRemoteCharset());
  }
  ...
  public static class Response {
    ...
    public Response(final String remotePath,
                    final net.schmizz.sshj.sftp.Response response)
        throws SFTPException, Buffer.BufferException {
      response.ensurePacketTypeIs(PacketType.EXTENDED_REPLY);

      if (!response.readStatusCode().equals(
              net.schmizz.sshj.sftp.Response.StatusCode.OK)) {
        throw new SFTPException("Bad response code: " +
     ...
     *
     * Depending on the target SSH server implementation, the result may be the
     * disk size of the physical disk where the queried path resides at.
     *
    ...
    /**
     * Return disk free space (filesystem block size * available filesystem
     * blocks).
     *
     * Depending on the target SSH server implementation, the result may be the
     * disk free space of the physical disk where the queried path resides at.
     *
     ...
     * @return fileSystemId
     */
    public long getRawFileSystemId() { return fileSystemId; }

    /**
     * Read an uint64 from the buffer. This workarounds sshj's original method
     * to prevent exception thrown which calls for {@link BigInteger} to store
     * negative {@link Long}s.
     *
     * Many thanks to Alexander--@github for the comments.
     *
     * @see Buffer#readUInt64()
     */
    private long readUInt64FromBuffer(final Buffer buffer)
        throws Buffer.BufferException {
      long uint64 =
          (buffer.readUInt32() << 32) + (buffer.readUInt32() & 0xffffffffL);
      return uint64;
    }
