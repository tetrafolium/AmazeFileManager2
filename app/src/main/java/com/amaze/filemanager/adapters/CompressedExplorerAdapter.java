...
/**
 * Created by Arpit on 25-01-2015 edited by Emmanuel
 * Messulam<emmanuelbendavid@gmail.com>
 */
public class CompressedExplorerAdapter
	extends RecyclerView.Adapter<CompressedItemViewHolder> {
...
public CompressedExplorerAdapter(
	final Context c, final UtilitiesProvider utilsProvider,
 ...
 * @param imageView the circular {@link CircleGradientDrawable} that is to be
 *     animated
 */
private void toggleChecked(final int position, final ImageView imageView) {
	...
	if (!compressedExplorerFragment.selection ||
		...
		/*compressedExplorerFragment.mActionMode =
		   compressedExplorerFragment.getActivity().startActionMode(
		       compressedExplorerFragment.mActionModeCallback);*/
		compressedExplorerFragment.mActionMode =
			compressedExplorerFragment.mainActivity.getAppbar()
			.getToolbar()
...
private void animate(final CompressedItemViewHolder holder) {
	...
	Animation localAnimation = AnimationUtils.loadAnimation(
		compressedExplorerFragment.getActivity(), R.anim.fade_in_top);
...
@Override
public CompressedItemViewHolder onCreateViewHolder(final ViewGroup parent,
                                                   final int viewType) {
...
@Override
public void onBindViewHolder(final CompressedItemViewHolder holder,
                             final int position) {
	if (!stoppedAnimation) {
		animate(holder);
	}
	...
	if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
		holder.checkImageView.setBackground(new CircleGradientDrawable(
							    compressedExplorerFragment.accentColor, utilsProvider.getAppTheme(),
							    compressedExplorerFragment.getResources().getDisplayMetrics()));
	} else
		holder.checkImageView.setBackgroundDrawable(new CircleGradientDrawable(
								    compressedExplorerFragment.accentColor, utilsProvider.getAppTheme(),
								    compressedExplorerFragment.getResources().getDisplayMetrics()));
	...
	if (rowItem.type == CompressedObjectParcelable.TYPE_GOBACK) {
		...
		if (rowItem.directory) {
			...
			if (compressedExplorerFragment.showSize)
				holder.txtDesc.setText(
					Formatter.formatFileSize(context, rowItem.size));
			holder.txtTitle.setText(
				rowItem.path.substring(rowItem.path.lastIndexOf("/") + 1));
	...
	holder.rl.setOnClickListener(p1->{
		    ...
		    : rowItem.path;
			                        ...
			                        String fileName = CompressedHelper.getFileName(
							compressedExplorerFragment.compressedFile.getName());
			                        String archiveCacheDirPath =
							compressedExplorerFragment.getActivity()
							.getExternalCacheDir()
							.getPath() +
							CompressedHelper.SEPARATOR + fileName;

			                        HybridFileParcelable file = new HybridFileParcelable(
							archiveCacheDirPath + CompressedHelper.SEPARATOR +
							rowItem.path.replaceAll("\\\\", CompressedHelper.SEPARATOR));
			                        ...
			                        Toast
			                        .makeText(compressedExplorerFragment.getContext(),
			                                  compressedExplorerFragment.getContext().getString(
								  R.string.please_wait),
			                                  Toast.LENGTH_SHORT)
			                        .show();
			                        decompressor.decompress(compressedExplorerFragment.getActivity()
			                                                .getExternalCacheDir()
			                                                .getPath(),
