...
public class LayoutElementParcelable implements Parcelable {
  ...
  public LayoutElementParcelable(final @NonNull Context c, final boolean isBack,
                                 final String goback,
                                 final boolean showThumbs) {
    this(c, true, new File("..").getName(), "..", "", "", goback, 0, false, "",
         true, showThumbs, OpenMode.UNKNOWN);
  }

  public LayoutElementParcelable(final @NonNull Context c, final String path,
                                 final String permissions, final String symlink,
                                 final String size, final long longSize,
                                 final boolean header, final String date,
                                 final boolean isDirectory,
                                 final boolean useThumbs,
                                 final OpenMode openMode) {
    this(c, new File(path).getName(), path, permissions, symlink, size,
         longSize, header, date, isDirectory, useThumbs, openMode);
  }

  public LayoutElementParcelable(final @NonNull Context c, final String title,
                                 final String path, final String permissions,
                                 final String symlink, final String size,
                                 final long longSize, final boolean header,
                                 final String date, final boolean isDirectory,
                                 final boolean useThumbs,
                                 final OpenMode openMode) {
    this(c, false, title, path, permissions, symlink, size, longSize, header,
  ...
  public LayoutElementParcelable(final @NonNull Context c, final boolean isBack,
    ...
    if (useThumbs) {
      ...
      case BOX:
        if (!isDirectory &&
            (filetype == Icons.IMAGE || filetype == Icons.VIDEO ||
             filetype == Icons.APK)) {
          this.iconData = new IconDataParcelable(
              IconDataParcelable.IMAGE_FROMCLOUD, path, fallbackIcon);
        } else {
        ...
        break;
      default:
        if (filetype == Icons.IMAGE || filetype == Icons.VIDEO ||
            filetype == Icons.APK) {
          this.iconData = new IconDataParcelable(
              IconDataParcelable.IMAGE_FROMFILE, path, fallbackIcon);
        } else {
          this.iconData = new IconDataParcelable(IconDataParcelable.IMAGE_RES,
                                                 fallbackIcon);
        }
      ...
      this.iconData =
          new IconDataParcelable(IconDataParcelable.IMAGE_RES, fallbackIcon);
    }
  ...
  public HybridFileParcelable generateBaseFile() {
    HybridFileParcelable baseFile = new HybridFileParcelable(
        desc, permissions, date, longSize, isDirectory);
