...
public AppsAdapter(final Context context, final ThemedActivity ba,
                   final UtilitiesProvider utilsProvider,
                   final AppsAdapterPreloadModel modelProvider,
                   final ViewPreloadSizeProvider<String> sizeProvider,
                   final int resourceId, final AppsListFragment app,
...
public View getView(final int position, final View convertView,
	...
	holder.rl.setOnClickListener(p1->{
			Intent i1 =
				app.getActivity().getPackageManager().getLaunchIntentForPackage(
					rowItem.packageName);
			if (i1 != null)
				app.startActivity(i1);
			else
				Toast
				.makeText(app.getActivity(), app.getString(R.string.not_allowed),
				          Toast.LENGTH_LONG)
				.show();
	...
	if (myChecked.get(position)) {
		holder.rl.setBackgroundColor(
			Utils.getColor(context, R.color.appsadapter_background));
	} else {
		if (utilsProvider.getAppTheme().equals(AppTheme.LIGHT)) {
	...
	v.setOnClickListener(view->{
			...
			popupMenu.setOnMenuItemClickListener(item->{
				...
				case R.id.share:
					...
					FileUtils.shareFiles(arrayList2, app.getActivity(),
					                     utilsProvider.getAppTheme(), colorAccent);
					return true;
				case R.id.unins:
					final HybridFileParcelable f1 =
						new HybridFileParcelable(rowItem.path);
					f1.setMode(OpenMode.ROOT);

					if ((Integer.valueOf(
						     rowItem.data.substring(0, rowItem.data.indexOf("_"))) &
					     ApplicationInfo.FLAG_SYSTEM) != 0) {
					        // system package
					        if (app.Sp.getBoolean(PreferencesConstants.PREFERENCE_ROOTMODE,
					                              false)) {
				...
				case R.id.play:
					...
					try {
					        intent1.setData(Uri.parse(
									String.format("market://details?id=%s", rowItem.packageName)));
					        app.startActivity(intent1);
					} catch (ActivityNotFoundException ifPlayStoreNotInstalled) {
					        intent1.setData(Uri.parse(String.format(
				...
				case R.id.backup:
					...
					baseFile.setName(rowItem.label + "_" +
					                 rowItem.packageName.substring(
								 rowItem.packageName.indexOf("_") + 1) +
					                 ".apk");
					ab.add(baseFile);
