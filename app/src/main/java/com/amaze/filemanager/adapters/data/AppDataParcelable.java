...
public final class AppDataParcelable implements Parcelable {
  ...
  public AppDataParcelable(final String label, final String path,
                           final String packageName, final String data,
                           final String fileSize, final long size,
  ...
  public static final class AppDataSorter
      implements Comparator<AppDataParcelable> {
     ...
     */
    @Override
    public int compare(final AppDataParcelable file1,
                       final AppDataParcelable file2) {
      if (sort == SORT_NAME) {
        // sort by name
        return asc * file1.label.compareToIgnoreCase(file2.label);
      } else if (sort == SORT_MODIF) {
        // sort by last modified
        return asc * Long.valueOf(file1.lastModification)
                         .compareTo(file2.lastModification);
