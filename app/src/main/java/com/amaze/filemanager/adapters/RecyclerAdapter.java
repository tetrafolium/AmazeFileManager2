 ...
 *
 * Copyright © 2015-2018 Arpit Khurana <arpitkh96@gmail.com>, Vishal Nehra
...
/**
 * This class is the information that serves to load the files into a "list" (a
 * RecyclerView). There are 3 types of item TYPE_ITEM, TYPE_HEADER_FOLDERS and
 * TYPE_HEADER_FILES, EMPTY_LAST_ITEM and TYPE_BACK represeted by
 * ItemViewHolder, SpecialViewHolder and EmptyViewHolder respectively. The
 * showPopup shows the file's popup menu. The 'go to parent' aka '..' button (go
 * to settings to activate it) is just a folder.
 *
 * Created by Arpit on 11-04-2015 edited by Emmanuel Messulam
 * <emmanuelbendavid@gmail.com> edited by Jens Klingenberg
...
public class RecyclerAdapter
  ...
  public static final int TYPE_ITEM = 0, TYPE_HEADER_FOLDERS = 1,
                          TYPE_HEADER_FILES = 2, EMPTY_LAST_ITEM = 3,
                          TYPE_BACK = 4;

  @IntDef({TYPE_ITEM, TYPE_HEADER_FOLDERS, TYPE_HEADER_FILES, EMPTY_LAST_ITEM,
           TYPE_BACK})
  public @interface ListElemType {}

  private static final int VIEW_GENERIC = 0, VIEW_PICTURE = 1, VIEW_APK = 2,
                           VIEW_THUMB = 3;
  ...
  private int grey_color, accentColor, iconSkinColor, goBackColor, videoColor,
      audioColor, pdfColor, codeColor, textColor, archiveColor, genericColor,
      apkColor;
  private int offset = 0;

  public RecyclerAdapter(final PreferenceActivity preferenceActivity,
                         final MainFragment m,
                         final UtilitiesProvider utilsProvider,
                         final SharedPreferences sharedPrefs,
                         final RecyclerView recyclerView,
                         final ArrayList<LayoutElementParcelable> itemsRaw,
    ...
    minRowHeight =
        context.getResources().getDimension(R.dimen.minimal_row_height);
   ...
   * @param imageView the check {@link CircleGradientDrawable} that is to be
   *     animated
   */
  public void toggleChecked(final int position, final ImageView imageView) {
    ...
    if (itemsDigested.get(position).getChecked() == ListItem.CHECKED) {
      ...
      Animation iconAnimation =
          AnimationUtils.loadAnimation(context, R.anim.check_out);
      if (imageView != null) {
        imageView.startAnimation(iconAnimation);
      } else {
        // TODO: we don't have the check icon object probably because of config
        // change
      }
    } else {
      ...
      Animation iconAnimation =
          AnimationUtils.loadAnimation(context, R.anim.check_in);
      if (imageView != null) {
        imageView.startAnimation(iconAnimation);
      } else {
        // TODO: we don't have the check icon object probably because of config
      ...
      if (mainFrag.mActionMode == null || !mainFrag.selection) {
        ...
        mainFrag.mActionMode =
            mainFrag.getMainActivity().startSupportActionMode(
                mainFrag.mActionModeCallback);
      }
    }
  ...
  public void toggleChecked(final boolean b, final String path) {
    int i =
        path.equals("/") || !getBoolean(PREFERENCE_SHOW_GOBACK_BUTTON) ? 0 : 1;
  ...
  public boolean areAllChecked(final String path) {
    ...
    int i = (path.equals("/") || !getBoolean(PREFERENCE_SHOW_GOBACK_BUTTON))
                ? 0
                : 1;
  ...
  private void animate(final ItemViewHolder holder) {
    ...
    Animation localAnimation =
        AnimationUtils.loadAnimation(context, R.anim.fade_in_top);
  ...
  /**
   * Adds item to the end of the list, don't use this unless you are dynamically
   * loading the adapter, after you are finished you must call createHeaders
   */
  ...
  public void setItems(final RecyclerView recyclerView,
                       final ArrayList<LayoutElementParcelable> arrayList) {
    setItems(recyclerView, arrayList, true);
  }

  private void setItems(final RecyclerView recyclerView,
                        final ArrayList<LayoutElementParcelable> arrayList,
    ...
    preloader = new RecyclerViewPreloader<>(GlideApp.with(mainFrag),
                                            modelProvider, sizeProvider,
                                            GlideConstants.MAX_PRELOAD_FILES);

    recyclerView.addOnScrollListener(preloader);
  }

  public void createHeaders(final boolean invalidate,
                            final List<IconDataParcelable> uris) {
  ...
  public RecyclerView.ViewHolder onCreateViewHolder(final ViewGroup parent,
    ...
    case TYPE_HEADER_FILES:
      ...
      int type = viewType == TYPE_HEADER_FOLDERS
    ...
    case TYPE_BACK:
      if (mainFrag.IS_LIST) {
        ...
        sizeProvider.addView(VIEW_GENERIC,
                             view.findViewById(R.id.generic_icon));
        sizeProvider.addView(VIEW_PICTURE,
                             view.findViewById(R.id.picture_icon));
    ...
    case EMPTY_LAST_ITEM:
      int totalFabHeight =
              (int)context.getResources().getDimension(R.dimen.fab_height),
          marginFab =
              (int)context.getResources().getDimension(R.dimen.fab_margin);
  ...
  public void onBindViewHolder(final RecyclerView.ViewHolder vholder,
    ...
    if (vholder instanceof ItemViewHolder) {
      ...
      boolean enableMarquee = sharedPrefs.getBoolean(
          PreferencesConstants.PREFERENCE_ENABLE_MARQUEE_FILENAME, true);
      holder.txtTitle.setEllipsize(enableMarquee ? TextUtils.TruncateAt.MARQUEE
                                                 : TextUtils.TruncateAt.MIDDLE);

      final boolean isBackButton =
          itemsDigested.get(p).specialType == TYPE_BACK;
      if (isBackButton) {
      ...
      if (mainFrag.IS_LIST) {
        ...
        holder.rl.setOnClickListener(v -> {
          mainFrag.onListItemClicked(isBackButton, vholder.getAdapterPosition(),
                                     rowItem, holder.checkImageView);
        });

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
          holder.checkImageView.setBackground(new CircleGradientDrawable(
              accentColor, utilsProvider.getAppTheme(),
        ...
        holder.rl.setOnLongClickListener(p1 -> {
          // check if the item on which action is performed is not the first
          // {goback} item
          if (!isBackButton) {
        ...
        holder.genericIcon.setOnClickListener(v -> {
          ...
          if (id == R.id.generic_icon || id == R.id.picture_icon ||
              id == R.id.apk_icon) {
            // TODO: transform icon on press to the properties dialog with
            // animation
            if (!isBackButton) {
              toggleChecked(vholder.getAdapterPosition(),
        ...
        case Icons.VIDEO:
          if (getBoolean(PREFERENCE_SHOW_THUMB)) {
            if (getBoolean(PREFERENCE_USE_CIRCULAR_IMAGES)) {
              showThumbnailWithBackground(holder, rowItem.iconData,
                                          holder.pictureIcon,
            ...
            holder.genericIcon.setImageResource(
                rowItem.filetype == Icons.IMAGE ? R.drawable.ic_doc_image
        ...
        case Icons.NOT_KNOWN:
          ...
          String ext = !rowItem.isDirectory
                           ? MimeTypes.getExtension(rowItem.title)
                           : null;
          if (ext != null && ext.trim().length() != 0) {
            ...
            modelProvider.getPreloadRequestBuilder(rowItem.iconData)
        ...
        if (itemsDigested.get(p).getChecked() == ListItem.CHECKED) {
          ...
          // making sure the generic icon background color filter doesn't get
          // changed to grey on picture/video/apk/generic text icons when
          // checked so that user can still look at the thumbs even after
          // selection
        ...
        holder.rl.setOnClickListener(v -> {
          mainFrag.onListItemClicked(isBackButton, vholder.getAdapterPosition(),
                                     rowItem, holder.checkImageViewGrid);
        });
        ...
        if (rowItem.filetype == Icons.IMAGE ||
          ...
          if (getBoolean(PREFERENCE_SHOW_THUMB)) {
            ...
            showRoundedThumbnail(holder, rowItem.iconData, holder.imageView1,
                                 rowItem.iconData::setImageBroken);
          } else {
            if (rowItem.filetype == Icons.IMAGE)
          ...
          if (getBoolean(PREFERENCE_SHOW_THUMB))
            showRoundedThumbnail(holder, rowItem.iconData, holder.genericIcon,
                                 rowItem.iconData::setImageBroken);
          else {
            holder.genericIcon.setImageResource(R.drawable.ic_doc_apk_white);
          }
        ...
        if (itemsDigested.get(p).getChecked() == ListItem.CHECKED) {
          ...
          if (utilsProvider.getAppTheme().equals(AppTheme.LIGHT))
            ...
            holder.rl.setBackgroundResource(
                R.drawable.ic_grid_card_background_dark);
            holder.rl.findViewById(R.id.icon_frame_grid)
                .setBackgroundColor(
                    Utils.getColor(context, R.color.icon_background_dark));
          }
        }
  ...
  @Override
  public int getCorrectView(final IconDataParcelable item,
  ...
  private void showThumbnailWithBackground(
      final ItemViewHolder viewHolder, final IconDataParcelable iconData,
    ...
    if (iconData.isImageBroken()) {
      ...
      GradientDrawable gradientDrawable =
          (GradientDrawable)viewHolder.genericIcon.getBackground();
    ...
    RequestListener<Drawable> requestListener =
        new RequestListener<Drawable>() {
          @Override
          public boolean onLoadFailed(final @Nullable GlideException e,
                                      final Object model, final Target target,
          ...
          public boolean onResourceReady(
              final Drawable resource, final Object model,
    ...
    modelProvider.getPreloadRequestBuilder(iconData)
        .listener(requestListener)
        .into(view);
  }

  private void showRoundedThumbnail(final ItemViewHolder viewHolder,
                                    final IconDataParcelable iconData,
                                    final ImageView view,
                                    final OnImageProcessed errorListener) {
    if (iconData.isImageBroken()) {
      View iconBackground = getBoolean(PREFERENCE_USE_CIRCULAR_IMAGES)
  ...
  private void showPopup(final View v, final LayoutElementParcelable rowItem) {
    v.setOnClickListener(view -> {
      PopupMenu popupMenu =
          new ItemPopupMenu(context, mainFrag.getMainActivity(), utilsProvider,
                            mainFrag, rowItem, view, sharedPrefs);
  ...
  private static class ListItem {
    ...
    ListItem(final LayoutElementParcelable elem) { this(false, elem); }
    ...
    public boolean getAnimating() { return animate; }
  }
