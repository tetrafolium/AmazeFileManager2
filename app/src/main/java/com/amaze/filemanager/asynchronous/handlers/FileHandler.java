...
public FileHandler(final MainFragment mainFragment,
                   final RecyclerView listView, final boolean useThumbs) {
...
public void handleMessage(final Message msg) {
	...
	case CustomFileObserver.NEW_ITEM:
		HybridFile fileCreated =
			new HybridFile(main.openMode, main.getCurrentPath() + "/" + path);
		main.getElementsList().add(
			fileCreated.generateLayoutElement(main.getContext(), useThumbs));
		break;
	...
	if (listView.getVisibility() == View.VISIBLE) {
		if (main.getElementsList().size() == 0) {
			...
			((RecyclerAdapter)listView.getAdapter())
			.setItems(listView, main.getElementsList());
		}
	} else {
