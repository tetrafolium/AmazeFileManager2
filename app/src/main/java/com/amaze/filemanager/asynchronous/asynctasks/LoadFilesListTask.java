...
public class LoadFilesListTask
	extends AsyncTask<Void, Void,
	                  Pair<OpenMode, ArrayList<LayoutElementParcelable> > > {
...
private OnAsyncTaskFinished<
	Pair<OpenMode, ArrayList<LayoutElementParcelable> > > listener;

public LoadFilesListTask(
	final Context c, final String path, final MainFragment ma,
	final OpenMode openmode, final boolean showThumbs,
	final boolean showHiddenFiles,
	final OnAsyncTaskFinished<
		Pair<OpenMode, ArrayList<LayoutElementParcelable> > > l) {
	this.path = path;
	this.ma = ma;
...
@Override
protected Pair<OpenMode, ArrayList<LayoutElementParcelable> >
doInBackground(final Void... p) {
	...
	case ONEDRIVE:
		...
		try {
			...
			e.printStackTrace();
			AppConfig.toast(
				c, c.getResources().getString(R.string.failed_no_connection));
			return new Pair<>(openmode, list);
		}
		break;
	default:
		// we're neither in OTG not in SMB, load the list based on root/general
		// filesystem
		list = new ArrayList<>();
		RootHelper.getFiles(path, ma.getMainActivity().isRootExplorer(),
		                    showHiddenFiles, mode->openmode = mode, file->{
				LayoutElementParcelable elem =
					createListParcelables(file);
				if (elem != null)
					list.add(elem);
			});
		break;
	...
	if (list != null && !(openmode == OpenMode.CUSTOM &&
...
@Override
protected void
onPostExecute(final Pair<OpenMode, ArrayList<LayoutElementParcelable> > list) {
	super.onPostExecute(list);
	listener.onAsyncTaskFinished(list);
}

private LayoutElementParcelable
createListParcelables(final HybridFileParcelable baseFile) {
	if (!dataUtils.isFileHidden(baseFile.getPath())) {
		...
		LayoutElementParcelable layoutElement = new LayoutElementParcelable(
			c, baseFile.getName(), baseFile.getPath(), baseFile.getPermission(),
			baseFile.getLink(), size, longSize, false, baseFile.getDate() + "",
			baseFile.isDirectory(), showThumbs, baseFile.getMode());
...
private ArrayList<LayoutElementParcelable> listImages() {
	...
	final Cursor cursor = c.getContentResolver().query(
...
private ArrayList<LayoutElementParcelable> listDocs() {
	...
	Cursor cursor =
		c.getContentResolver().query(MediaStore.Files.getContentUri("external"),
		                             projection, null, null, null);
	String[] types = new String[] {
		".pdf",        ".xml", ".html", ".asm",  ".text/x-asm", ".def",
		".in",         ".rc",  ".list", ".log",  ".pl",         ".prop",
		".properties", ".rc",  ".doc",  ".docx", ".msg",        ".odt",
		".pages",      ".rtf", ".txt",  ".wpd",  ".wps"
	};
	if (cursor == null)
		return docs;
	else if (cursor.getCount() > 0 && cursor.moveToFirst()) {
	...
	cursor.close();
	Collections.sort(
		docs, (lhs, rhs)->- 1 * Long.valueOf(lhs.date).compareTo(rhs.date));
	if (docs.size() > 20)
		...
		if (!f.equals("/")) {
			...
			if (hybridFileParcelable != null) {
				...
				if (!hybridFileParcelable.isSmb() &&
					...
					LayoutElementParcelable parcelable =
						createListParcelables(hybridFileParcelable);
					if (parcelable != null)
						songs.add(parcelable);
				}
			}
...
private ArrayList<LayoutElementParcelable> listRecentFiles() {
	...
	Date d = c.getTime();
	Cursor cursor = this.c.getContentResolver().query(
	...
	Collections.sort(
		recentFiles,
		(lhs, rhs)->- 1 * Long.valueOf(lhs.date).compareTo(rhs.date));
 ...
 *     com.amaze.filemanager.utils.OTGUtil#PREFIX_OTG}
...
private void listCloud(final String path, final CloudStorage cloudStorage,
                       final OpenMode openMode,
                       final OnFileFound fileFoundCallback)
