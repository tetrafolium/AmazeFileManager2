...
/**
 * Created by arpitkh996 on 12-01-2016, modified by Emmanuel
 * Messulam<emmanuelbendavid@gmail.com>
 *
 *  This AsyncTask works by creating a tree where each folder that can be
 * fusioned together with another in the destination is a node (CopyNode). While
 * the tree is being created an indeterminate ProgressDialog is shown. Each node
 * is copied when the conflicts are dealt with (the dialog is shown, and the
 * tree is walked via a BFS). If the process is cancelled (via the button in the
 * dialog) the dialog closes without any more code to be executed,
 * finishCopying() is never executed so no changes are made.
 */
public class PrepareCopyTask
    extends AsyncTask<ArrayList<HybridFileParcelable>, String,
                      PrepareCopyTask.CopyNode> {

  private enum DO_FOR_ALL_ELEMENTS { DO_NOT_REPLACE, REPLACE }
  ...
  private final ArrayList<ArrayList<HybridFileParcelable>>
      filesToCopyPerFolder = new ArrayList<>();
  private ArrayList<HybridFileParcelable>
      filesToCopy; // a copy of params sent to this

  public PrepareCopyTask(final MainFragment ma, final String path,
                         final Boolean move, final MainActivity con,
  ...
  protected void onPreExecute() {
    ...
    dialog = ProgressDialog.show(context, "",
                                 context.getString(R.string.processing), true);
  }
  ...
  @Override
  protected CopyNode
  doInBackground(final ArrayList<HybridFileParcelable>... params) {
    filesToCopy = params[0];
    long totalBytes = 0;

    if (openMode == OpenMode.OTG || openMode == OpenMode.DROPBOX ||
        openMode == OpenMode.BOX || openMode == OpenMode.GDRIVE ||
        openMode == OpenMode.ONEDRIVE || openMode == OpenMode.ROOT) {
  ...
  private ArrayList<HybridFileParcelable>
  checkConflicts(final ArrayList<HybridFileParcelable> filesToCopy,
                 final HybridFile destination) {
  ...
  protected void onPostExecute(final CopyNode copyFolder) {
    ...
    if (openMode == OpenMode.OTG || openMode == OpenMode.GDRIVE ||
        openMode == OpenMode.DROPBOX || openMode == OpenMode.BOX ||
        openMode == OpenMode.ONEDRIVE || openMode == OpenMode.ROOT) {
  ...
  private void startService(final ArrayList<HybridFileParcelable> sourceFiles,
                            final String target, final OpenMode openmode) {
    Intent intent = new Intent(context, CopyService.class);
    intent.putParcelableArrayListExtra(CopyService.TAG_COPY_SOURCES,
                                       sourceFiles);
  ...
  private void
  onEndDialog(final String path,
              final ArrayList<HybridFileParcelable> filesToCopy,
              final ArrayList<HybridFileParcelable> conflictingFiles) {
    if (conflictingFiles != null && counter != conflictingFiles.size() &&
      ...
      CopyNode c = !copyFolder.hasStarted() ? copyFolder.startCopy()
                                            : copyFolder.goToNextNode();
  ...
  private void
  doNotReplaceFiles(final String path,
                    final ArrayList<HybridFileParcelable> filesToCopy,
                    final ArrayList<HybridFileParcelable> conflictingFiles) {
  ...
  private void
  replaceFiles(final String path,
               final ArrayList<HybridFileParcelable> filesToCopy,
               final ArrayList<HybridFileParcelable> conflictingFiles) {
    if (counter < conflictingFiles.size()) {
      if (dialogState != null) {
  ...
  private void finishCopying(
      ...
      final ArrayList<ArrayList<HybridFileParcelable>> filesToCopyPerFolder) {
    for (int i = 0; i < filesToCopyPerFolder.size(); i++) {
      if (filesToCopyPerFolder.get(i) == null ||
          filesToCopyPerFolder.get(i).size() == 0) {
    ...
    if (filesToCopyPerFolder.size() != 0) {
      int mode =
          mainActivity.mainActivityHelper.checkFolder(new File(path), context);
      if (mode == MainActivityHelper.CAN_CREATE_FILES &&
          !path.contains("otg:/")) {
        // This is used because in newer devices the user has to accept a
        // permission,
  ...
  class CopyNode {
    ...
    CopyNode(final String p,
      ...
      for (int i = 0; i < conflictingFiles.size(); i++) {
        if (conflictingFiles.get(i).isDirectory()) {
          ...
          nextNodes.add(new CopyNode(
              path + "/" + conflictingFiles.get(i).getName(),
    ...
    boolean hasStarted() { return queue != null; }

    String getPath() { return path; }

    ArrayList<HybridFileParcelable> getFilesToCopy() { return filesToCopy; }

    ArrayList<HybridFileParcelable> getConflictingFiles() {
      return conflictingFiles;
    }

    private CopyNode getUnvisitedChildNode(final Set<CopyNode> visited,
                                           final CopyNode node) {
