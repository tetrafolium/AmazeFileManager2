 ...
 *
 * It works by adding a {@link HostKeyVerifier} that accepts all SSH host keys,
 * then obtain the key shown by server, and return to the task's caller.
 *
 * {@link CountDownLatch} with {@link AtomicReference} combo is used to ensure
 * SSH host key is obtained successfully on returning to the task caller.
 *
 * Mainly used by {@link com.amaze.filemanager.ui.dialogs.SftpConnectDialog} on
 * saving SSH connection settings.
 *
 * @see HostKeyVerifier
 * @see SSHClient#addHostKeyVerifier(String)
 * @see
 *     com.amaze.filemanager.ui.dialogs.SftpConnectDialog#onCreateDialog(Bundle)
 */
