...
public class SearchAsyncTask
	extends AsyncTask<String, HybridFileParcelable, Void> {
...
public SearchAsyncTask(final Activity a,
                       final SearchWorkerFragment.HelperCallbacks l,
                       final String input, final OpenMode openMode,
                       final boolean root, final boolean regex,
...
/**
 * Recursively search for occurrences of a given text in file names and
 * publish the result
 *
...
private void search(final HybridFile directory, final SearchFilter filter) {
	if (directory.isDirectory(
		    activity.get())) { // do you have permission to read this directory?
		...
		Log.d(TAG,
		      "Cannot search " + directory.getPath() + ": Permission Denied");
	}
}
...
/**
 * Recursively find a java regex pattern {@link Pattern} in the file names and
 * publish the result
 *
...
/**
 * Recursively match a java regex pattern {@link Pattern} with the file names
...
public interface SearchFilter { boolean searchFilter(String fileName); }
}
