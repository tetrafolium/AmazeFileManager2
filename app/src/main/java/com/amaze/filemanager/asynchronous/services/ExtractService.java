...
public class ExtractService extends AbstractProgressiveService {
  ...
  public int onStartCommand(final Intent intent, final int flags,
    ...
    PendingIntent stopPendingIntent = PendingIntent.getBroadcast(
        context, 1234, stopIntent, PendingIntent.FLAG_UPDATE_CURRENT);
    NotificationCompat.Action action = new NotificationCompat.Action(
        R.drawable.ic_zip_box_grey, getString(R.string.stop_ftp),
        stopPendingIntent);

    mBuilder = new NotificationCompat.Builder(
        context, NotificationConstants.CHANNEL_NORMAL_ID);
  ...
  public class DoWork extends AsyncTask<Void, IOException, Boolean> {
    ...
    private DoWork(final ExtractService extractService,
                   final ProgressHandler progressHandler, final String cpath,
                   final String epath, final String[] entries) {
    ...
    protected Boolean doInBackground(final Void... p) {
      while (!isCancelled()) {
        ...
        final Extractor extractor = CompressedHelper.getExtractorInstance(
            extractService.getApplicationContext(), f, extractionPath,
            new Extractor.OnUpdate() {
              private int sourceFilesProcessed = 0;

              @Override
              public void onStart(final long totalBytes,
                                  final String firstEntryName) {
                // setting total bytes calculated from zip entries
                progressHandler.setTotalSize(totalBytes);

                extractService.addFirstDatapoint(firstEntryName, 1, totalBytes,
                                                 false);
              ...
              public void onUpdate(final String entryPath) {
                ...
                if (entriesToExtract != null) {
                  progressHandler.setSourceFilesProcessed(
        ...
        try {
          ...
          if (PasswordRequiredException.class.isAssignableFrom(e.getClass()) ||
              e.getCause() != null && ZipException.class.isAssignableFrom(
                                          e.getCause().getClass())) {
            ...
            AppConfig.toast(extractService,
                            extractService.getString(R.string.error));
            paused = true;
            publishProgress(e);
          }
        }
    ...
    protected void onProgressUpdate(final IOException... values) {
      ...
      GeneralDialogCreation.showPasswordDialog(
          ...
          R.string.authenticate_password, (dialog, which) -> {
            ...
            ArchivePasswordCache.getInstance().put(
                compressedPath, editText.getText().toString());
    ...
    public void onPostExecute(final Boolean hasInvalidEntries) {
      ...
      // check whether watcherutil was initialized. It was not initialized when
      // we got exception in extracting the file
      if (watcherUtil != null)
    ...
    private void toastOnParseError(final IOException result) {
      Toast
          .makeText(AppConfig.getInstance().getMainActivityContext(),
                    AppConfig.getInstance().getResources().getString(
                        R.string.cannot_extract_archive, compressedPath,
