...
public class CopyService extends AbstractProgressiveService {
  ...
  public int onStartCommand(final Intent intent, final int flags,
    ...
    ArrayList<HybridFileParcelable> files =
        intent.getParcelableArrayListExtra(TAG_COPY_SOURCES);
    String targetPath = intent.getStringExtra(TAG_COPY_TARGET);
    int mode =
        intent.getIntExtra(TAG_COPY_OPEN_MODE, OpenMode.UNKNOWN.ordinal());
    ...
    PendingIntent stopPendingIntent = PendingIntent.getBroadcast(
        c, 1234, stopIntent, PendingIntent.FLAG_UPDATE_CURRENT);
    NotificationCompat.Action action = new NotificationCompat.Action(
        R.drawable.ic_content_copy_white_36dp, getString(R.string.stop_ftp),
        stopPendingIntent);

    mBuilder = new NotificationCompat
                   .Builder(c, NotificationConstants.CHANNEL_NORMAL_ID)
  ...
  public void onDestroy() { this.unregisterReceiver(receiver3); }
  ...
  private class DoInBackground extends AsyncTask<Bundle, Void, Void> {
    ...
    protected Void doInBackground(final Bundle... p1) {
      ...
      progressHandler.setProgressListener(
          (speed) -> publishResults(speed, false, move));

      watcherUtil = new ServiceWatcherUtil(progressHandler);

      addFirstDatapoint(sourceFiles.get(0).getName(), sourceFiles.size(),
                        totalSize, move);
      ...
      if (copy.failedFOps.size() == 0) {
        ...
        // adding/updating new encrypted db entry if any encrypted file was
        // copied/moved
        for (HybridFileParcelable sourceFile : sourceFiles) {
          try {
            ...
            Toast
                .makeText(c, getString(R.string.encryption_fail_copy),
                          Toast.LENGTH_SHORT)
                .show();
          }
        }
    ...
    public void onPostExecute(final Void b) {
      ...
      //  publishResults(b, "", totalSourceFiles, totalSourceFiles, totalSize,
      //  totalSize, 0, true, move);
    ...
    /**
     * Iterates through every file to find an encrypted file and update/add a
     * new entry about it's metadata in the database
     * @param sourceFile the file which is to be iterated
     */
    private void
    findAndReplaceEncryptedEntry(final HybridFileParcelable sourceFile) {

      // even directories can end with CRYPT_EXTENSION
      if (sourceFile.isDirectory() &&
          !sourceFile.getName().endsWith(CryptUtil.CRYPT_EXTENSION)) {
        sourceFile.forEachChildrenFile(getApplicationContext(), isRootExplorer,
                                       file -> {
                                         // iterating each file inside source
                                         // files which were copied to find
                                         // instance of any copied / moved
                                         // encrypted file
        ...
        if (sourceFile.getName().endsWith(CryptUtil.CRYPT_EXTENSION)) {
          try {
            ...
            CryptHandler cryptHandler =
                new CryptHandler(getApplicationContext());
            EncryptedEntry oldEntry =
                cryptHandler.findEntry(sourceFile.getPath());
    ...
    class Copy {
       ...
       */
      public void execute(final ArrayList<HybridFileParcelable> sourceFiles,
                          final String targetPath, final boolean move,
                          final OpenMode mode) {
        ...
        if (FileUtil.checkFolder((targetPath), c) == 1) {
          for (int i = 0; i < sourceFiles.size(); i++) {
            ...
            try {
              ...
              if (targetPath.contains(getExternalCacheDir().getPath())) {
                ...
                hFile = new HybridFile(OpenMode.FILE, targetPath,
                                       sourceFiles.get(i).getName(),
                                       f1.isDirectory());
              } else {

                // the target open mode is where we're currently at
                hFile = new HybridFile(mode, targetPath,
                                       sourceFiles.get(i).getName(),
                                       f1.isDirectory());
              }
          ...
          for (int i = 0; i < sourceFiles.size(); i++) {
            if (!progressHandler.getCancelled()) {
              HybridFile hFile =
                  new HybridFile(mode, targetPath, sourceFiles.get(i).getName(),
      ...
      void copyRoot(final HybridFileParcelable sourceFile,
                    final HybridFile targetFile, final boolean move) {
      ...
      private void copyFiles(final HybridFileParcelable sourceFile,
        ...
        if (sourceFile.isDirectory()) {
          ...
          sourceFile.forEachChildrenFile(c, false, file -> {
            HybridFile destFile =
                new HybridFile(targetFile.getMode(), targetFile.getPath(),
  ...
  // avoid using the method as there is no way to know when we would be
  // returning from command callbacks rather confirm from the command result
  // itself, inside it's callback
  boolean checkFiles(final HybridFile hFile1, final HybridFile hFile2)
      ...
      if (baseFiles.size() > 0) {
        ...
        for (HybridFileParcelable baseFile : baseFiles) {
          if (!checkFiles(
                  ...
                  new HybridFile(hFile2.getMode(), hFile2.getPath() + "/" +
                                                       (baseFile.getName()))))
            b = false;
        }
        return b;
