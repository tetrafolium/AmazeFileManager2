...
public class CloudLoaderAsyncTask extends AsyncTask<Void, Void, Boolean> {
  ...
  public CloudLoaderAsyncTask(final MainActivity mainActivity,
                              final CloudHandler cloudHandler,
                              final Cursor data) {
    this.data = data;
      ...
      do {
        ...
        case 2:
          ...
          try {
            ...
            final MainActivity mainActivity = this.mainActivity.get();
            if (mainActivity != null) {
              cloudStorageDrive = new GoogleDrive(
                  mainActivity.getApplicationContext(), data.getString(1), "",
            ...
            if ((savedCloudEntryGdrive =
                     cloudHandler.findEntry(OpenMode.GDRIVE)) != null) {
              // we already have the entry and saved state, get it

              try {
                cloudStorageDrive.loadAsString(
                    savedCloudEntryGdrive.getPersistData());
              } catch (ParseException e) {
                e.printStackTrace();
                // we need to update the persist string as existing one is been
                // compromised

                cloudStorageDrive.login();
                cloudEntryGdrive = new CloudEntry(
                    OpenMode.GDRIVE, cloudStorageDrive.saveAsString());
            ...
            if (mainActivity != null) {
              AppConfig.toast(mainActivity,
                              R.string.failed_cloud_new_connection);
              mainActivity.deleteConnection(OpenMode.GDRIVE);
            } else {
              cancel(true);
            }
        ...
        case 3:
          ...
          try {
            ...
            if (mainActivity != null) {
              cloudStorageDropbox =
                  new Dropbox(mainActivity.getApplicationContext(),
            ...
            if ((savedCloudEntryDropbox =
                     cloudHandler.findEntry(OpenMode.DROPBOX)) != null) {
              // we already have the entry and saved state, get it
              try {
                cloudStorageDropbox.loadAsString(
                ...
                cloudEntryDropbox = new CloudEntry(
                    OpenMode.DROPBOX, cloudStorageDropbox.saveAsString());
        ...
        case 4:
          ...
          try {
            ...
            if ((savedCloudEntryBox = cloudHandler.findEntry(OpenMode.BOX)) !=
                null) {
              // we already have the entry and saved state, get it
              try {
                cloudStorageBox.loadAsString(
                ...
                cloudEntryBox = new CloudEntry(OpenMode.BOX,
                                               cloudStorageBox.saveAsString());
                cloudHandler.updateEntry(OpenMode.BOX, cloudEntryBox);
              }
            } else {
              cloudStorageBox.login();
              cloudEntryBox =
                  new CloudEntry(OpenMode.BOX, cloudStorageBox.saveAsString());
          ...
          try {
            ...
            if (mainActivity != null) {
              cloudStorageOnedrive =
                  new OneDrive(mainActivity.getApplicationContext(),
                               data.getString(1), data.getString(2));
            } else {
              cancel(true);
              return false;
            ...
            if ((savedCloudEntryOnedrive =
                     cloudHandler.findEntry(OpenMode.ONEDRIVE)) != null) {
              // we already have the entry and saved state, get it
              try {
                cloudStorageOnedrive.loadAsString(
                ...
                cloudEntryOnedrive = new CloudEntry(
                    OpenMode.ONEDRIVE, cloudStorageOnedrive.saveAsString());
        ...
        default:
          ...
          if (mainActivity != null) {
            Toast
                .makeText(mainActivity, R.string.cloud_error_failed_restart,
                          Toast.LENGTH_LONG)
                .show();
          } else
            cancel(true);
  ...
  protected void onCancelled() {
    ...
    final MainActivity mainActivity = this.mainActivity.get();
    if (mainActivity != null) {
      mainActivity.getSupportLoaderManager().destroyLoader(
          MainActivity.REQUEST_CODE_CLOUD_LIST_KEY);
      mainActivity.getSupportLoaderManager().destroyLoader(
          MainActivity.REQUEST_CODE_CLOUD_LIST_KEYS);
    }
  }
