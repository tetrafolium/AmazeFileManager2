...
public class DecryptService extends AbstractProgressiveService {
  ...
  public int onStartCommand(final Intent intent, final int flags,
    ...
    NotificationCompat.Action action = new NotificationCompat.Action(
        R.drawable.ic_folder_lock_open_white_36dp, getString(R.string.stop_ftp),
        stopPendingIntent);
    ...
    startForeground(NotificationConstants.DECRYPT_ID,
  ...
  class BackgroundTask extends AsyncTask<Void, Void, Void> {
    ...
    protected Void doInBackground(final Void... params) {
      String baseFileFolder = baseFile.isDirectory()
                                  ? baseFile.getPath()
                                  : baseFile.getPath().substring(
                                        0, baseFile.getPath().lastIndexOf('/'));
