/*
 * Copyright (C) 2014 Arpit Khurana <arpitkh96@gmail.com>, Emmanuel
...
/**
 * AsyncTask that moves files from source to destination by trying to rename
 * files first, if they're in the same filesystem, else starting the copy
 * service. Be advised - do not start this AsyncTask directly but use {@link
 * PrepareCopyTask} instead
 */
...
public MoveFiles(final ArrayList<ArrayList<HybridFileParcelable> > files,
                 final MainFragment ma, final Context context,
                 final OpenMode mode) {
	mainFrag = ma;
			...
			for (HybridFileParcelable f : files.get(i)) {
				...
				if (!source.renameTo(dest)) {
					...
					if (mainFrag.getMainActivity().isRootExplorer()) {
						try {
							if (!RootUtils.rename(f.getPath(),
							                      paths.get(i) + "/" + f.getName()))
								return false;
	...
	if (movedCorrectly) {
		...
		for (int i = 0; i < paths.size(); i++) {
			for (HybridFileParcelable f : files.get(i)) {
				...
				FileUtils.scanFile(new File(paths.get(i) + "/" + f.getName()),
				                   context);
			}
		}
				...
				for (int i = 0; i < paths.size(); i++) {
				        for (HybridFileParcelable file : files.get(i)) {
				                if (file.getName().endsWith(CryptUtil.CRYPT_EXTENSION)) {
									...
									cryptHandler.findEntry(file.getPath());
		...
		if (destinationSize < totalBytes) {
			...
			Toast
			.makeText(context,
			          context.getResources().getString(R.string.in_safe),
			          Toast.LENGTH_LONG)
			.show();
			return;
		}
		...
		for (int i = 0; i < paths.size(); i++) {
			...
			intent.putExtra(CopyService.TAG_IS_ROOT_EXPLORER,
			                mainFrag.getMainActivity().isRootExplorer());
