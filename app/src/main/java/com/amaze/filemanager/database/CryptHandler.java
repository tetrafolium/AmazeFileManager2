...
public void onCreate(final SQLiteDatabase db) {
	...
	String CREATE_TABLE_ENCRYPTED =
		"CREATE TABLE " + TABLE_ENCRYPTED + "(" + COLUMN_ENCRYPTED_ID +
		" INTEGER PRIMARY KEY," + COLUMN_ENCRYPTED_PATH + " TEXT," +
...
public void clear(final String path) {
	try {
		SQLiteDatabase sqLiteDatabase = getWritableDatabase();
		sqLiteDatabase.delete(TABLE_ENCRYPTED, COLUMN_ENCRYPTED_PATH + " = ?",
		                      new String[] {path});
...
public void updateEntry(final EncryptedEntry oldEncryptedEntry,
                        final EncryptedEntry newEncryptedEntry)
throws GeneralSecurityException, IOException {
	SQLiteDatabase sqLiteDatabase = getWritableDatabase();
	ContentValues contentValues = new ContentValues();
	...
	sqLiteDatabase.update(TABLE_ENCRYPTED, contentValues,
	                      COLUMN_ENCRYPTED_ID + " = ?",
	                      new String[] {oldEncryptedEntry.getId() + ""});
}

public EncryptedEntry findEntry(final String path)
throws GeneralSecurityException, IOException {
	String query = "Select * FROM " + TABLE_ENCRYPTED + " WHERE " +
	               COLUMN_ENCRYPTED_PATH + "= \"" + path + "\"";
