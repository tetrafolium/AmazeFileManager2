...
public class CloudHandler extends SQLiteOpenHelper {
  ...
  public void onCreate(final SQLiteDatabase db) {
    ...
    String CREATE_TAB_TABLE = "CREATE TABLE " + TabHandler.TABLE_TAB + "(" +
                              ...
                              " INTEGER PRIMARY KEY," + TabHandler.COLUMN_PATH +
                              " TEXT," + TabHandler.COLUMN_HOME + " TEXT"
                              + ")";

    String CREATE_TABLE_ENCRYPTED =
        "CREATE TABLE " + TabHandler.TABLE_ENCRYPTED + "(" +
    ...
    String CREATE_TABLE_CLOUD =
        "CREATE TABLE " + CloudHandler.TABLE_CLOUD_PERSIST + "(" +
        CloudHandler.COLUMN_CLOUD_ID + " INTEGER PRIMARY KEY," +
  ...
  public void addEntry(final CloudEntry cloudEntry)
    ...
    contentValues.put(COLUMN_CLOUD_SERVICE,
                      cloudEntry.getServiceType().ordinal());

    try {

      contentValues.put(
          COLUMN_CLOUD_PERSIST,
          CryptUtil.encryptPassword(context, cloudEntry.getPersistData()));
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public void updateEntry(final OpenMode serviceType,
                          final CloudEntry newCloudEntry)
      throws CloudPluginException {
    ...
    contentValues.put(COLUMN_CLOUD_SERVICE,
                      newCloudEntry.getServiceType().ordinal());
    try {

      contentValues.put(
          COLUMN_CLOUD_PERSIST,
          CryptUtil.encryptPassword(context, newCloudEntry.getPersistData()));
    } catch (Exception e) {
      e.printStackTrace();
      // failed to encrypt, revert back to plain
    ...
    sqLiteDatabase.update(TABLE_CLOUD_PERSIST, contentValues,
                          COLUMN_CLOUD_SERVICE + " = ?",
                          new String[] {serviceType.ordinal() + ""});
  }

  public CloudEntry findEntry(final OpenMode serviceType)
      throws CloudPluginException {

    if (!CloudSheetFragment.isCloudProviderAvailable(context))
      throw new CloudPluginException();

    String query = "Select * FROM " + TABLE_CLOUD_PERSIST + " WHERE " +
                   COLUMN_CLOUD_SERVICE + "= \"" + serviceType.ordinal() + "\"";
    ...
    if (cursor.moveToFirst()) {
      ...
      try {
        cloudEntry.setPersistData(
            CryptUtil.decryptPassword(context, cursor.getString(2)));
      } catch (Exception e) {
        e.printStackTrace();
        cloudEntry.setPersistData("");
        return cloudEntry;
