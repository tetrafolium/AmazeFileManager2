...
public class UtilsHandler extends SQLiteOpenHelper {
  ...
  private static final int DATABASE_VERSION =
      3; // increment only when making change in schema
  ...
  private String queryList = "CREATE TABLE IF NOT EXISTS " + TABLE_LIST + " (" +
                             COLUMN_ID + " INTEGER PRIMARY KEY," + COLUMN_PATH +
                             " TEXT UNIQUE"
                             + ");";
  ...
  private String queryBookmarks = "CREATE TABLE IF NOT EXISTS " +
                                  TABLE_BOOKMARKS + " (" + COLUMN_ID +
                                  " INTEGER PRIMARY KEY," + COLUMN_NAME +
                                  " TEXT," + COLUMN_PATH + " TEXT UNIQUE"
                                  + ");";
  ...
  private static final String querySftp =
      "CREATE TABLE IF NOT EXISTS " + TABLE_SFTP + " (" + COLUMN_ID +
      " INTEGER PRIMARY KEY," + COLUMN_NAME + " TEXT," + COLUMN_PATH +
      " TEXT UNIQUE," + COLUMN_HOST_PUBKEY + " TEXT," +
      COLUMN_PRIVATE_KEY_NAME + " TEXT," + COLUMN_PRIVATE_KEY + " TEXT"
      + ");";
  ...
  public void onUpgrade(final SQLiteDatabase db, final int oldVersion,
    ...
    case 2:
      ...
      db.execSQL("INSERT INTO " + backupTable + " SELECT * FROM " +
                 TABLE_HISTORY + " group by path;");
      ...
      db.execSQL("INSERT INTO " + backupTable + " SELECT * FROM " + TABLE_LIST +
                 " group by path;");
      ...
      db.execSQL("INSERT INTO " + backupTable + " SELECT * FROM " + TABLE_GRID +
                 " group by path;");
      ...
      db.execSQL("INSERT INTO " + backupTable + " SELECT * FROM " +
                 TABLE_BOOKMARKS + " group by path;");
      db.execSQL("DROP TABLE " + TABLE_BOOKMARKS + ";");
      db.execSQL("ALTER TABLE " + backupTable + " RENAME TO " +
                 TABLE_BOOKMARKS + ";");

      backupTable = TEMP_TABLE_PREFIX + TABLE_SMB;
      db.execSQL(querySmb.replace(TABLE_SMB, backupTable));
      db.execSQL("INSERT INTO " + backupTable + " SELECT * FROM " + TABLE_SMB +
                 " group by path;");
      ...
      db.execSQL("INSERT INTO " + backupTable + " SELECT * FROM " + TABLE_SFTP +
                 " group by path;");
  ...
  public enum Operation { HISTORY, HIDDEN, LIST, GRID, BOOKMARKS, SMB, SFTP }
  ...
  public void addCommonBookmarks() {
    ...
    String[] dirs = new String[] {
        sd + Environment.DIRECTORY_DCIM, sd + Environment.DIRECTORY_DOWNLOADS,
        sd + Environment.DIRECTORY_MOVIES, sd + Environment.DIRECTORY_MUSIC,
        sd + Environment.DIRECTORY_PICTURES};

    for (String dir : dirs) {
      saveToDatabase(
          new OperationData(Operation.BOOKMARKS, new File(dir).getName(), dir));
    }
  }

  public void addSsh(final String name, final String path, final String hostKey,
  ...
  public void updateSsh(final String connectionName,
                        final String oldConnectionName, final String path,
                        final String sshKeyName, final String sshKey) {
    ...
    database.update(getTableForOperation(Operation.SFTP), values,
  ...
  public LinkedList<String> getHistoryLinkedList() {
    ...
    Cursor cursor =
        sqLiteDatabase.query(getTableForOperation(Operation.HISTORY), null,
  ...
  public ConcurrentRadixTree<VoidValue> getHiddenFilesConcurrentRadixTree() {
    ...
    Cursor cursor =
        getReadableDatabase().query(getTableForOperation(Operation.HIDDEN),
                                    null, null, null, null, null, null);
    boolean hasNext = cursor.moveToFirst();
    while (hasNext) {
      paths.put(cursor.getString(cursor.getColumnIndex(COLUMN_PATH)),
                VoidValue.SINGLETON);
      hasNext = cursor.moveToNext();
    }
    cursor.close();
  ...
  public ArrayList<String> getListViewList() { return getPath(Operation.LIST); }

  public ArrayList<String> getGridViewList() { return getPath(Operation.GRID); }
  ...
  public ArrayList<String[]> getBookmarksList() {
    ...
    Cursor cursor =
        sqLiteDatabase.query(getTableForOperation(Operation.BOOKMARKS), null,
                             null, null, null, null, null);
    ...
    while (hasNext) {
      row.add(
          new String[] {cursor.getString(cursor.getColumnIndex(COLUMN_NAME)),
  ...
  public ArrayList<String[]> getSmbList() {
    ...
    Cursor cursor = sqLiteDatabase.query(getTableForOperation(Operation.SMB),
                                         null, null, null, null, null, null);
    boolean hasNext = cursor.moveToFirst();
    ArrayList<String[]> row = new ArrayList<>();
    while (hasNext) {
  ...
  public List<String[]> getSftpList() {
    ...
    Cursor cursor =
        sqLiteDatabase.query(getTableForOperation(Operation.SFTP),
                             new String[] {COLUMN_NAME, COLUMN_PATH}, null,
    ...
    while (hasNext) {
      ...
      if (path == null) {
        ...
        retval.add(new String[] {
            cursor.getString(cursor.getColumnIndex(COLUMN_NAME)), path});
      }
      hasNext = cursor.moveToNext();
    }
    cursor.close();
  ...
  public String getSshHostKey(final String uri) {
    ...
    if (uri != null) {
      ...
      Cursor result = sqLiteDatabase.query(
          TABLE_SFTP, new String[] {COLUMN_HOST_PUBKEY}, COLUMN_PATH + " = ?",
  ...
  private String getSshAuthPrivateKeyColumn(final String uri,
                                            final String columnName) {
    // If connection is using key authentication, no need to decrypt the path at
    // all
    SQLiteDatabase sqLiteDatabase = getReadableDatabase();
  ...
  private void removeBookmarksPath(final String name, final String path) {
    ...
    sqLiteDatabase.delete(TABLE_BOOKMARKS,
  ...
  /**
   * Remove SMB entry
   * @param path the path we get from saved runtime variables is a decrypted, to
   *     remove entry,
  ...
  private void removeSmbPath(final String name, final String path) {
    ...
    try {
      ...
      sqLiteDatabase.delete(
          TABLE_SMB, COLUMN_NAME + " = ? AND " + COLUMN_PATH + " = ?",
  ...
  private void removeSftpPath(final String name, final String path) {
    ...
    try {
      ...
      sqLiteDatabase.delete(
          TABLE_SFTP, COLUMN_NAME + " = ? AND " + COLUMN_PATH + " = ?",
  ...
  public void renameBookmark(final String oldName, final String oldPath,
                             final String newName, final String newPath) {
    renamePath(Operation.BOOKMARKS, oldName, oldPath, newName, newPath);
  }

  public void renameSMB(final String oldName, final String oldPath,
                        final String newName, final String newPath) {
  ...
  private void setPath(final Operation operation, final String path) {
    ...
    if (Operation.HISTORY.equals(operation))
      sqLiteDatabase.delete(getTableForOperation(operation), "path = ?",
                            new String[] {path});

    sqLiteDatabase.insert(getTableForOperation(operation), null, contentValues);
  }

  private void setPath(final Operation operation, final String name,
                       final String path) {
    SQLiteDatabase sqLiteDatabase = getWritableDatabase();
    ContentValues contentValues = new ContentValues();
  ...
  private void renamePath(final Operation operation, final String name,
                          final String path) {
    SQLiteDatabase sqLiteDatabase = getWritableDatabase();
    ContentValues contentValues = new ContentValues();
    contentValues.put(COLUMN_NAME, name);
  ...
  private void renamePath(final Operation operation, final String oldName,
