...
public static int getSortType(final Context context, final String path) {
	...
	final int globalSortby =
		Integer.parseInt(sharedPref.getString("sortby", "0"));
	if (!onlyThis) {
		return globalSortby;
	}
...
public void clear(final String path) {
	try {
		SQLiteDatabase sqLiteDatabase = getWritableDatabase();
		sqLiteDatabase.delete(TABLE_SORT, COLUMN_SORT_PATH + " = ?",
		                      new String[] {path});
...
public Sort findEntry(final String path) {
	...
	String query = "Select * FROM " + TABLE_SORT + " WHERE " +
	               COLUMN_SORT_PATH + "= \"" + path + "\"";
