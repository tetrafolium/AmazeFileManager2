...
public class TabHandler extends SQLiteOpenHelper {
  ...
  public TabHandler(final Context context) {
    ...
    // The call to onUpgrade() is not performed unless getWritableDatabase() is
    // called. See more at
    // https://github.com/TeamAmaze/AmazeFileManager/pull/1262
    getWritableDatabase();
  }
  ...
  public void onCreate(final SQLiteDatabase db) {
    ...
    String CREATE_TABLE_ENCRYPTED =
        "CREATE TABLE IF NOT EXISTS " + TABLE_ENCRYPTED + "(" +
        COLUMN_ENCRYPTED_ID + " INTEGER PRIMARY KEY," + COLUMN_ENCRYPTED_PATH +
        " TEXT," + COLUMN_ENCRYPTED_PASSWORD + " TEXT"
        + ")";

    String CREATE_TABLE_CLOUD =
        "CREATE TABLE IF NOT EXISTS " + CloudHandler.TABLE_CLOUD_PERSIST + "(" +
    ...
    String CREATE_TABLE_SORT =
        "CREATE TABLE IF NOT EXISTS " + SortHandler.TABLE_SORT + "(" +
  ...
  public void clear() {
    try {
      ...
      sqLiteDatabase.delete(TABLE_TAB, COLUMN_TAB_NO + " = ?",
                            new String[] {"" + 1});
      sqLiteDatabase.delete(TABLE_TAB, COLUMN_TAB_NO + " = ?",
                            new String[] {"" + 2});
  ...
  public Tab findTab(final int tabNo) {
    String query = "Select * FROM " + TABLE_TAB + " WHERE " + COLUMN_TAB_NO +
                   "= \"" + tabNo + "\"";
    SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
  ...
  public List<Tab> getAllTabs() {
    ...
    Cursor cursor =
        sqLiteDatabase.query(TABLE_TAB, null, null, null, null, null, null);
    boolean hasNext = cursor.moveToFirst();

    // Looping through all rows and adding them to list
    while (hasNext) {
      Tab tab =
          new Tab(cursor.getInt(0), cursor.getString(1), cursor.getString(2));
      // Adding them to list
      tabList.add(tab);
